...
> Used for tracking history of certain columns on the encntr_occurrence_code table.
...
| 13 | `OCCURRENCE_CD` | DOUBLE | NOT NULL |  | A UB92 occurrence code (Universal Billing). |
| 14 | `OCCURRENCE_CODE_HIST_ID` | DOUBLE | NOT NULL |  | Uniquely identifies a row tracking history of certain columns on the encntr_occurrence_code table. |
| 15 | `OCCURRENCE_CODE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related ENCNTR_OCCURRENCE_CODE record. |
| 16 | `OCCURRENCE_DT_TM` | DATETIME |  |  | Date and time that the UB92 occurrence, indicated by the row, happened. |
...
| 18 | `SEQUENCE` | DOUBLE |  |  | **OBSOLETE** NOT USED AT THIS TIME. Placeholder for sequential order of the occurrence codes. **OBSOLETE** |
| 19 | `TRANSACTION_DT_TM` | DATETIME |  |  | More aptly named activity_dt_tm; holds the current date and time of the system when the row was inserted. This will match the create_dt_tm from the corresponding pm_hist_tracking row. |
...
|--------|--------------|---------------|
| `OCCURRENCE_CODE_ID` | [ENCNTR_OCCURRENCE_CODE](ENCNTR_OCCURRENCE_CODE.md) | `OCCURRENCE_CODE_ID` |
