...
| 5 | `AGENT_IND` | DOUBLE | NOT NULL |  | Determines if the sequencing group is used as an agent or as a sequencing group. |
...
| 16 | `SEQ_GROUP_ID` | DOUBLE | NOT NULL | PK | A unique identifier for a scheduling sequence group. |
| 17 | `SEQ_NBR` | DOUBLE | NOT NULL |  | Determines the order of the object within a collection. |
| 18 | `SEQ_TYPE_CD` | DOUBLE | NOT NULL | FK→ | The coded identifier for the interaction type (interaction, or sequencing only) |
| 19 | `SEQ_TYPE_MEANING` | VARCHAR(12) |  |  | A 12-character description corresponding to the type of interaction. |
...
| `SEQ_TYPE_CD` | [CODE_VALUE](CODE_VALUE.md) | `CODE_VALUE` |

## Referenced by (4)

| From table | Column |
|------------|--------|
| [SCH_APPT_INTER](SCH_APPT_INTER.md) | `SEQ_GROUP_ID` |
| [SCH_ORDER_INTER](SCH_ORDER_INTER.md) | `SEQ_GROUP_ID` |
| [SCH_SEQ_LIST](SCH_SEQ_LIST.md) | `CHILD_SEQ_GROUP_ID` |
| [SCH_SEQ_LIST](SCH_SEQ_LIST.md) | `SEQ_GROUP_ID` |
