...
> Contains the field for an appointment type that are dependent on its current state.
...
| 8 | `DISP_SCHEME_ID` | DOUBLE | NOT NULL | FK→ | The coded identifier of the disp scheme. |
...
| 11 | `SCH_STATE_CD` | DOUBLE | NOT NULL | FK→ | The coded identifier corresponding to the current state of the appointment. |
| 12 | `STATE_MEANING` | VARCHAR(12) | NOT NULL |  | The 12-character string corresponding to the current state of the appointment. |
