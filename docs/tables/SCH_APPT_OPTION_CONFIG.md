...
> Stored appointment option configuration for appointment actions.
...
| 5 | `APPOINTMENT_OPTION` | VARCHAR(100) | NOT NULL |  | The value to return that relates to the appointment level configuration for a scheduling action. |
| 6 | `APPT_TYPE_CD` | DOUBLE | NOT NULL |  | The appointment type code (from scheduling appointment type code set: 14230) the OPTION_VALUE applies to |
...
| 9 | `LOCATION_CD` | DOUBLE | NOT NULL | FK→ | The unit (from location code set: 220) related to the option |
...
| 11 | `OPTION_MEANING` | VARCHAR(12) | NOT NULL |  | The CDF meaning of the input option action |
| 12 | `ORDER_OPTION` | VARCHAR(100) | NOT NULL |  | The value to return that relates to the order level configuration for a scheduling action. |
| 13 | `SCH_APPT_OPTION_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCH_APPT_OPTION_CONFIG table. |
