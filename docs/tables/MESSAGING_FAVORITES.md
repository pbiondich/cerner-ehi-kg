...
> Stores favorite recipient lists for providers and patients. Favorites recipients can be associated with a variety of entity types (Pool, Personnel, Distribution List, Email, etc).
...
| 2 | `FAVORITE_ID` | DOUBLE | NOT NULL |  | The ID of the table SEQUENCE NAME: CARENET_SEQ |
| 3 | `FAVORITE_TYPE_CD` | DOUBLE | NOT NULL |  | The type code value that indicates the type of favorite. |
| 4 | `MODE_FLAG` | DOUBLE | NOT NULL |  | the mode of which the favorites was established(either favorites was used for Inbox delivery, fax delivery, secure email delivery or mail delivery) |
| 5 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | The ID of the parent entity related to the parent entity name. |
| 6 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | The parent entity name that indicates the type of entity. |
| 7 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The person the favorite on this line is associated with. |
| 8 | `PRSNL_GROUP_ID` | DOUBLE | NOT NULL | FK→ | The ID of the personnel group who the favorite is associated. |
| 9 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The ID of the personnel who the favorite is associated. |
