...
> This table defines an RCA rule service result.
...
| 2 | `DATA_TYPE_CD` | DOUBLE | NOT NULL |  | The code value of the result's data type. Values come from code set 4124000. |
| 3 | `RCA_RULE_SERVICE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related RCA_RULE_SERVICE. |
| 4 | `RCA_RULE_SERVICE_RESULT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RCA_RULE_SERVICE_RESULT table. |
| 5 | `SERVICE_KEY_TXT` | VARCHAR(100) | NOT NULL |  | The key used in the service that identifies the value for this result. |
