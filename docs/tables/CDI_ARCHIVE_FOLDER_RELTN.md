...
> Relates content management folders to archive configurations.
...
| 2 | `CDI_ARCHIVE_FOLDER_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CDI_ARCHIVE_FOLDER_RELTN table. |
| 3 | `CDI_ARCHIVE_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the CDI_ARCHIVE table. |
| 4 | `CDI_CM_FOLDER_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the CDI_CM_FOLDER table. |
...
|--------|--------------|---------------|
| `CDI_ARCHIVE_ID` | [CDI_ARCHIVE](CDI_ARCHIVE.md) | `CDI_ARCHIVE_ID` |
| `CDI_CM_FOLDER_ID` | [CDI_CM_FOLDER](CDI_CM_FOLDER.md) | `CDI_CM_FOLDER_ID` |
