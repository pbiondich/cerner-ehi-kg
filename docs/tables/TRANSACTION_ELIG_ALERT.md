...
> Use this table to store details for transactions with alerts posted. An alert signifies that the eligibility response contains data that may need to be reviewed.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ALERT_CD` | DOUBLE | NOT NULL |  | An alert signifies that a condition exists within an eligibility transaction that may warrant a manual review. |
| 2 | `TRANSACTION_ELIGIBILITY_ID` | DOUBLE | NOT NULL | FK→ | The identifier of the TRANSACTION_ELIGIBILITY table to which this alert is related. |
| 3 | `TRANSACTION_ELIG_ALERT_ID` | DOUBLE | NOT NULL |  | The unique primary key of this table. It is an internally generated number. |
