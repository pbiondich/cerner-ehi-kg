...
> Cache CommonWell data to reduce the number of exernal calls made.
...
| 5 | `CW_DATA_CACHE_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 6 | `ENROLLED_IND` | DOUBLE |  |  | Determines if enrolled or not. |
| 7 | `NEW_CONNECTIONS_CNT` | DOUBLE |  |  | Number of new connections. |
| 8 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the organization table. It is an internal system assigned number. This column is either valued with the facility or the client organization for the encounter. |
...
| 10 | `RETRIEVED_DT_TM` | DATETIME |  |  | Date and time the status was retrieved from CommonWell. |
| 11 | `RETRIEVED_STATUS_FLAG` | DOUBLE |  |  | A value that identifies the retrieved status. |
