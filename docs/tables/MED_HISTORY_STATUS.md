...
> Track the status of incoming external medical history.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `MED_HISTORY_STATUS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the med_history_status table. |
| 2 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The person_id corresponding to the external medications to be imported. |
| 3 | `START_DT_TM` | DATETIME | NOT NULL |  | The original start time of the transaction |
| 4 | `STATUS_CD` | DOUBLE | NOT NULL |  | The status of the external med history transaction. |
