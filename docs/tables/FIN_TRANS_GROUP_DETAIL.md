...
> Defines the event details of each financial transaction service

**Description:** Financial Transaction Group Detail  
**Table type:** ACTIVITY  
**Primary key:** `FIN_TRANS_GROUP_DETAIL_ID`  
**Columns:** 19  
...
| 7 | `FIN_TRANS_GROUP_DETAIL_ID` | DOUBLE | NOT NULL | PK | Unique generataed number that identifies a single row on the PFT_TRANS_GROUP_DETAIL table. |
| 8 | `FIN_TRANS_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related row on the fin_trans_group table. |
| 9 | `HTTP_URL` | VARCHAR(512) |  |  | The URL used to make the HTTP call |
| 10 | `SERVICE_CORRELATION_IDENT` | VARCHAR(50) | NOT NULL |  | The unique identifier that is related to the service. This can be used to retrive the service logs. |
| 11 | `SERVICE_HTTP_STATUS_CODE` | VARCHAR(50) | NOT NULL |  | The HTTP status code that the service returned. |
| 12 | `SERVICE_NAME` | VARCHAR(50) | NOT NULL |  | The name of the service that is being executed ("Payment" or "Adjustment" or "Remark") |
| 13 | `SERVICE_STATUS_CD` | DOUBLE | NOT NULL |  | The status of the service |
...
| From table | Column |
|------------|--------|
| [FIN_TRANS_LONG_TEXT](FIN_TRANS_LONG_TEXT.md) | `FIN_TRANS_GROUP_DETAIL_ID` |
