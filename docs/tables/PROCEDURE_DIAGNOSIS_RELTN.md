...
> Relates Millennium concept Procedure to a Diagnosis
...
| 3 | `CHILD_PROCEDURE_ID` | DOUBLE | NOT NULL | FK→ | Forgein key, Unique generated number that identifies a single row on the PROCEDURE table [Primary key id from the PROCEDURE table] |
...
| 5 | `PARENT_DIAGNOSIS_ID` | DOUBLE | NOT NULL | FK→ | Forgein key, Unique generated number that identifies a single row on the DIAGNOSIS table |
| 6 | `PROCEDURE_DIAGNOSIS_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PROCEDURE_DIAGNOSIS_RELTN table. |
...
|--------|--------------|---------------|
| `CHILD_PROCEDURE_ID` | [PROCEDURE](PROCEDURE.md) | `PROCEDURE_ID` |
| `PARENT_DIAGNOSIS_ID` | [DIAGNOSIS](DIAGNOSIS.md) | `DIAGNOSIS_ID` |
