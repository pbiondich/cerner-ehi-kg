...
> Contains information for the Screening for Metabolic Disorders IPFQR condition

**Description:** LH_ABS_SMD_METRICS  
**Table type:** ACTIVITY  
...
| 4 | `BMI_FLAG` | DOUBLE |  |  | Identifies if BMI was documented in the patient's medical record |
| 5 | `BP_FLAG` | DOUBLE |  |  | Identifies if blood pressure was documented in the patient's medical record |
...
| 10 | `DISC_DISP_FLAG` | DOUBLE |  |  | The discharge disposition on the day of discharge |
...
| 17 | `D_BR_CCN_ID` | DOUBLE | NOT NULL | FK→ | CMS Certification Number. Foreign key to the LH_D_BR_CCN |
| 18 | `D_BR_HCO_ID` | DOUBLE | NOT NULL | FK→ | Healthcare organization Number. Foreign key to the LH_D_BR_HCO |
...
| 27 | `EXCLUDE_1_IND` | DOUBLE |  |  | Identifies if this case meets the criteria to be 'excluded' for the Screening for Metabolic Disorders metric. |
...
| 31 | `GLUCOSE_FLAG` | DOUBLE |  |  | Identifies if blood glucose was documented in the patient's medical record |
| 32 | `HEALTH_SYSTEM_ID` | DOUBLE | NOT NULL |  | Identifies the delivery network responsible for supplying the data. |
| 33 | `HEALTH_SYSTEM_SOURCE_ID` | DOUBLE | NOT NULL | FK→ | Identifies the unique source within the delivery network responsible for supplying the data. |
| 34 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the last ETL process started that updated this record. |
| 35 | `LH_ABS_SMD_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_ABS_SMD_METRICS table. |
| 36 | `LIPID_FLAG` | DOUBLE |  |  | Identifies if a lipid panel was documented in the patient's medical record |
...
| 38 | `NUMERATOR_1_IND` | DOUBLE |  |  | Identifies if this case meets the criteria to be 'met' or 'not met' for the Screening for Metabolic Disorders metric. |
| 39 | `NUM_PSYCH_MED` | VARCHAR(10) |  |  | Indicates the number of antipsychotic medications prescribed for the patient at discharge |
| 40 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Identifies the medical record number of the patient. |
| 41 | `OTH_DIAGNOSIS_LIST` | VARCHAR(250) |  |  | A comma separated list of other diagnoses associated with the encounter. |
| 42 | `OTH_PROCEDURE_DATES_LIST` | VARCHAR(400) |  |  | A comma separated list of dates the other procedures associated with the encounter were performed. |
| 43 | `OTH_PROCEDURE_LIST` | VARCHAR(400) |  |  | A comma separated list of other procedures associated with the encounter. |
| 44 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility |
| 45 | `PAYMENT_SOURCE_FLAG` | DOUBLE |  |  | Identifies the source of payment for the encounter. |
...
| 54 | `PSYCHSETNG_FLAG` | DOUBLE |  |  | Identifies if there is a response for Psychiatric Care Setting. 0 - No, 1 - Yes, 999 - Missing. |
| 55 | `REJECT_1_IND` | DOUBLE |  |  | Identifies if this case meets the criteria to be 'rejected' for the Screening for Metabolic Disorders metric. |
| 56 | `RSNINCOMPMS_FLAG` | DOUBLE |  |  | Identifies if a reason for not performing a metabolic screening was documented in the patient's medical record |
| 57 | `SAMPLE_IND` | DOUBLE |  |  | Indicates if the case was sampled |
| 58 | `SMD1_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason obtained during the completion of abstraction. |
...
| 62 | `UPDT_SOURCE` | VARCHAR(50) |  |  | The script name responsible for updating the reecord. |
