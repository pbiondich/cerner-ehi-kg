...
> Fact table for Restraint Management Lighthouse Reporting

**Description:** LH_F_REST_METRICS  
**Table type:** ACTIVITY  
...
| 6 | `AT_RISK_IND` | DOUBLE |  |  | Identifies if the patient is at risk. |
...
| 15 | `D_ATTEND_PRSNL_ID` | DOUBLE | NOT NULL | FK→ | Identifies the final attending physician associated to the encounter during the visit. |
...
| 45 | `EDUCATION_IND` | DOUBLE |  |  | Identifies patients that has restraint at risk education documented. |
| 46 | `ED_ENCNTR_ID` | DOUBLE |  |  | Identifies if the current or previous encounter was admitted to the ED. In order to qualify- the previous encounter should have been discharged within 30 minutes. |
| 47 | `ED_RESTRAINT_IND` | DOUBLE |  |  | Identifies patients who were restrained on admission to ED |
...
| 52 | `F_REST_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_F_REST_METRICS table. |
...
| 56 | `LOGICAL_DOMAIN_ID` | DOUBLE | NOT NULL |  | The unique identifier for the logical domain. This identifier allows the data to be grouped by logical domain. |
...
| 59 | `PLAN_INITIATED_IND` | DOUBLE |  |  | Identifies patients if the PowerPlan has been initiated or the order set has been ordered. |
| 60 | `POPULATION_IND` | DOUBLE |  |  | Identifies patients that qualify for the ventilated population for the Lighthouse metric. |
...
| 63 | `RESTRAINT_IND` | DOUBLE |  |  | Identifies patients who are restrained |
| 64 | `RISK_DT_TM` | DATETIME |  |  | Identifies when the first at risk document for restraint for patients |
| 65 | `RISK_PRIOR_RESTRAINT_IND` | DOUBLE |  |  | Identifies patients that are at risk- had an restrained document- and were at risk prior to the restraint |
| 66 | `RISK_UTC_DT_TM` | DATETIME |  |  | Identifies when the first at risk document for restraint for patients; normalized to GMT |
| 67 | `UPDT_CNT` | DOUBLE | NOT NULL |  | Set to 0 on insert. Incremented by 1 on update. Used to recognize update conflict where data in a row updated by one application is at risk of being lost by a second application attempting to update the row. |
| 68 | `UPDT_DT_TM` | DATETIME | NOT NULL |  | The date and time the row was last inserted or updated. |
| 69 | `UPDT_SOURCE` | VARCHAR(50) | NOT NULL |  | The script name responsible for updating the record. |
| 70 | `UPDT_TASK` | VARCHAR(50) | NOT NULL |  | The registered (assigned) task number for the process that inserted or updated the row. |
