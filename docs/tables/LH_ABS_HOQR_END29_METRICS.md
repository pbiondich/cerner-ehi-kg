...
> Contains information based on HOQR OP-29: Appropriate Follow-up Interval for Normal Colonoscopy in Average Risk Patients condition for an encounters
...
| 5 | `COMPLETED_DT_TM` | DATETIME |  |  | The Date and Time the abstracted case was completed |
| 6 | `CPTCODE` | VARCHAR(10) |  |  | Identify the CPT code is selected for the patient |
| 7 | `CPTCODEDT_TXT` | VARCHAR(10) |  |  | The date for CPT code performed. User Entered (MM-DD-YYYY) Includes dashes |
| 8 | `CPTCODE_DT_TM` | DATETIME |  |  | The date/time on which CPT code performed. |
| 9 | `CPTCODE_MOD_NUM_TXT` | VARCHAR(10) |  |  | The CPTCODE modifier number. |
| 10 | `CPTCODE_MOD_TXT` | VARCHAR(50) |  |  | The CPTCODE modifier description. Describes what the modifier number means. |
| 11 | `CPTCODE_UTC_DT_TM` | DATETIME |  |  | The The date/time on which CPT code performed. normalized to GMT. |
...
| 20 | `D_BR_CCN_ID` | DOUBLE | NOT NULL | FK→ | CMS Certification Number. Foreign key to the LH_D_BR_CCN |
...
| 32 | `D_PERSON_ID` | DOUBLE | NOT NULL | FK→ | Identifies the person associated with the quality measure. Foreign key to the LH_D_PERSON |
| 33 | `ENCNTR_ID` | DOUBLE | NOT NULL |  | Identifies the encounter against which the quality measure is associated. Foreign key to the ENCOUNTER table. |
...
| 37 | `HCPCSCODE` | VARCHAR(10) |  |  | Identify the HCPCS code is selected for the patient |
| 38 | `HCPCSCODEDT_TXT` | VARCHAR(10) |  |  | The date for HCPCS code performed. User Entered (MM-DD-YYYY) Includes dashes |
| 39 | `HCPCSCODE_DT_TM` | DATETIME |  |  | The date/time on which HCPCS code performed. |
| 40 | `HCPCSCODE_UTC_DT_TM` | DATETIME |  |  | The The date/time on which HCPCS code performed. normalized to GMT. |
...
| 44 | `LH_ABS_HOQR_END29_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_ABS_HOQR_END29_METRICS table. |
...
| 46 | `NUMERATOR_29_IND` | DOUBLE |  |  | Identifies if this case meets the criteria to be 'met' or 'not met' |
| 47 | `OP29_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason obtained during the completion of abstraction. |
| 48 | `OP_COLONBX_FLAG` | DOUBLE |  |  | Screening includes biopsy or polypectomy. |
| 49 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Identifies the medical record number of the patient. |
| 50 | `OTH_DIAGNOSIS_LIST` | VARCHAR(250) |  |  | A comma separated list of other diagnoses associated with the encounter. |
...
| 55 | `PRS_TXT` | VARCHAR(10) |  |  | Identifies the Patient Reliability Score (PRS). |
| 56 | `RECFU10_FLAG` | DOUBLE |  |  | Identifies if recommended 10 year follow-up was documented |
| 57 | `RSNNORECFU10_FLAG` | DOUBLE |  |  | Identifies if a reason for no recommended 10 year follow-up was documented |
| 58 | `SAMPLE_IND` | DOUBLE |  |  | Indicates if the case was sampled |
