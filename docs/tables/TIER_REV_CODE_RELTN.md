...
> This table stores all revenue codes for one row in procedure_code_tier
...
| 7 | `PROCEDURE_CODE_TIER_ID` | DOUBLE | NOT NULL | FK→ | Unique identifier to the procedure_code_tier table |
| 8 | `REV_CODE` | VARCHAR(200) |  |  | Display value for revenue code from code_set 20769 |
| 9 | `SEQUENCE` | DOUBLE | NOT NULL |  | The sequence of how the revenue code gets added in the build tool |
| 10 | `TIER_REV_CODE_RELTN_ID` | DOUBLE | NOT NULL |  | Primary key |
...
|--------|--------------|---------------|
| `PROCEDURE_CODE_TIER_ID` | [PROCEDURE_CODE_TIER](PROCEDURE_CODE_TIER.md) | `PROCEDURE_CODE_TIER_ID` |
