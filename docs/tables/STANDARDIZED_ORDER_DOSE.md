...
| 5 | `RELATIONAL_OPERATOR_FLAG` | DOUBLE | NOT NULL |  | Identifies the relational operator for comparison with compare_value1 and compare_value2. |
| 6 | `ROUTE_CD` | DOUBLE | NOT NULL |  | Identifies a specific route of administration for medication. |
| 7 | `STANDARDIZED_ORDER_DOSE_ID` | DOUBLE | NOT NULL |  | Identifies the standardized dose range for the ordering pharmaceutical |
| 8 | `STD_DOSE_UNIT_CD` | DOUBLE | NOT NULL |  | Unit of measure for standard dose |
| 9 | `STD_DOSE_VALUE` | DOUBLE | NOT NULL |  | The standard dose value |
| 10 | `SYNONYM_ID` | DOUBLE | NOT NULL | FK→ | Identifies the order catalog synonym for the standardized order dose |
