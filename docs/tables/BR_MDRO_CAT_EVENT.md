...
> Stores information about Events and their Multiple Drug Resistance Organism categories.

**Description:** Bedrock Multiple Drug Resistance Organism Category Event  
**Table type:** REFERENCE  
**Primary key:** `BR_MDRO_CAT_EVENT_ID`  
**Columns:** 12  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BR_MDRO_CAT_EVENT_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the BR_MDRO_CAT_EVENT table. |
| 2 | `BR_MDRO_CAT_ID` | DOUBLE | NOT NULL | FK→ | Stores Category ID of the Category this Event CD is associated to. |
| 3 | `BR_MDRO_ID` | DOUBLE | NOT NULL | FK→ | A pointer to the name and other base information of the MDRO. |
| 4 | `EVENT_CD` | DOUBLE | NOT NULL |  | Stores Code value of the Event cd (Serology Results) that is defined as Multiple Drug Resistant." |
| 5 | `LOCATION_CD` | DOUBLE | NOT NULL | FK→ | Indicates the MDRO category and event combination is only valid for the given location code_value. |
| 6 | `LOOKBACK_TIME_SPAN_NBR` | DOUBLE | NOT NULL |  | Time period used for building a worklist. |
| 7 | `LOOKBACK_TIME_SPAN_UNIT_CD` | DOUBLE | NOT NULL |  | The unit of measure for the lookback time span. |
...
| From table | Column |
|------------|--------|
| [BR_CAT_EVENT_NORMALCY](BR_CAT_EVENT_NORMALCY.md) | `BR_MDRO_CAT_EVENT_ID` |
