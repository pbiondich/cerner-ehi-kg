...
> Stores the work list configuration for each logical domain.
...
| 5 | `CONFIGURATION_BLOB` | LONGBLOB |  |  | Contains the configurations value |
...
| 7 | `PM_QUE_WORK_LIST_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique system generated number used to identify a single row on the PM_QUE_WORK_LIST_CONFIG table. |
| 8 | `PM_QUE_WORK_LIST_ID` | DOUBLE |  | FK→ | PM Queue Work List ID from PM_QUE_WORK_LIST table to which the configuration is related. |
...
| `PM_QUE_WORK_LIST_ID` | [PM_QUE_WORK_LIST](PM_QUE_WORK_LIST.md) | `WORK_LIST_ID` |
