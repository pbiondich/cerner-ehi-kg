...
> Encounter based table for ProFile Chart Abstracting information
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ABSTRACTING_ID` | DOUBLE | NOT NULL |  | This is the value of the unique primary identifier of the abstracting table. It is an internal system assigned number. |
...
| 7 | `COMPLETED_DT_TM` | DATETIME |  |  | The date and time that the abstracting information for an encounter was saved as final. |
| 8 | `CREATE_DT_TM` | DATETIME |  |  | The date that the record was created in the table. |
...
| 11 | `ENCNTR_SLICE_ID` | DOUBLE | NOT NULL | FK→ | Identifies an Encounter as it relates to a time slice. |
...
| 14 | `SVC_CAT_HIST_ID` | DOUBLE | NOT NULL | FK→ | This is a unique identifier for the service category history table. |
