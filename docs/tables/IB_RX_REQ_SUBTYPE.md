...
> Contains subtypes related to prescription change requests from a foreign pharmacy system.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `IB_RX_REQ_ID` | DOUBLE | NOT NULL | FK→ | Reference identifier of the eRx related to the sub type. |
| 2 | `IB_RX_REQ_SUBTYPE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the IB_RX_REQ_SUBTYPE table. |
| 3 | `SUBTYPE_CD` | DOUBLE | NOT NULL |  | SUB Type for the erx request . Example include AUTHORITY, NADEAN. CS 4624006 |
