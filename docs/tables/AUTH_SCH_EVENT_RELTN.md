...
> Stores information about a relationship between an authorization and a schedule event.

**Description:** Authorization Schedule Event Relationship  
**Table type:** ACTIVITY  
**Primary key:** `AUTH_SCH_EVENT_RELTN_ID`  
**Columns:** 12  
...
| 5 | `AUTHORIZATION_ID` | DOUBLE | NOT NULL | FK→ | Unique system generated number that identifies a single row on the AUTHORIZATION table to which the schedule event is related. |
| 6 | `AUTH_SCH_EVENT_RELTN_ID` | DOUBLE | NOT NULL | PK | Unique system generated number that identifies a single row on the AUTH_SCH_EVENT_RELTN table. |
| 7 | `SCH_EVENT_ID` | DOUBLE | NOT NULL | FK→ | Unique system generated number that identifies a single row on the SCH_EVENT table. |
...
| From table | Column |
|------------|--------|
| [AUTH_SCH_EVENT_RELTN_HIST](AUTH_SCH_EVENT_RELTN_HIST.md) | `AUTH_SCH_EVENT_RELTN_ID` |
