# LH_E_HYBRID_2025_METRICS

> Stores data gathered by the LH_E_HYBRID_2025 script.

**Description:** Lighthouse eMeasures Hybrid 2025 Metrics  
**Table type:** ACTIVITY  
...
| 40 | `HEART_RATE_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first heart rate recorded for the episode of care. |
| 41 | `HEART_RATE_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first heart rate recorded for the episode of care. |
| 42 | `HWM_IPP_IND` | DOUBLE |  |  | Indicates that the encounter is in Initial population for Hybrid Mortality Measure. |
| 43 | `HWM_RESULT_IND` | DOUBLE |  |  | Indicates that at least one of the results exists for Hybrid Mortality Measure. |
| 44 | `HWR_IPP_IND` | DOUBLE |  |  | Indicates that the encounter is in Initial population for Hybrid Re-admission Measure. |
| 45 | `HWR_RESULT_IND` | DOUBLE |  |  | Indicates that at least one of the results exists for Hybrid Re-admission Measure. |
| 46 | `IP_ADMIT_DT_TM` | DATETIME |  |  | The start of the inpatient encounter. |
| 47 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the last ETL process started that updated this record. |
| 48 | `LH_E_HYBRID_2025_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_HYBRID_2025_METRICS table. |
| 49 | `LOGICAL_DOMAIN_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, If you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
| 50 | `MBI_TXT` | VARCHAR(50) |  |  | The Medicare MBI number is the identification number given to a patient who is covered by Medicare |
| 51 | `MEDICARE_ADV_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display for Medicare Advantage type. |
| 52 | `MEDICARE_FFS_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display for Medicare Fee-For-Service type. |
| 53 | `MED_SERVICE_TXT` | VARCHAR(40) |  |  | The text for the medical service of the encounter. |
...
| 56 | `OBS_SERV_ARRIVAL_DT_TM` | DATETIME |  |  | The start of the observation encounter. |
| 57 | `OBS_SERV_DEPART_DT_TM` | DATETIME |  |  | The end of the observation encounter. |
| 58 | `OBS_SERV_ENCNTR_ID` | DOUBLE | NOT NULL |  | The ID of the observation services encounter associated to the record. |
| 59 | `OP_SURG_ARRIVAL_DT_TM` | DATETIME |  |  | The start of the outpatient surgery encounter. |
| 60 | `OP_SURG_DEPART_DT_TM` | DATETIME |  |  | The end of the outpatient surgery encounter. |
| 61 | `OP_SURG_ENCNTR_ID` | DOUBLE | NOT NULL |  | The ID of the outpatient surgery encounter associated to the record. |
| 62 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Medical Record Number of the record. |
| 63 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility. |
| 64 | `PATIENT_IDENT` | VARCHAR(50) |  |  | Identifies the earliest documented medical record number of the patient. |
| 65 | `PERSON_ETHNIC_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store ethnicity display. |
| 66 | `PERSON_GENDER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store gender display. |
| 67 | `PERSON_ID` | DOUBLE | NOT NULL |  | The ID of the person associated to the record. |
| 68 | `PERSON_PAYER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display. |
| 69 | `PERSON_RACE_DESC` | VARCHAR(300) |  |  | String to store multiple races. |
| 70 | `PLATELET_LAB_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first platelet count recorded for the episode of care. |
| 71 | `PLATELET_LAB_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first platelet count recorded for the episode of care. |
| 72 | `POTAS_LAB_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first potassium lab test for the episode of care. |
| 73 | `POTAS_LAB_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first potassium lab test for the episode of care. |
| 74 | `RESP_RATE_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first respiratory rate recorded for the episode of care. |
| 75 | `RESP_RATE_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first respiratory rate recorded for the episode of care. |
| 76 | `SBP_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first systolic blood pressure recorded for the episode of care. |
| 77 | `SBP_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first systolic blood pressure recorded for the episode of care. |
| 78 | `SCRIPT_VERSION` | DOUBLE |  |  | The version number of the script that populated the row |
| 79 | `SOD_LAB_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first sodium lab test for the episode of care. |
| 80 | `SOD_LAB_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first sodium lab test for the episode of care. |
| 81 | `SRC_UPDT_DT_TM` | DATETIME |  |  | The date and time the row was last inserted or updated to be preserved during extracts. |
| 82 | `SRC_UPDT_SOURCE` | VARCHAR(50) |  |  | The script name responsible for updating the record to be preserved during extracts. |
| 83 | `TIMEZONE_TXT` | VARCHAR(300) |  |  | The timezone of the record in ""America/Chicago"" format. |
...
| 87 | `UPDT_TASK` | VARCHAR(50) |  |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 88 | `WBC_LAB_RSLT_DT_TM` | DATETIME |  |  | The date/time of the first white blood cells count lab test for the episode of care. |
| 89 | `WBC_LAB_RSLT_TXT` | VARCHAR(30) |  |  | The result of the first white blood cells count lab test for the episode of care. |
