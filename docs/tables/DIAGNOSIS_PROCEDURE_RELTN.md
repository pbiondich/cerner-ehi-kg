...
> Relates Millennium concept Diagnosis to a Procedure
...
| 3 | `CHILD_DIAGNOSIS_ID` | DOUBLE |  | FK→ | Foreign key, Unique generated number that identifies a single row on the DIAGNOSIS table [Primary key id from the DIAGNOSIS table]. |
| 4 | `DIAGNOSIS_PROCEDURE_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the DIAGNOSIS_PROCEDURE_RELTN table. |
...
| 6 | `PARENT_PROCEDURE_ID` | DOUBLE |  | FK→ | Foreign key, Unique generated number that identifies a single row on the PROCEDURE table [Primary key id from the PROCEDURE table]. |
...
|--------|--------------|---------------|
| `CHILD_DIAGNOSIS_ID` | [DIAGNOSIS](DIAGNOSIS.md) | `DIAGNOSIS_ID` |
| `PARENT_PROCEDURE_ID` | [PROCEDURE](PROCEDURE.md) | `PROCEDURE_ID` |
