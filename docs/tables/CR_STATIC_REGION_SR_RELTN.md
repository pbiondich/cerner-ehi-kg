...
> This table stores the relationships between Report Static Regions (Page Masters) and service resources for a given date range.
...
| 3 | `CR_STATIC_REGION_SR_RELTN_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
...
| 5 | `SERVICE_RESOURCE_CD` | DOUBLE | NOT NULL | FK→ | A unique internal identifier for this service resource. CODE SET:221 |
| 6 | `STATIC_REGION_ID` | DOUBLE | NOT NULL | FK→ | The id of the static region with which this row is associated. |
