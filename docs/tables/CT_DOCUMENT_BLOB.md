...
> This table is used to store the physical document blob for documents saved for a clinical trial.
...
| 2 | `BLOB_LENGTH` | DOUBLE | NOT NULL |  | Original length of the blob |
| 3 | `CT_DOCUMENT_BLOB_ID` | DOUBLE | NOT NULL |  | Unique identifier for each row (PK). Referenced by the ct_document_version table. |
| 4 | `CT_DOCUMENT_VERSION_ID` | DOUBLE | NOT NULL | FK→ | FOREIGN KEY FROM CT_DOCUMENT_VERSION |
| 5 | `LONG_BLOB` | LONGBLOB |  |  | Long raw defined column stores large binary data from the ct_document_version table. |
