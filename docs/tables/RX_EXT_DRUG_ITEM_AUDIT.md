...
> This will show the mapping between the itemid and drug identifiers.
...
| 2 | `DRUG_IDENT` | VARCHAR(14) | NOT NULL |  | NDC (National Drug Code) comes from MED_IDENTIFIER.VALUE |
| 3 | `ITEM_ID` | DOUBLE | NOT NULL | FK→ | The item that is mapped to the drug identifier. |
| 4 | `RX_EXT_DRUG_ITEM_AUDIT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RX_EXT_DRUG_ITEM_AUDIT table. |
