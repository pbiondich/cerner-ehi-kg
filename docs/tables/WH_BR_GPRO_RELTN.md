...
> This table stores the relationship between GPROs and eligible providers.
...
| 3 | `BR_GPRO_ID` | DOUBLE | NOT NULL |  | The GPRO that this provider is associated with. |
| 4 | `BR_GPRO_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the br_gpro_reltn table. |
...
| 11 | `ORIG_BR_GPRO_RELTN_ID` | DOUBLE | NOT NULL |  | Used for versioning. Contains the original PK from BR_GPRO_RELTN |
| 12 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | The provider that is a part of this GPRO. |
| 13 | `PARENT_ENTITY_NAME` | VARCHAR(32) |  |  | The table that is the source of information for the parent_entity_id column. |
