...
> Each row on the table represents actions taken on the rows of the passive alert table.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION_TYPE_TXT` | VARCHAR(30) | NOT NULL |  | Action type for the alert such as dismissing, etc |
...
| 5 | `PASSIVE_ALERT_ACTION_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 6 | `PASSIVE_ALERT_ID` | DOUBLE | NOT NULL | FK→ | Value of the unique primary identifier of a row in the PASSIVE_ALERT table. |
| 7 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | Value of the unique primary identifier of the PERSON / PRSNL table. |
