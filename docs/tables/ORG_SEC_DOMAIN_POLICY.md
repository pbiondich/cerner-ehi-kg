...
> Stores the policies defining organization access control for the domain.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ORG_SEC_DOMAIN_POLICY_ID` | DOUBLE | NOT NULL |  | System Genertated number to uniquely identify a row on the ORG_SEC_DOMAIN_POLICY table. |
| 2 | `POLICY_ACTIVE_IND` | DOUBLE | NOT NULL |  | Defines if the policy is active/turned on (1) or inactive/turned off (0) |
| 3 | `POLICY_DISPLAY` | VARCHAR(40) | NOT NULL |  | The display value for the poliscy enum/ |
| 4 | `POLICY_ENUM` | DOUBLE | NOT NULL |  | The enum value defining which policy the row represents. |
