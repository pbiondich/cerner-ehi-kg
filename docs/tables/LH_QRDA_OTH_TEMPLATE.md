...
> This table is used to store elements that are used to create the Other Template Section in the body of a QRDA file for submission
...
| 5 | `END_EFFECTIVE_OFFSET_UTC` | VARCHAR(6) |  |  | EFFECTIVE_OFFSET_UTC stores UTC offset for END_EFFECTIVE_DT_TM |
...
| 11 | `LH_QRDA_OTH_TEMPLATE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_QRDA_OTH_TEMPLATE table. |
...
| 14 | `OBS_CODE` | VARCHAR(50) |  |  | Code derived from Appendix_F-Results tab of Downloadable Resources Table |
| 15 | `OBS_CODE_DISPLAY` | VARCHAR(500) |  |  | Text representation of the result |
| 16 | `OBS_CODE_DISPLAY_NEG` | VARCHAR(500) |  |  | Text representation of the result (negation) |
| 17 | `OBS_CODE_NEG` | VARCHAR(50) |  |  | Represents a given code value (not Cerner's code value) from obs_cd_system (negation) |
| 18 | `OBS_CODE_SYSTEM` | VARCHAR(50) |  |  | The code system from which observation_code was derived from |
...
| 24 | `OTHER_TEMPLATE` | VARCHAR(200) | NOT NULL |  | The name of the template the particular event corresponds to. |
| 25 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | The value of the primary identifier of the table to which the Other Template section is related (i.e. lh_qrda_pqrs_id) |
| 26 | `PARENT_ENTITY_ID2` | DOUBLE | NOT NULL |  | The value of the primary identifier of the table to which the Other Template section is related (i.e. lh_qrda_pqrs_id) |
| 27 | `PARENT_ENTITY_NAME` | VARCHAR(50) | NOT NULL |  | The name of the table this Other Template section is related (i.e. LH_QRDA_PQRS) |
| 28 | `PARENT_ENTITY_NAME2` | VARCHAR(50) | NOT NULL |  | The name of the table this Other Template section is related (i.e. LH_QRDA_PQRS) |
| 29 | `REPORTING_YEAR` | DOUBLE |  |  | Stores the reporting year. |
| 30 | `RESULT_UNIT` | VARCHAR(50) |  |  | The units of the numeric result value for the record. This field contains the result unit associated with the respective template data. |
| 31 | `RESULT_VALUE_TXT` | VARCHAR(255) |  |  | This field contains the result value associated with the respective template data. |
...
| 34 | `UPDT_SOURCE` | VARCHAR(50) | NOT NULL |  | The script name responsible for updating the record. |
| 35 | `UPDT_TASK` | VARCHAR(50) | NOT NULL |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
