...
> This table stores the history of locations for every AOAV encounter
...
| 5 | `AOAV_ENCNTR_LOC_HIST_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 6 | `AOAV_PERSON_ENCNTR_ID` | DOUBLE | NOT NULL |  | #OBSOLETE in DBARCHDTL-22877 - ; Unique number that identifies a single row on the AOAV_PERS Unique number that identifies a single row on the AOAV_PERSON_ENCNTR table. |
...
| 8 | `CALCULATED_FLAG` | DOUBLE | NOT NULL |  | The calculated flag giving the state of the person-encounter data. |
| 9 | `ENCNTR_ID` | DOUBLE |  | FK→ | Unique Identifier of the encounter; |
| 10 | `ENCNTR_LOC_HIST_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the encounter location history table. |
...
| 12 | `LOC_NURSE_UNIT_CD` | DOUBLE | NOT NULL |  | Nurse Unit location code |
