...
> Documents patient-level notification choices
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BEG_EFFECTIVE_DT_ISO` | VARCHAR(10) |  |  | Stores a string value of the Begin effective date in the following ISO supported format:; YYYYMMDD; |
| 2 | `BEG_EFFECTIVE_DT_TM` | DATETIME |  |  | The date and time for which this table row becomes effective. Normally, this will be the date and time the row is added, but could be a past or future date and time. |
| 3 | `HM_PERSON_NOTIFICATION_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY FOR THE TABLE |
| 4 | `METHOD_RESTRICTION_FLAG` | DOUBLE | NOT NULL |  | How the patient prefers to receive the specified notification. Currently this only applies when NOTIFY is 'Self'. 0 - No Restrictions; 1 - No Calls |
| 5 | `NOTIFICATION_RECIPIENT_FLAG` | DOUBLE | NOT NULL |  | The recipient the patient has elected for notifications of the specified type. 1 Do Not Notify; 2 Self; 3 Provider |
| 6 | `NOTIFICATION_TYPE_FLAG` | DOUBLE | NOT NULL |  | The type of notification |
| 7 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The person this decision pertains to |
