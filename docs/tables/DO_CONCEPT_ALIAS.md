...
> Each concept has zero to many aliases unique to the context version The aliases are like CDF meanings in that they are what a developer codes against.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `DO_CONCEPT_ALIAS` | VARCHAR(200) | NOT NULL |  | Alias of the concept. |
| 2 | `DO_CONCEPT_ALIAS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the DO_CONCEPT_ALIAS table. |
| 3 | `DO_CONCEPT_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a single row on the DO_CONCEPT table. |
| 4 | `DO_CONTEXT_VERSION_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a single row on the DO_CONTEXT_VERSION table. |
