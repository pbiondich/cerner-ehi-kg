...
> Information related to the PACS associated to a facility.
...
| 2 | `BEGIN_EFFECTIVE_DT_TM` | DATETIME |  |  | The date/time after which the row is considered valid as active current data. This may be valued with the date that the row became active. |
...
| 4 | `FACILITY_CD` | DOUBLE | NOT NULL | FK→ | The facility where this PACS exists. |
| 5 | `RAD_VDI_PACS_CONFIG_ID` | DOUBLE | NOT NULL | FK→ | The information about the PACS unit that exists in this facility. |
| 6 | `RAD_VDI_PACS_FACILITY_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RAD_VDI_PACS_FACILITY table. |
...
| `RAD_VDI_PACS_CONFIG_ID` | [RAD_VDI_PACS_CONFIG](RAD_VDI_PACS_CONFIG.md) | `RAD_VDI_PACS_CONFIG_ID` |
