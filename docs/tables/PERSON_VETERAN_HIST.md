...
> Used for tracking changes to the person veteran table.
...
| 7 | `PERSON_VETERAN_HIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PERSON_VETERAN_HIST table. |
...
| 9 | `SERVICE_RLTD_DISABILITY_CD` | DOUBLE | NOT NULL |  | Defines the status that indicates whether or not the patient has a service related disability. |
| 10 | `SERVICE_RLTD_DISABILITY_PCT` | DOUBLE |  |  | Determines the percentage of benefits a veteran receives for their service related disability. |
