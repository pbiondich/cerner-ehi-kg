...
> This table will contain all healthexpert session consequents to be published
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `HE_CONSEQUENT_QUEUE_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 2 | `JOB_INSTANCE_NBR` | DOUBLE |  |  | The instance number of the publishing job currently publishing the consequent.; |
| 3 | `KNOWLEDGEBASE_NAME` | VARCHAR(100) |  |  | The unique name of the knowledge base associated with the session. |
| 4 | `PRIORITY_NBR` | DOUBLE |  |  | The priority of each consequent to be published |
| 5 | `SESSION_CONSEQUENT_ID` | DOUBLE |  | FK→ | The ID of the consequent to be published (FK) from the he_session_consequent |
...
|--------|--------------|---------------|
| `SESSION_CONSEQUENT_ID` | [HE_SESSION_CONSEQUENT](HE_SESSION_CONSEQUENT.md) | `SESSION_CONSEQUENT_ID` |
