...
> Fact table for Falls History Lighthouse Report
...
| 14 | `FALL_IND` | DOUBLE |  |  | Identifies each fall |
| 15 | `FIRST_PROCESS_DT_TM` | DATETIME |  |  | The date/time the record was first loaded into the table. |
| 16 | `F_FALLS_HST_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_F_FALLS_HST_METRICS table. |
...
| 19 | `INCDNT_DT_TM` | DATETIME |  |  | The date/time on which the incident occurred for the patient. |
| 20 | `INCDNT_UTC_DT_TM` | DATETIME |  |  | The date/time on which the incident first occurred for the patient; normalized to GMT. |
| 21 | `INJURY_IND` | DOUBLE |  |  | Identifies if the fall resulted in an injury |
| 22 | `INJURY_LEVEL_TXT` | VARCHAR(50) |  |  | Identifies the injury level of the fall |
| 23 | `IN_LOC_WIN_24H_IND` | DOUBLE |  |  | Identifies if patient was in the unit in the first 24hrs |
...
| 26 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility. |
| 27 | `PLAN_INITIATED_IND` | DOUBLE |  |  | Identifies if the at risk patient has had an orderset or powerplan activated. |
...
| 30 | `RISK_PRIOR_FALL_IND` | DOUBLE |  |  | Identifies patients that are at risk, had an incident documented, and were at risk prior to the incident. |
