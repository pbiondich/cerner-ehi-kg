# LH_E_PCB_2025_METRICS

> Stores data gathered by the LH_E_PCB_2025 script.

**Description:** Lighthouse eMeasures PC Baby 2025 Metrics  
**Table type:** ACTIVITY  
...
| 42 | `LH_E_PCB_2025_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_PCB_2025_METRICS table. |
...
| 67 | `NUM_5_IND` | DOUBLE |  |  | Identifies if the patient is in the numerator for PC05 |
| 68 | `NUM_6_IND` | DOUBLE |  |  | Identifies if the patient is in the numerator for PC-06 |
| 69 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Medical Record Number of the record. |
| 70 | `PARENTERAL_NUTRI_NOMEN` | VARCHAR(60) |  |  | The nomenclature parenteral nutrition |
...
| 73 | `PERSON_ETHNIC_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store ethnicity display. |
| 74 | `PERSON_GENDER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store gender display. |
| 75 | `PERSON_ID` | DOUBLE | NOT NULL |  | The ID of the person associated to the record. |
| 76 | `PERSON_PAYER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display. |
| 77 | `PERSON_RACE_DESC` | VARCHAR(300) |  |  | String to store multiple races. |
| 78 | `PHOTOTHERAPY_PROC_DT_TM` | DATETIME |  |  | phototherapy procedure was performed |
| 79 | `PHOTOTHERAPY_PROC_NOMEN` | VARCHAR(60) |  |  | Nomeclature of phototherapy procedure |
| 80 | `SCRIPT_VERSION` | DOUBLE |  |  | The version number of the script that populated the row |
| 81 | `SEVERE_BIRTH_TRAUMA_NOMEN` | VARCHAR(60) |  |  | Nomeclature of severe birth trauma diagnosis |
| 82 | `SEVERE_COMP_IND` | DOUBLE |  |  | Identifies if patient qualified for Numerator via severe complications |
| 83 | `SEVERE_HYPO_ASPHYXIA_NOMEN` | VARCHAR(60) |  |  | Nomeclature of severe_hypo_asphyxia diagnosis |
| 84 | `SEVERE_SHOCK_AND_RES_NOMEN` | VARCHAR(60) |  |  | Nomeclature of severe shock and resuscitation diagnosis |
| 85 | `SEVERE_SHOCK_PROC_DT_TM` | DATETIME |  |  | The date/time when severe_shock procedure was performed |
| 86 | `SEVERE_SHOCK_PROC_NOMEN` | VARCHAR(60) |  |  | Nomeclature of severe shock procedure |
| 87 | `SINGLE_LIVEBORN_CESAREAN_NOMEN` | VARCHAR(60) |  |  | Nomeclature of single liveborn cesarean diagnosis |
| 88 | `SINGLE_LIVEBORN_VAGINAL_NOMEN` | VARCHAR(60) |  |  | Nomeclature of single liveborn vaginal diagnosis |
| 89 | `SINGLE_LIVE_BORN_NOMEN` | VARCHAR(60) |  |  | The nomenclature of Single Liveborn Newborn in Hospital |
| 90 | `SOCIAL_INDICATIONS_NOMEN` | VARCHAR(60) |  |  | Nomeclature of social indications diagnosis |
...
| 94 | `TOTAL_PAREN_NUTRI_DT_TM` | DATETIME |  |  | The date of administration of Total Parenteral Nutrition medication |
| 95 | `TOTAL_PAREN_NUTRI_ORD_DETAIL` | VARCHAR(100) |  |  | Order details for qualifying medication administration for Total Parenteral Nutrition. |
| 96 | `UPDT_CNT` | DOUBLE |  |  | Set to 0 on insert. Incremented by 1 on update. Used to recognize update conflict where data in a row updated by one application is at risk of being lost by a second application attempting to update the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 97 | `UPDT_DT_TM` | DATETIME |  |  | The date and time the row was last inserted or updated. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
