...
> This table holds the relationship between the bt_cond_crit_r table and the code_value table.
...
| 6 | `BT_COND_CRIT_R_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related row on the bt_cond_crit_r table. |
| 7 | `CODE_VALUE` | DOUBLE |  |  | This is the code value for certain code sets that allow for multiple selects. |
| 8 | `CODE_VAL_GRP_R_ID` | DOUBLE | NOT NULL |  | Code Value Group Relation ID |
