...
> Stores relationship between bedrock department types and the order types it can be scheduled for.
...
| 3 | `CATALOG_TYPE_CD` | DOUBLE | NOT NULL |  | Catalog type of the default orders the department type can be scheduled for |
| 4 | `DEPT_TYPE_ID` | DOUBLE | NOT NULL |  | ID of the department type |
