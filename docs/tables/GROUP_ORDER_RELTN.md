...
> Associate one or more group identifiers to a group of orders.

**Description:** Group Order Relationship  
**Table type:** ACTIVITY  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `GROUP_IDENTIFIER` | VARCHAR(150) | NOT NULL |  | The group/bundle identifier (UUID). Originates from a 3rd party. |
| 2 | `GROUP_ORDER_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the group_order_reltn table. |
| 3 | `GROUP_ORDER_RELTN_TYPE_TFLG` | VARCHAR(16) | NOT NULL |  | The type of relationship between the group identifier and the order identifier |
| 4 | `ORDER_ID` | DOUBLE |  | FK→ | The ID of the order related to the group |
