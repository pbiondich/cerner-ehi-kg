...
> All blood bank specific comments on a person.
...
| 5 | `BB_COMMENT_ID` | DOUBLE | NOT NULL |  | The primary key to this table. An internal system-assigned number that makes each row unique. |
| 6 | `COMMENT_ADDED_PRSNL_ID` | DOUBLE |  | FK→ | Uniquely identifies the personnel who added the comment. |
| 7 | `COMMENT_DT_TM` | DATETIME |  |  | Indicates date and time the comment was written.; |
...
| 9 | `LONG_TEXT_ID` | DOUBLE | NOT NULL | FK→ | The primary key to the LONG_TEXT table. An internal system-assigned number. On this table, it corresponds to the text contained in the blood bank comments entered. |
...
|--------|--------------|---------------|
| `COMMENT_ADDED_PRSNL_ID` | [PRSNL](PRSNL.md) | `PERSON_ID` |
