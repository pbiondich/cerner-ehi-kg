...
> Stores information about an item (i.e. order) associated to an authorization.
...
| 5 | `AUTHORIZATION_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a single row on the AUTHORIZATION table to which this authorization item is associated. |
| 6 | `AUTHORIZATION_ITEM_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the AUTHORIZATION_ITEM table. |
| 7 | `AUTH_CNT` | DOUBLE | NOT NULL |  | The number of visits, days, or treatments that are authorized by the carrier for payment. |
| 8 | `AUTH_REMAIN_CNT` | DOUBLE | NOT NULL |  | The remaining number of units for the item that are authorized by the carrier for payment. |
| 9 | `AUTH_USED_CNT` | DOUBLE | NOT NULL |  | The number of units for the item authorized by the carrier for payment that has been used. |
| 10 | `AUTH_USED_MANUAL_ADJ_CNT` | DOUBLE | NOT NULL |  | The manual adjustment for the used number of units that were captured as part of a different encounter or by an external system . For example, if not all of the authorizations were used at the same facility, the manual adjustment will contain the number used at a different facility. |
| 11 | `ORDER_CATALOG_CD` | DOUBLE | NOT NULL | FK→ | The coded value for the order catalog for the order associated with this authorization item. |
| 12 | `ORDER_SYNONYM_ID` | DOUBLE | NOT NULL | FK→ | Id of the synonym for the order associated with this authorization item. |
| 13 | `THRESHOLD_NBR` | DOUBLE | NOT NULL |  | The remaining number of units that when reached shall trigger a timely notice to an end user that the patient is close to exhausting the original authorized value. |
...
| From table | Column |
|------------|--------|
| [AUTHORIZATION_ITEM_HIST](AUTHORIZATION_ITEM_HIST.md) | `AUTHORIZATION_ITEM_ID` |
| [AUTH_ITEM_ORDER_RELTN](AUTH_ITEM_ORDER_RELTN.md) | `AUTHORIZATION_ITEM_ID` |
