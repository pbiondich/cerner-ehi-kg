...
> The relationships between the person and the clinical service.
...
| 6 | `CLINICAL_SERVICE_CD` | DOUBLE | NOT NULL |  | Associates the clinical diagnosis to a particular setting of care within an encounter. |
| 7 | `CLINICAL_SERVICE_RELTN_ID` | DOUBLE | NOT NULL |  | Unique identifier for the Clinical Service Relation table. |
...
| 9 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | The value of the primary identifier of the table to which the 'clinical_service_reltn' row is related |
| 10 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | Name of the parent entity table |
| 11 | `PRIORITY` | DOUBLE |  |  | Optional field. It can be used if the person/org relation has more than 1 clinical service associated with it. |
