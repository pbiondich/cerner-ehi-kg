...
> Holds the IDs that make up the study group.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ENTITY_ID` | DOUBLE |  |  | ID that we are storing - e.g. person_id, encntr_id, etc. |
| 2 | `PARENT_ENTITY_NAME` | VARCHAR(255) |  |  | Name of table that entity_id is from. |
| 3 | `SG_ID` | DOUBLE | NOT NULL |  | Parent study group. |
| 4 | `SG_ITEM_ID` | DOUBLE | NOT NULL |  | Unique identifier. |
