...
> History of tracked orders for physicians
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ORDER_ID` | DOUBLE | NOT NULL | FK→ | The unique order identifier. |
| 2 | `ORDER_TRACKING_HIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ORDER_TRACKING_HISTORICAL table. |
| 3 | `REMOVAL_SYSTEM_FLAG` | DOUBLE | NOT NULL |  | The value to identifiy the sytem that populated the row. 0 - Undefined. 1 - Orders Tracking Worklist Async Script. 2 - Cleanup Ops Job, 3 - Manual Completion Service. |
