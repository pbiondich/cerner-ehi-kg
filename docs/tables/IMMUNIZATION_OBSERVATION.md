...
> Documented patient observations that correspond to a codified Indication or Contraindication for Immunization Forecasting.

**Description:** Immunization Observation  
**Table type:** ACTIVITY  
**Primary key:** `IMMUNIZATION_OBSERVATION_ID`  
**Columns:** 18  
...
| 5 | `BEGIN_DT_TM` | DATETIME |  |  | Begin Date Time of the Observation |
...
| 7 | `END_DT_TM` | DATETIME |  |  | End Date Time of the Observation |
...
| 9 | `IMMUNIZATION_OBSERVATION_ID` | DOUBLE | NOT NULL | PK | PRIMARY KEY |
| 10 | `OBSERVATION_CD` | DOUBLE | NOT NULL |  | observation code |
| 11 | `OBSERVATION_ID` | DOUBLE |  | FK→ | a grouper for history on the table and will reference the IMMUNIZATION_OBSERVATION_ID associated with the initial write to the table. |
| 12 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key to the Patient Person |
...
| 18 | `VACCINE_GROUP_CD` | DOUBLE | NOT NULL |  | Code Value for the Vaccine Group |
...
|--------|--------------|---------------|
| `OBSERVATION_ID` | [IMMUNIZATION_OBSERVATION](IMMUNIZATION_OBSERVATION.md) | `IMMUNIZATION_OBSERVATION_ID` |
...
| From table | Column |
|------------|--------|
| [IMMUNIZATION_OBSERVATION](IMMUNIZATION_OBSERVATION.md) | `OBSERVATION_ID` |
