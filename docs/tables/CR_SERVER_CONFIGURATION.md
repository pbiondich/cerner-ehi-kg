...
> This table stores the server configuration values used in report generation.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CR_SERVER_CONFIGURATION_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 2 | `CR_SERVER_VALUE_TXT` | VARCHAR(255) | NOT NULL |  | This value corresponds to the particular XR server environment configuration and is defined by the TYPE_FLAG. The value will be used in the XR Report Generation Service during report processing |
| 3 | `TYPE_FLAG` | DOUBLE | NOT NULL |  | The value defined for the TYPE_FLAG.Uniquely identifies a server config type. 23 values, |
