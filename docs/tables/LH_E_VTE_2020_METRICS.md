# LH_E_VTE_2020_METRICS

> Stores data gathered by the LH_E_VTE_2020 script.

**Description:** Lighthouse eMeasures VTE 2020 Metrics  
**Table type:** ACTIVITY  
...
| 68 | `LH_E_VTE_2020_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_VTE_2020_METRICS table. |
...
| 111 | `THROM_VTE1_ADMIN_DT_TM` | DATETIME |  |  | The first administration of Direct Thrombin Inhibitor meeting VTE1 requirements |
| 112 | `THROM_VTE2_ADMIN_DT_TM` | DATETIME |  |  | The first administration of Direct Thrombin Inhibitor meeting VTE2 requirements |
| 113 | `TIME_ZONE_TXT` | VARCHAR(300) |  |  | The timezone of the record in ""America/Chicago"" format. |
| 114 | `UPDT_CNT` | DOUBLE |  |  | Set to 0 on insert. Incremented by 1 on update. Used to recognize update conflict where data in a row updated by one application is at risk of being lost by a second application attempting to update the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 115 | `UPDT_DT_TM` | DATETIME |  |  | The date and time the row was last inserted or updated. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 116 | `UPDT_SOURCE` | VARCHAR(50) |  |  | The script name responsible for updating the record. |
| 117 | `UPDT_TASK` | VARCHAR(50) |  |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 118 | `VFP_APPLIED_VTE1_DT_TM` | DATETIME |  |  | The first application of VFP after meeting VTE1 requirements |
| 119 | `VFP_APPLIED_VTE2_DT_TM` | DATETIME |  |  | The first application of VFP after meeting VTE2 requirements |
| 120 | `VFP_MEDRES_MASK` | DOUBLE |  |  | Identifies which Medical Reasons for not ordering/administering VFP were documented |
| 121 | `VFP_PATREF_MASK` | DOUBLE |  |  | Identifies which Patient Refusals for not ordering/administering VFP were documented |
| 122 | `VTE1_ICU_ADMIT_DT_TM` | DATETIME |  |  | The start date/time of ICU for VTE-1 |
| 123 | `VTE1_ICU_DEPART_DT_TM` | DATETIME |  |  | The end date/time of ICU for VTE-1 |
| 124 | `VTE1_LOW_RISK_IND` | DOUBLE |  |  | Indicates if Low Risk is documented on a patient for VTE-1 |
| 125 | `VTE2_ICU_ADMIT_DT_TM` | DATETIME |  |  | The start date/time of ICU for VTE-2 |
| 126 | `VTE2_ICU_DEPART_DT_TM` | DATETIME |  |  | The end date/time of ICU for VTE-2 |
| 127 | `VTE2_LOW_RISK_IND` | DOUBLE |  |  | Indicates if Low Risk is documented on a patient for VTE-2 |
| 128 | `VTE_DX_NOMEN` | VARCHAR(50) |  |  | The nomenclature of a qualifying VTE diagnosis. |
| 129 | `VTE_DX_PRIOR_DT_TM` | DATETIME |  |  | The date/time of the VTE diagnosis prior to the inpatient encounter |
| 130 | `VTE_DX_PRIOR_NOMEN` | VARCHAR(50) |  |  | The nomenclature of the VTE diagnosis prior to inpatient |
| 131 | `WARF_MEDRES_MASK` | DOUBLE |  |  | Identifies which Medical Reasons for not ordering/administering Warfarin were documented |
| 132 | `WARF_PATREF_MASK` | DOUBLE |  |  | Identifies which Patient Refusal for not ordering/administering Warfarin were documented |
| 133 | `WARF_VTE1_ADMIN_DT_TM` | DATETIME |  |  | The first date/time of Warfarin after meeting VTE1 requirements |
| 134 | `WARF_VTE2_ADMIN_DT_TM` | DATETIME |  |  | The first date/time of Warfarin after meeting VTE2 requirements |
