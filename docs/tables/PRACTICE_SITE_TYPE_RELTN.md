...
> Defines whether a given practice site is referral, internal, external or owned etc., through code vlaues.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `PRACTICE_SITE_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key relationship to the Practice_Site table. |
| 2 | `PRACTICE_SITE_TYPE_CD` | DOUBLE | NOT NULL |  | Code value specifies whether the Practive_Site is Referral, Internal, External, etc. |
| 3 | `PRACTICE_SITE_TYPE_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the Practice_Site_Type_Reltn table. |
