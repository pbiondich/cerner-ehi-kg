...
> Stores organization level consent synchronization history
...
| 2 | `CONSENT_SYNC_AUDIT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the consent_sync_audit table. |
| 3 | `CONSENT_TYPE_FLAG` | DOUBLE |  |  | Flag indicating the type of the consent. |
| 4 | `CREATED_BEFORE_DT_TM` | DATETIME |  |  | Datetime represents the last time a consent was synchronized. |
| 5 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | Organization Identifier corresponding to the consent synchronization. |
| 6 | `SUCCESS_IND` | DOUBLE | NOT NULL |  | Indicates whether the sync was completed successfully. |
| 7 | `SYNC_DT_TM` | DATETIME |  |  | Datetime representing when the sync event occured. |
