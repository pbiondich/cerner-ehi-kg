...
> This table is stores information about a person and encounter that is used to calculate room and bed charges.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACCOMMODATION_CD` | DOUBLE | NOT NULL |  | Defines Accommodation level of encounter |
| 2 | `ACCOMMODATION_REQUEST_CD` | DOUBLE | NOT NULL |  | Requested accommodation level which can differ from actual accommodation level assigned |
...
| 10 | `LOC_BED_CD` | DOUBLE | NOT NULL | FK→ | The code_value for the bed in which the patient is located. |
| 11 | `LOC_BUILDING_CD` | DOUBLE | NOT NULL | FK→ | The code value for the building at which the patient is located. |
| 12 | `LOC_FACILITY_CD` | DOUBLE | NOT NULL | FK→ | The code value for the facility at which the patient is located. |
| 13 | `LOC_NURSE_UNIT_CD` | DOUBLE | NOT NULL | FK→ | The code value for the Location Nurse Unitat which the patient is located. |
| 14 | `LOC_ROOM_CD` | DOUBLE | NOT NULL | FK→ | The code_value representing the room in which the patient is located. |
...
| 17 | `ROOM_BED_CHARGE_ID` | DOUBLE | NOT NULL |  | Unique Identifier for PM_ROOM_BED_CHARGE Table |
