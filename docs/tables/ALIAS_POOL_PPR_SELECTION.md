...
> Defines the person relationship types that generate aliases for a related person.
...
| 5 | `ALIAS_POOL_CD` | DOUBLE |  | FK→ | Identifies the alias pool code to use when generating the alias for the related person. |
| 6 | `ALIAS_POOL_PPR_SELECTION_ID` | DOUBLE | NOT NULL |  | A system generated number to uniquely identify a row on the ALIAS_POOL_PPR_SELECTION table. |
| 7 | `PERSON_RELTN_TYPE_CD` | DOUBLE |  |  | Identifies the person relationship type code that determines if the alias should be generated for the related person. |
