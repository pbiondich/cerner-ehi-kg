...
> Table used to store the default weight adjustment and CRCL method of an Order Synonym.
...
| 4 | `OCS_DEF_DOSE_CALC_METHOD_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the OCS_DEF_DOSE_CALC_METHOD table. |
| 5 | `SYNONYM_ID` | DOUBLE | NOT NULL | FK→ | Identifies the synonym. Foreign Key from ORDER_CATALOG_SYNONYM table. |
