...
> Log containing detail execution status for reports scheduled in Discern Analytics 2.0
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BATCH_REPORT_END_DT_TM` | DATETIME |  |  | Date and time that the batch report finished. |
| 2 | `BATCH_REPORT_START_DT_TM` | DATETIME | NOT NULL |  | Date and time that the batch report began execution. |
| 3 | `CREATED_DOCUMENT_ID` | DOUBLE | NOT NULL | FK→ | Document identifier that was created from the executed report |
| 4 | `DA_BATCH_REPORT_ID` | DOUBLE | NOT NULL | FK→ | The report that these statistics are for. |
| 5 | `DA_BATCH_REPORT_LOG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the DA_BATCH_REPORT_LOG table. |
| 6 | `DA_BATCH_SCHED_LOG_ID` | DOUBLE | NOT NULL | FK→ | The statistics for the schedule that this report was part of. |
| 7 | `ERROR_TXT_ID` | DOUBLE | NOT NULL | FK→ | Error associated with the execution of the report. |
| 8 | `SUCCESS_IND` | DOUBLE | NOT NULL |  | Indicate if the batch report was actually successful or not. |
...
| `DA_BATCH_REPORT_ID` | [DA_BATCH_REPORT](DA_BATCH_REPORT.md) | `DA_BATCH_REPORT_ID` |
