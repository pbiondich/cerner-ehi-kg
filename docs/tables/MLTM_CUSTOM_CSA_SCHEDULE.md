...
> This table contains customizations of the CSA schedules of drugs at the Generic Drug, and Generic Fomulation levels.
...
| 3 | `CSA_SCHEDULE` | VARCHAR(2) |  |  | CSA Schedule value to be used for this drug/formuation |
| 4 | `DRUG_IDENTIFIER` | VARCHAR(6) |  |  | Multum provided drug identifier which is having CSA customized. Format is d##### |
...
| 6 | `MAIN_MULTUM_DRUG_CODE` | DOUBLE |  |  | Multum provided indicator that represents a generic trade product.Multum provided indicator that represents a generic trade product. |
| 7 | `MLTM_CUSTOM_CSA_SCHEDULE_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 8 | `STATE_CD` | DOUBLE |  |  | Code value from code set 62 which identifies the state the customization is valid for. |
