...
> The table defines the confidentiality(4702082) associated with Procedure data.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION_PRSNL_ID` | DOUBLE |  | FK→ | The ID of the personnel who has taken an action (modified, deleted, etc) this label. |
...
| 4 | `CREATED_BY_PRSNL_ID` | DOUBLE |  | FK→ | The ID of the personnel who associated theis label with the data entity. |
...
| 6 | `PROCEDURE_ID` | DOUBLE |  | FK→ | Unique identifier for a clinical PROCEDURE record from PROCEDURE table. The procedure associated to the security label. |
| 7 | `PROCEDURE_SEC_LBL_ID` | DOUBLE | NOT NULL |  | Procedure Security Label id is the primary unique identification number of the procedure_sec_lbl table. It is an internal system assigned sequence number. |
| 8 | `SENSITIVITY_REASON_CD` | DOUBLE |  |  | The code value indicating why the tagged data is considered sensitive. Values should be taken from the SENSITIVITY_REASON codeset (4702082). |
