...
> Table used to store information regarding insurance payer's timely filing
...
| 2 | `AUTO_RELEASE_DAYS` | DOUBLE | NOT NULL |  | The number of days in advance of timely filing days for the insurance payer organization in context on which the system should auto release the l claim holds for the health plan in context |
...
| 5 | `LIMIT_DAYS` | DOUBLE | NOT NULL |  | The number of days from the date of service of the patient with in which the medical claims needs to be reaching the insurance for the insurance payer organization in context for reimbursement. |
| 6 | `NOTIFY_DAYS` | DOUBLE | NOT NULL |  | The number of days in advance of timely filing days on which the system should Notify the user about medical claims for the health plan in context being approaching the timely filing limit. |
| 7 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | The organization that the filing record belongs to. |
| 8 | `ORG_TIMELY_FILING_ID` | DOUBLE | NOT NULL |  | Unique, generated numberr that identifies a single row on the ORG_TIMELY_FILING table. |
