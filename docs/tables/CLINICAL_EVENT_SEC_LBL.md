...
| 4 | `CLINICAL_EVENT_SEC_LBL_ID` | DOUBLE | NOT NULL |  | System generated number used to uniquely identify a row on the CLINICAL_EVENT_SEC_LBL table. |
| 5 | `CREATED_BY_PRSNL_ID` | DOUBLE |  | FK→ | The ID of the personnel who associated this label with the data entity. |
...
| 7 | `EVENT_ID` | DOUBLE |  |  | Identifies a unique event on the CLINICAL_EVENT table. |
| 8 | `SENSITIVITY_REASON_CD` | DOUBLE |  |  | The code value indicating why the tagged data is considered sesitive. |
