...
> Stores configuration index information for Health Information Management solutions.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `HIM_CONFIG_ID` | DOUBLE | NOT NULL | FK→ | Foreign key, Unique generated number that identifies a single row on the HIM_CONFIG table. |
| 2 | `HIM_CONFIG_INDEX_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the HIM_CONFIG_INDEX table. |
...
| 4 | `NAME_TXT` | VARCHAR(200) | NOT NULL |  | Key used to identify the configuration index. |
...
| 10 | `VALUE_TXT` | VARCHAR(100) | NOT NULL |  | Key used to identify the configuration index. |
...
|--------|--------------|---------------|
| `HIM_CONFIG_ID` | [HIM_CONFIG](HIM_CONFIG.md) | `HIM_CONFIG_ID` |
