...
> Stores mappings of events to identifiers unique across all client domains.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CE_UUID_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CE_UUID table. |
| 2 | `CLIENT_IDENT` | VARCHAR(100) |  |  | A unique client identifier for FHIR applications |
| 3 | `EVENT_ID` | DOUBLE | NOT NULL |  | The event_id from Clinical_event that this UUID is defined for. |
| 4 | `EVENT_UUID` | VARCHAR(100) | NOT NULL |  | Universally unique identifier for a clinical event across all client domains. |
