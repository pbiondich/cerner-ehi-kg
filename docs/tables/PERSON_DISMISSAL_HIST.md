...
> Used for tracking changes to the person dismissal rows
...
| 5 | `CHANGE_BIT` | DOUBLE | NOT NULL |  | Identifies which columns have changed. |
| 6 | `DISMISS_DT_TM` | DATETIME |  |  | The date that the patient was dismissed from the location or provider. |
| 7 | `LOCATION_CD` | DOUBLE | NOT NULL |  | The location from which the patient is being or has been dismissed. This should be no lower in the location hierarchy than the ambulatory/nurse unit. |
| 8 | `PERSON_DISMISSAL_HIST_ID` | DOUBLE | NOT NULL |  | Uniquely identifies a single history row related to PERSON_DISMISSAL. |
| 9 | `PERSON_DISMISSAL_ID` | DOUBLE | NOT NULL |  | Uniquely identifies the Person_Dismissal row to which this history is related. |
...
| 11 | `PRSNL_PERSON_ID` | DOUBLE | NOT NULL |  | Uniquely identifies the provider from which the patient is being or has been dismissed. |
| 12 | `REINSTATE_DT_TM` | DATETIME |  |  | The date that the patient was reinstated from the dismissal. |
