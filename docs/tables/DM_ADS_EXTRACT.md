...
> Stores how data should be replicated, with particular focus on how activity tables are related to each other and the queries used to replicate a referentially intact set of activity data based on a client supplied set of keys for driver level tables.

**Description:** DM_ACTIVITY_DATA_SAMPLER - Extract  
**Table type:** ACTIVITY  
**Primary key:** `DM_ADS_EXTRACT_ID`  
**Columns:** 23  
...
| 4 | `DM_ADS_EXTRACT_ID` | DOUBLE | NOT NULL | PK | Unique Identifier. Sequence based unique identifier for table. |
...
| From table | Column |
|------------|--------|
| [DM_ADS_CONFIG_DRIVER](DM_ADS_CONFIG_DRIVER.md) | `DM_ADS_EXTRACT_ID` |
| [DM_ADS_CONFIG_EXTRACT](DM_ADS_CONFIG_EXTRACT.md) | `DM_ADS_EXTRACT_ID` |
