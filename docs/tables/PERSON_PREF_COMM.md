...
**Table type:** ACTIVITY  
**Primary key:** `PERSON_PREF_COMM_ID`  
**Columns:** 16  
...
| 6 | `COMMUNICATION_TYPE_CD` | DOUBLE | NOT NULL |  | identifies the type of communication (i.e., appointment reminders, clinical communication, statements, etc). |
| 7 | `CONTACT_METHOD_CD` | DOUBLE | NOT NULL |  | denotes the person's preferred method of communication should he or she need to be contacted for the associated communication type. |
...
| 9 | `PERSON_PREF_COMM_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a row on the PERSON_PREF_COMM table. |
| 10 | `PHONE_TYPE_CD` | DOUBLE |  |  | identifies the type of email, phone, or fax (i.e., home, business, etc). |
...
| 16 | `VERIFY_DT_TM` | DATETIME |  |  | the date and time the communication type was last verified. |
...
| From table | Column |
|------------|--------|
| [PERSON_PREF_COMM_HIST](PERSON_PREF_COMM_HIST.md) | `PERSON_PREF_COMM_ID` |
