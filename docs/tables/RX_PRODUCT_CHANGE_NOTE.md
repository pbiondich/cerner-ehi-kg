...
> To add and track product notes for a product.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CREATE_DT_TM` | DATETIME |  |  | The date and time that the row was created. |
| 2 | `CREATE_PRSNL_ID` | DOUBLE |  | FK→ | The personnel that caused the insert of the row. |
| 3 | `ITEM_ID` | DOUBLE | NOT NULL | FK→ | The item that the note pertains to. Cannot be null or empty. |
| 4 | `PRODUCT_CHANGE_NOTE_CLOB` | LONGTEXT |  |  | Captured by Formulary Manager and contributes to the product update notes entered by the user for each product/item modification.; |
| 5 | `RX_PRODUCT_CHANGE_NOTE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RX_PRODUCT_CHANGE_NOTE table. |
