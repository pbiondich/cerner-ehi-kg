...
| 9 | `LAST_SEEN_DT_TM` | DATETIME |  |  | The date and time of the last time the readme was checked. |
...
| 11 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the prsnl table. It is an internal system assigned number. |
| 12 | `TEXT_LINK_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for the scheduling text record. |
| 13 | `TOTAL_COUNTER` | DOUBLE |  |  | A counter used to track the number of times a user has viewed a specific template. |
| 14 | `TOTAL_SECONDS` | DOUBLE |  |  | A summation of the number of seconds a user has spent viewing the template. |
...
| `TEXT_LINK_ID` | [SCH_TEXT_LINK](SCH_TEXT_LINK.md) | `TEXT_LINK_ID` |
