...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BATCH_REPORT_CD` | DOUBLE | NOT NULL |  | Batch report code |
| 2 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | identifier for the filter in the report like item_id, code_value,person_id, personnel id, work-list id etc. |
| 3 | `PARENT_ENTITY_NAME` | CHAR(32) |  |  | Table name that the value in column parent_entity_id originates. |
| 4 | `PHA_REPORT_FILTER_ID` | DOUBLE | NOT NULL |  | pha report filter id |
| 5 | `REPORT_FILTER_CD` | DOUBLE | NOT NULL |  | REport filter code |
