...
> This table stores field errors found by the bill record validation process
...
| 6 | `BILL_VRSN_NBR` | DOUBLE | NOT NULL | FK→ | Along with corsp_activity_id, uniquely identifies a bill record |
| 7 | `BR_FIELD_ERRORS_ID` | DOUBLE | NOT NULL |  | Primary Key |
| 8 | `CORSP_ACTIVITY_ID` | DOUBLE | NOT NULL | FK→ | Along with bill_vrsn_nbr, uniquely identifies a bill record |
| 9 | `ELEMENT_NAME` | VARCHAR(40) |  |  | The element name of the field catalog entry |
...
| 11 | `ERROR_CD` | DOUBLE | NOT NULL |  | The code value that describes the error for the field |
| 12 | `FIELD_CATALOG_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier of a field catalog entry |
| 13 | `LIST_INDEX` | DOUBLE | NOT NULL |  | The list index of the field catalog entry |
| 14 | `LIST_NAME` | VARCHAR(40) |  |  | The list name of the field catalog entry |
