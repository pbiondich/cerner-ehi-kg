...
> Used for tracking history of columns on the encntr_medicare_mgmt table.
...
| 5 | `AVAILABLE_DAYS` | DOUBLE |  |  | The date and time at which the number of available days was verified. |
| 6 | `AVAILABLE_DAYS_VERIFY_DT_TM` | DATETIME |  |  | The date and time at which the number of available days was verified. |
| 7 | `CHANGE_BIT` | DOUBLE | NOT NULL |  | Identifies which columns have had a change. |
| 8 | `ENCNTR_MEDICARE_MGMT_HIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the encntr_medicare_mgmt_hist table. |
| 9 | `ENCNTR_MEDICARE_MGMT_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a single row on the encntr_medicare_mgmt table. |
| 10 | `IN_RESIDENCE_DAYS` | DOUBLE |  |  | The total number of individual days that qualified as being in residence, and does not include future or planned days. |
...
| 12 | `REMAINING_COVERED_DAYS` | DOUBLE |  |  | The number of covered days remaining for Medicare in the current benefit period as of the current date. |
...
|--------|--------------|---------------|
| `ENCNTR_MEDICARE_MGMT_ID` | [ENCNTR_MEDICARE_MGMT](ENCNTR_MEDICARE_MGMT.md) | `ENCNTR_MEDICARE_MGMT_ID` |
