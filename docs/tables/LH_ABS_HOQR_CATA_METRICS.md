...
> Contains information based on HOQR OP-31: Appropriate Follow-up Interval for Normal Colonoscopy in Average Risk Patients condition for an encounters
...
| 8 | `CPTCODE_DESC_TXT` | VARCHAR(255) |  |  | Textual description of the CPTCODE documented for the case |
| 9 | `CPTCODE_DT_TM` | DATETIME |  |  | The date/time on which CPT code performed. |
| 10 | `CPTCODE_UTC_DT_TM` | DATETIME |  |  | The The date/time on which CPT code performed. normalized to GMT. |
...
| 28 | `D_METRIC_3_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric. |
| 29 | `D_METRIC_4_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric. |
| 30 | `D_METRIC_5_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric. |
| 31 | `D_PERSON_ID` | DOUBLE | NOT NULL | FK→ | Identifies the person associated with the quality measure. Foreign key to the LH_D_PERSON |
...
| 39 | `LH_ABS_HOQR_CATA_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCR_LH_ABS_HOQR_CATA_METRICS table. |
...
| 41 | `NUMERATOR_31_IND` | DOUBLE |  |  | Identifies if this case meets the criteria to be 'met' or 'not met' |
| 42 | `OP31_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason obtained during the completion of abstraction. |
...
| 45 | `PHYSICIAN_1_TXT` | VARCHAR(50) |  |  | First physician identifier |
| 46 | `PHYSICIAN_2_TXT` | VARCHAR(50) |  |  | Second physician identifier |
| 47 | `PRS_TXT` | VARCHAR(10) |  |  | Identifies the Patient Reliability Score (PRS). |
| 48 | `SAMPLE_IND` | DOUBLE |  |  | Indicates if the case was sampled |
...
| 50 | `UPDT_DT_TM` | DATETIME |  |  | The date and time the row was last inserted or updated. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
...
| 54 | `VSLFNCTNSRVY_FLAG` | DOUBLE |  |  | Identifies if a pre and post operative visual function instrument was completed |
| 55 | `VSLIMPRVMNT_FLAG` | DOUBLE |  |  | Identifies if there was improvement in patients visual function within 90 days post cataract surgery |
