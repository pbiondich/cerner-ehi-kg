...
> Stores post acute provider email addresses
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `EMAIL_ADDR` | VARCHAR(100) | NOT NULL |  | Contains the provider email address. |
| 2 | `RCM_POST_ACUTE_PROVIDER_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the associated post acute provider. |
| 3 | `RCM_POST_ACUTE_PROV_EMAIL_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RCM_POST_ACUTE_PROVIDER_EMAIL table. |
...
|--------|--------------|---------------|
| `RCM_POST_ACUTE_PROVIDER_ID` | [RCM_POST_ACUTE_PROVIDER](RCM_POST_ACUTE_PROVIDER.md) | `RCM_POST_ACUTE_PROVIDER_ID` |
