...
> An alias table to allow identification and reporting of CDS activity by an identifier other than the unique key on the CDS_BATCH_CONTENT table
...
| 5 | `ALIAS` | VARCHAR(35) | NOT NULL |  | The alias is an identifier for a row in the CDS_batch_content table. |
...
| 7 | `CDS_BATCH_CONTENT_ALIAS_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 8 | `CDS_BATCH_CONTENT_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the parent row in the CDS_BATCH_CONTENT table |
| 9 | `CDS_BAT_CONT_ALIAS_TYPE_CD` | DOUBLE | NOT NULL |  | Defines the kind or type of alias to the row on the CDS_BATCH_CONTENT table |
