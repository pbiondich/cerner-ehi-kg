...
> Provides a method for multiple code values to define an Authorization

**Description:** Authorization Code Value Relationship  
**Table type:** ACTIVITY  
**Primary key:** `AUTHORIZATION_CODE_VALUE_R_ID`  
**Columns:** 15  
...
| 5 | `AUTHORIZATION_CODE_VALUE_R_ID` | DOUBLE | NOT NULL | PK | PRIMARY KEY |
| 6 | `AUTHORIZATION_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifiers a single row on the AUTHORIZATION table to which this Code Value descriptor applies. |
...
| 8 | `CODE_SET` | DOUBLE | NOT NULL |  | This is a code set that can have multiple code values for one Authorization. |
| 9 | `CODE_VALUE` | DOUBLE | NOT NULL | FK→ | This is the code value from the given code set that allows multiple values to define the Authorization. |
...
| From table | Column |
|------------|--------|
| [AUTH_CODE_VALUE_R_HIST](AUTH_CODE_VALUE_R_HIST.md) | `AUTHORIZATION_CODE_VALUE_R_ID` |
