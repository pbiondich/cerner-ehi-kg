...
> Stores the attachment information pertaining to every secure email sent by RRD.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `MEDIA_OBJECT_IDENTIFIER` | VARCHAR(64) |  |  | The unique identifier for the document in CAMM. |
| 2 | `MEDIA_OBJECT_NAME` | VARCHAR(255) |  |  | The name of the document in CAMM.; |
| 3 | `MEDIA_OBJECT_VERSION_NBR` | DOUBLE |  |  | The version of the document in CAMM.; |
| 4 | `RRD_SECURE_EMAIL_ATTACH_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 5 | `RRD_SECURE_EMAIL_DETAIL_ID` | DOUBLE |  | FK→ | Foreign key to RRD_SECURE_EMAIL_DETAIL table, indicating the secure email this recipient was including with. |
