...
> Schema to store financial transaction service request and reply.
...
| 7 | `FIN_TRANS_GROUP_DETAIL_ID` | DOUBLE | NOT NULL | FK→ | Unique generataed number that identifies a single row on the PFT_TRANS_GROUP_DETAIL table. |
| 8 | `FIN_TRANS_LONG_TEXT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the FIN_TRANS_LONG_TEXT table. |
| 9 | `LONG_TEXT` | LONGTEXT |  |  | Stores the service payload. |
| 10 | `PAYLOAD_TYPE_NAME` | VARCHAR(30) | NOT NULL |  | The service payload type. ("REQUEST" or "REPLY") |
...
|--------|--------------|---------------|
| `FIN_TRANS_GROUP_DETAIL_ID` | [FIN_TRANS_GROUP_DETAIL](FIN_TRANS_GROUP_DETAIL.md) | `FIN_TRANS_GROUP_DETAIL_ID` |
