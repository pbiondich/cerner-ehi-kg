...
> Used for tracking history of changes to the person_plan_profile rows.
...
| 6 | `PERSON_PLAN_PROFILE_HIST_ID` | DOUBLE | NOT NULL |  | Uniquely identifies a history record for the person_plan_profile table. |
| 7 | `PERSON_PLAN_PROFILE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the row to which this history is related. |
...
| 9 | `PROFILE_TYPE_CD` | DOUBLE | NOT NULL |  | Indicates the type of health insurance profile for the person plan profile row (i.e., professional, institutional, workmen's comp). |
| 10 | `TRACKING_BIT` | DOUBLE | NOT NULL |  | Identifies which columns are being tracked for history. |
| 11 | `TRANSACTION_DT_TM` | DATETIME |  |  | ** OBSOLETE **. Use column updt_dt_tm for any filtering/ordering query. If transaction date time is needed, it should be retrieved from pm_hist_tracking table. Note that its date may be in the past, as in before the update date time. |
