...
> Location Status information for a given picklist
...
| 2 | `LOCATION_CD` | DOUBLE | NOT NULL | FK→ | The location that the status is for |
| 3 | `LOCATION_STATUS_CD` | DOUBLE | NOT NULL |  | The status of the location for the given picklist |
| 4 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The user who took an action that caused the location status to be updated |
| 5 | `SN_PICKLIST_ID` | DOUBLE | NOT NULL | FK→ | The picklist that the location status is for |
| 6 | `SN_PICKLIST_LOC_STATUS_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 7 | `STATUS_DT_TM` | DATETIME |  |  | The date and time the status of the location was updated |
| 8 | `SYSTEM_TRIGGERED_IND` | DOUBLE | NOT NULL |  | Indicates that the status was automatically triggered from another action within the picklist |
