...
> Contains the insurance profile linked to appointment type.
...
| 5 | `APPT_TYPE_CD` | DOUBLE | NOT NULL | FK→ | Indicate the Appointment Type Code. |
...
| 8 | `SCH_AT_INSUR_PROFILE_R_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCH_AT_INSUR_PROFILE_R table. |
| 9 | `SEQ_NBR` | DOUBLE | NOT NULL |  | Determines the order among the insurance profiles. |
