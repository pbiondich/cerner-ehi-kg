...
> The Problem Personnel Relation table records the relationships that providers have to a problem.
...
| 11 | `PROBLEM_ID` | DOUBLE | NOT NULL | FK→ | Unique identifier to track problem evolution. |
| 12 | `PROBLEM_PRSNL_ID` | DOUBLE | NOT NULL |  | Unique identifier for problem personnel relationship. |
| 13 | `PROBLEM_RELTN_CD` | DOUBLE | NOT NULL |  | The reason or purpose of the relationship. |
| 14 | `PROBLEM_RELTN_DT_TM` | DATETIME |  |  | The date and time that the personnel relationship was created. |
| 15 | `PROBLEM_RELTN_PRSNL_ID` | DOUBLE | NOT NULL |  | The unique identifier of the care provider that established the relationship. |
