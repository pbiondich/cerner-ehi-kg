...
> The purpose of this table is to store user-specific runtime prompt values for reports.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `DA_REPORT_DEFAULT_PROMPTS_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 2 | `DA_REPORT_ID` | DOUBLE |  | FK→ | discern analytics report identifier |
| 3 | `DEFAULT_PROMPTS_TEXT_ID` | DOUBLE |  | FK→ | pointer to the long_text_reference row that contains the default prompts |
...
| 9 | `USER_PRSNL_ID` | DOUBLE |  | FK→ | the user that selected these prompts during execution. |
