...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTIVE_DT_TM` | DATETIME | NOT NULL |  | Date and Time the comment exception was made active or inactive |
...
| 4 | `COMMENT_TXT` | LONGTEXT |  |  | The comment text that should be set for the exception |
| 5 | `COMMENT_TYPE_CD` | DOUBLE | NOT NULL |  | The comment type being added or modified on the exception; |
| 6 | `CREATE_DT_TM` | DATETIME | NOT NULL |  | Date and Time the comment exception was created; |
| 7 | `CREATE_PRSNL_ID` | DOUBLE | NOT NULL | FK→ | User who created the comment exception; |
| 8 | `SN_PREF_CARD_EXCEPTION_ID` | DOUBLE |  | FK→ | Preference Card Exception the item is for |
| 9 | `SN_PREF_CARD_EXCEPT_COMMENT_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
