...
> Contains information about referral alias data.
...
| 5 | `ALIAS` | VARCHAR(200) | NOT NULL |  | Contains the alias for the referral. |
| 6 | `ALIAS_POOL_CD` | DOUBLE | NOT NULL |  | Referral alias pool code identifies a unique set or list of referral identifiers (i.e., numbers). The referral alias pool code also determines the accept/display format for the unique set of identifiers. |
...
| 9 | `REFERRAL_ALIAS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the REFFERAL_ALIAS table. |
| 10 | `REFERRAL_ALIAS_TYPE_CD` | DOUBLE | NOT NULL |  | Identifies the type of the referral alias. |
