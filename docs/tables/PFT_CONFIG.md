...
> This table will store configuration information
...
| 7 | `INFO_DOMAIN_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, if you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
| 8 | `INFO_DOMAIN_TXT` | VARCHAR(100) |  |  | A textual name of the domain. |
| 9 | `INFO_DT_TM` | DATETIME |  |  | Stores the date and time when the value is a date |
| 10 | `INFO_NAME` | VARCHAR(255) |  |  | Stores the name when the value portion is a name. |
| 11 | `INFO_NBR` | DOUBLE |  |  | Stores the number when the value is a number. |
| 12 | `INFO_TXT` | VARCHAR(4000) |  |  | Stores the text when the value is a text. |
| 13 | `PFT_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PFT_CONFIG table. |
