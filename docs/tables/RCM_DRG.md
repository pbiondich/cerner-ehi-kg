...
> Loaded select drg (by contributor system) which are first on our worklists from a care management load from an encounter.
...
| 2 | `DRG_PRIORITY_NBR` | DOUBLE |  |  | The selected working drg priority number. |
| 3 | `ENCNTR_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the encounter related to this row. |
| 4 | `RCM_DRG_ID` | DOUBLE | NOT NULL |  | Unique generate number that identifies a single row on the RCM_DRG table. |
