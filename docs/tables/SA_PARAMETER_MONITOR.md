...
> Based on how often they start/stop monitors within a case. Estimate 1:1 with SA_PARAMETER (maybe a bit larger). 783,000
...
| 5 | `ACTIVITY_DT_TM` | DATETIME |  |  | The date/time the monitoring of the parameter was turned on or off |
| 6 | `MONITOR_IND` | DOUBLE |  |  | Indicates whether the parameter is being monitored (1=data pulled from BMDI) or not monitored (0=no data pulled from BMDI for this parameter) |
| 7 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The user who turned the monitoring of the parameter on or off |
| 8 | `SA_MACRO_ID` | DOUBLE | NOT NULL | FK→ | Ties to the macro that turned the monitoring of the parameter on or off, if 0, done by user |
| 9 | `SA_PARAMETER_ID` | DOUBLE | NOT NULL | FK→ | The parent SA_PARAMETER record that is being or not being monitored |
| 10 | `SA_PARAMETER_MONITOR_ID` | DOUBLE | NOT NULL |  | Unique value that identifies the monitor record |
