...
> This table stores warnings related to orders.
...
| 2 | `ORDER_ID` | DOUBLE | NOT NULL | FK→ | The identifier of the order which is associated to the warning. |
| 3 | `ORDER_WARNING_ID` | DOUBLE | NOT NULL |  | The primary key of this table. It is internally assigned by the system. |
| 4 | `RESOLVED_ACTION_SEQUENCE` | DOUBLE | NOT NULL |  | If populated, indicates the order action sequence which resolved this warning. |
...
| 10 | `WARNING_LEVEL_FLAG` | DOUBLE | NOT NULL |  | Illustrates the warning level. Valid flag values are: 1 -Severe, 2 - Audit, 3 - Informational |
| 11 | `WARNING_TYPE_FLAG` | DOUBLE | NOT NULL |  | Illustrates the type of warning. Valid flag values are: 1 - Protocol person mismatch, 2 - Protocol/DoT removed for GDPR. |
