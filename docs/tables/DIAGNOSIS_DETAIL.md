...
> Stores detail information about Hospital Acquired Conditions for diagnosis.
...
| 3 | `DIAGNOSIS_DETAIL_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the Diagnosis_Detail table. |
| 4 | `DIAGNOSIS_ID` | DOUBLE | NOT NULL | FK→ | The Diagnosis ID this detail row is related to. |
...
| 6 | `HAC_COMPLICATION1_CD` | DOUBLE | NOT NULL |  | First Hospital Acquired Condition complication associated to the diagnosis. |
| 7 | `HAC_COMPLICATION2_CD` | DOUBLE | NOT NULL |  | Second Hospital Acquired Condition complication associated to the diagnosis. |
