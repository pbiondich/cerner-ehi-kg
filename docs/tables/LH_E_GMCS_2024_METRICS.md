...
> Stores data gathered by the LH_E_GMCS_2024 script.
...
| 27 | `HOSP_DIET_REF_ORD_DT_TM` | DATETIME |  |  | Identifies the date/time of qualifying Intervention order |
...
| 31 | `LH_E_GMCS_2024_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_GMCS_2024_METRICS table. |
...
| 33 | `MALNUTRITION_DX_DT_TM` | DATETIME |  |  | Identifies the date/time of qualifying Malnutrition diagnosis |
| 34 | `MALNUTRITION_DX_NOMEN` | VARCHAR(60) |  |  | Identifies qualifying Malnutrition diagnosis code |
| 35 | `MAL_AT_RISK_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a Malnutrition At Risk Assessment Performed |
| 36 | `MAL_NOT_AT_RISK_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a Malnutrition Not At Risk Assessment Performed |
| 37 | `MAL_RISK_SCRNG_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a Malnutrition Risk Screening Assessment Performed |
| 38 | `MEAS_OBS_1` | DOUBLE |  |  | Encounters with Malnutrition Risk Screening and Identified Result |
| 39 | `MEAS_OBS_2` | DOUBLE |  |  | Encounter with Nutrition Assessment and Identified Status |
| 40 | `MEAS_OBS_3` | DOUBLE |  |  | Encounters with Malnutrition Diagnosis |
| 41 | `MEAS_OBS_4` | DOUBLE |  |  | Encounters with Nutrition Care Plan |
| 42 | `NAS_MOD_MAL_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a qualifying Assessment performed. |
| 43 | `NAS_NM_MAL_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a qualifying Assessment performed. |
| 44 | `NAS_SEV_MAL_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a qualifying Assessment performed |
| 45 | `NUTRITION_ASSES_PERF_IND` | DOUBLE |  |  | Identifies whether there was a Nutrition Assessment Performed |
| 46 | `NUTRI_CARE_PLAN_PERF_DT_TM` | DATETIME |  |  | Identifies the date/time of qualifying Intervention performed |
| 47 | `OBS_SERV_ARRIVAL_DT_TM` | DATETIME |  |  | The start of the qualifying observation encounter. |
| 48 | `OBS_SERV_DEPART_DT_TM` | DATETIME |  |  | The end of the qualifying observation encounter. |
| 49 | `OBS_SERV_ENCNTR_ID` | DOUBLE | NOT NULL |  | The ID of the qualifying Observation encounter associated to the record. |
| 50 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Medical Record Number of the record. |
| 51 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility. |
| 52 | `PATIENT_IDENT` | VARCHAR(50) |  |  | Identifies the earliest documented medical record number of the patient. |
| 53 | `PERSON_ETHNIC_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store ethnicity display. |
| 54 | `PERSON_GENDER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store gender display. |
| 55 | `PERSON_ID` | DOUBLE | NOT NULL |  | The ID of the person associated to the record. |
| 56 | `PERSON_PAYER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display. |
| 57 | `PERSON_RACE_DESC` | VARCHAR(300) |  |  | String to store multiple races. |
...
| 62 | `TOTAL_MAL_COMP_SCORE_DEN` | DOUBLE |  |  | The total number of clinically eligible denominators. |
| 63 | `TOTAL_MAL_COMP_SCORE_MEAS_OBS` | DOUBLE |  |  | The sum of performed Measure Observations 1, 2, 3, and 4 |
