...
> Holds the relationship between a batch and a transaction
...
| 5 | `ACTIVITY_ID` | DOUBLE | NOT NULL | FK→ | The Activity_ID from the Trans_Log table. |
| 6 | `BATCH_TRANS_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key to the Batch table. |
| 7 | `BATCH_TRANS_RELTN_ID` | DOUBLE | NOT NULL |  | Batch Transaction Relation |
...
| 9 | `BILLING_ENTITY_ID` | DOUBLE | NOT NULL | FK→ | Billing entity id |
| 10 | `DISABLED_IND` | DOUBLE |  |  | Disabled Indicator - No longer used |
...
| 12 | `SEQUENCE_NBR` | DOUBLE |  |  | The sequence number to determine the record order. |
