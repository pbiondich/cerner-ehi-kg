# LH_E_STROKE_2022_METRICS

> Stores data gathered by the LH_E_STROKE_2022 script.
...
| 26 | `D_ADMIT_BUILDING_ID` | DOUBLE |  | FK→ | Dimension ID for admit building. |
| 27 | `D_ADMIT_FACILITY_ID` | DOUBLE |  | FK→ | Dimension ID for admit facility. |
| 28 | `D_ADMIT_NURSE_UNIT_ID` | DOUBLE |  | FK→ | Dimension ID for admit nurse unit. |
| 29 | `D_ADMIT_SRC_ID` | DOUBLE |  | FK→ | Dimension ID for admit source. |
| 30 | `D_ADMIT_TYPE_ID` | DOUBLE |  | FK→ | Dimension ID for admit type. |
| 31 | `D_ATTEND_PRSNL_ID` | DOUBLE |  | FK→ | Dimension ID for attending physician. |
| 32 | `D_BR_CCN_ID` | DOUBLE |  | FK→ | Dimension ID for CCN. |
| 33 | `D_BR_HCO_ID` | DOUBLE |  | FK→ | Dimension ID for HCO. |
| 34 | `D_DISCHARGE_BUILDING_ID` | DOUBLE |  | FK→ | Dimension ID for discharge building. |
| 35 | `D_DISCHARGE_FACILITY_ID` | DOUBLE |  | FK→ | Dimension ID for discharge facility. |
| 36 | `D_DISCHARGE_NURSE_UNIT_ID` | DOUBLE |  | FK→ | Dimension ID for discharge nurse unit. |
| 37 | `D_DISCH_DISP_ID` | DOUBLE |  | FK→ | Dimension ID for discharge disposition. |
| 38 | `D_ENCNTR_TYPE_ID` | DOUBLE |  | FK→ | Dimension ID for encounter type. |
| 39 | `D_METRIC_2_ID` | DOUBLE |  | FK→ | Identifies the metric identifier for the Lighthouse metric: STK-2 |
| 40 | `D_METRIC_3_ID` | DOUBLE |  | FK→ | Identifies the metric identifier for the Lighthouse metric: STK-3 |
| 41 | `D_METRIC_5_ID` | DOUBLE |  | FK→ | Identifies the metric identifier for the Lighthouse metric: STK-5 |
| 42 | `D_METRIC_6_ID` | DOUBLE |  | FK→ | Identifies the metric identifier for the Lighthouse metric: STK-6 |
| 43 | `D_PERSON_ID` | DOUBLE |  | FK→ | Dimension ID for person data. |
...
| 46 | `ED_ENCNTR_ID` | DOUBLE |  |  | Identifies the emergency encounter against which the quality measure is associated. Foreign key to the ENCOUNTER table. |
| 47 | `ENCNTR_ID` | DOUBLE |  |  | The ID of the qualifying encounter associated to the record. |
...
| 59 | `HEALTH_SYSTEM_ID` | DOUBLE |  |  | Identifies the delivery network responsible for supplying the data. |
| 60 | `HEALTH_SYSTEM_SOURCE_ID` | DOUBLE |  | FK→ | Identifies the unique source within the delivery network responsible for supplying the data. |
...
| 68 | `LH_E_STROKE_2022_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_STROKE_2022_METRICS table. |
| 69 | `LOGICAL_DOMAIN_ID` | DOUBLE |  |  | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, If you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
...
| 76 | `OBS_SERV_ENCNTR_ID` | DOUBLE |  |  | The ID of the observation services encounter associated to the record. |
...
| 80 | `PC_AT_ADMIN_DT_TM` | DATETIME |  |  | Identifies the date/time Pharmacological contraindication Administered was documented |
| 81 | `PC_AT_DISCH_DT_TM` | DATETIME |  |  | Identifies the date/time Pharmacological contraindication at Discharge was documented |
| 82 | `PERSON_ETHNIC_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store ethnicity display. |
| 83 | `PERSON_GENDER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store gender display. |
| 84 | `PERSON_ID` | DOUBLE |  |  | The ID of the person associated to the record. |
| 85 | `PERSON_PAYER_CODE_DISPLAY` | VARCHAR(300) |  |  | String to store payer display. |
| 86 | `PERSON_RACE_DESC` | VARCHAR(300) |  |  | String to store multiple races. |
...
| 90 | `STATIN_ALLERGY_IND` | DOUBLE |  |  | Identifies if there was a Allergy Reason for not giving Statins |
| 91 | `STATIN_DISCH_DT_TM` | DATETIME |  |  | The date/time of the Statin prescribed at discharge |
| 92 | `STATIN_DISCH_MED_RES_IND` | DOUBLE |  |  | Identifies if there was a Medical Reason for not giving Statins |
| 93 | `STATIN_DISCH_PAT_REF_IND` | DOUBLE |  |  | Identifies if there was a Patient Refusal for not giving Statins |
