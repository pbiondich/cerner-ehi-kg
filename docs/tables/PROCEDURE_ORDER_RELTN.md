...
> This table associates Procedures to Orders. This relationship is needed to comply with USCDI regulations.
...
| 2 | `ORDER_ID` | DOUBLE |  | FK→ | The ID of the order associated to the procedure. It is a foreign key to the Orders table. |
| 3 | `PROCEDURE_ID` | DOUBLE |  | FK→ | The ID of the procedure associated to the order. It is a foreign key to the Procedure table. |
| 4 | `PROCEDURE_ORDER_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PROCEDURE_ORDER_RELTN table. |
