...
> This table stores Payment Plan to Pft_Encntr reltn information for payment plans managed by third party.
...
| 6 | `CBOS_PE_RELTN_ID` | DOUBLE | NOT NULL | FK→ | Contributor System identifies the source feed of data from which a row was populated. this is mainly used to determine how to update a set of data that may have originated from more than one source feed. |
...
| 8 | `EXT_PAY_PLAN_PE_RELTN_ID` | DOUBLE | NOT NULL |  | System generated number that uniquely identifies a row on the EXT_PAY_PLAN_PE_RELTN table. |
| 9 | `PFT_ENCNTR_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related row on the pft_encntr table. |
| 10 | `PFT_PAYMENT_PLAN_ID` | DOUBLE | NOT NULL | FK→ | Contributor System identifies the source feed of data from which a row was populated. this is mainly used to determine how to update a set of data that may have originated from more than one source feed. |
