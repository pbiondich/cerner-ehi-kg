...
> This table is used to store Mother PC metrics from the Lighthouse Abstractor (eQuality Check). This table is at the encounter grain.
...
| 4 | `ANT_STER_IND` | DOUBLE |  |  | Identifies if antenatal steroid therapy was initiated |
| 5 | `CLINICAL_TRIAL_FLAG` | DOUBLE |  |  | Identifies if the patient is excluded from the measure due to participation in a clinical trial. |
...
| 10 | `DISC_DISP_FLAG` | DOUBLE |  |  | Identifies the discharge disposition of the patient. |
...
| 29 | `D_REASON_1_ID` | DOUBLE | NOT NULL | FK→ | The reason for the PC-1 outcome. Relates to parent table LH_D_MEASURE_REASON. |
...
| 31 | `EXCLUDE_1_IND` | DOUBLE |  |  | Represents if the person is excluded for PC1 |
| 32 | `EXCLUDE_2_IND` | DOUBLE |  |  | Represents if the person is excluded for PC2 |
| 33 | `EXCLUDE_3_IND` | DOUBLE |  |  | Represents if the person is excluded for PC3 |
| 34 | `EXTRACT_DT_TM` | DATETIME |  |  | The date/time that the record was extracted from the source system. |
| 35 | `FINANCIAL_NBR_TXT` | VARCHAR(50) |  |  | The financial number alias associated to the encounter. |
| 36 | `FIRST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the first ETL process started that created this record. |
| 37 | `GESTATION_AGE` | VARCHAR(10) |  |  | Age in weeks of gestation |
...
| 40 | `HXSTILLBIRTH_IND` | DOUBLE |  |  | History of Still birth indicator |
| 41 | `LABOR_IND` | DOUBLE |  |  | Identifies if the patient was in labor |
| 42 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the last ETL process started that updated this record. |
| 43 | `LH_ABS_PC_MOTHER_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_ABS_PC_MOTHER_METRICS table. |
...
| 45 | `NUMERATOR_1_IND` | DOUBLE |  |  | Represents if the person is in the numerator for PC1 |
| 46 | `NUMERATOR_2_IND` | DOUBLE |  |  | Represents if the person is in the numerator for PC2 |
| 47 | `NUMERATOR_3_IND` | DOUBLE |  |  | Represents if the person is in the numerator for PC3 |
| 48 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Identifies the medical record number of the patient. |
| 49 | `OTH_DIAGNOSIS_LIST` | VARCHAR(400) |  |  | A comma separated list of other diagnoses associated with the encounter. |
| 50 | `OTH_PROCEDURE_DATES_LIST` | VARCHAR(400) |  |  | A comma separated list of dates the other procedures associated with the encounter were performed. |
| 51 | `OTH_PROCEDURE_LIST` | VARCHAR(400) |  |  | A comma separated list of other procedures associated with the encounter. |
| 52 | `PARITY` | VARCHAR(10) |  |  | How many deliveries did the patient experience prior to current hospitalization |
| 53 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility |
| 54 | `PAYMENT_SOURCE_FLAG` | DOUBLE |  |  | Identifies the source of payment for the encounter |
| 55 | `PC1_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason for PC-1 obtained during the completion of abstraction. |
| 56 | `PC2_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason for PC-2 obtained during the completion of abstraction. |
| 57 | `PHYSICIAN_1_TXT` | VARCHAR(50) |  |  | First physician identifier. |
| 58 | `PHYSICIAN_2_TXT` | VARCHAR(50) |  |  | Second physician identifier. |
...
| 64 | `PRIOR_UTER_SURG_IND` | DOUBLE |  |  | Identifies if prior uterine surgery had been performed |
| 65 | `PRS_TXT` | VARCHAR(10) |  |  | Identifies the Patient Reliability Score (PRS). |
| 66 | `REASON_NO_ANT_STER_IND` | DOUBLE |  |  | Identifies the patient has a reason for not having antenatal steroid therapy initiated |
| 67 | `REJECT_1_IND` | DOUBLE |  |  | Represents if the person is rejected for PC1 |
| 68 | `REJECT_2_IND` | DOUBLE |  |  | Represents if the person is rejected for PC2 |
| 69 | `REJECT_3_IND` | DOUBLE |  |  | Represents if the person is rejected for PC3 |
| 70 | `SAMPLE_IND` | DOUBLE |  |  | This case represent part of a sample |
| 71 | `SPON_RUPT_MEM_IND` | DOUBLE |  |  | Identifies if spontaneous rupture of membranes had been documented |
...
| `D_REASON_1_ID` | [LH_D_MEASURE_REASON](LH_D_MEASURE_REASON.md) | `D_MEASURE_REASON_ID` |
...
| `HEALTH_SYSTEM_SOURCE_ID` | [LH_D_FACILITY](LH_D_FACILITY.md) | `HEALTH_SYSTEM_SOURCE_ID` |
| `HEALTH_SYSTEM_SOURCE_ID` | [LH_D_MEASURE_REASON](LH_D_MEASURE_REASON.md) | `HEALTH_SYSTEM_SOURCE_ID` |
