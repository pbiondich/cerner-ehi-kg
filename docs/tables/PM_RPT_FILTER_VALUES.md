...
> Determines the valid values per filter in Person Management Reporting
...
| 10 | `END_IND` | DOUBLE |  |  | Used only when the filter has been defined as using 'between' logic. This is the filter value used at the end of the 'between' statement. |
| 11 | `FILTER_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key tying the filter values to a specific filter on the pm_rpt_filter table |
| 12 | `FILTER_VALUES_ID` | DOUBLE | NOT NULL |  | Primary Key attribute |
| 13 | `START_IND` | DOUBLE |  |  | Used only when the filter has been defined as using 'between' logic. This is the filter value used at the start of the 'between' statement. |
...
| 19 | `VALUE` | CHAR(100) |  |  | The value used for this filter |
...
|--------|--------------|---------------|
| `FILTER_ID` | [PM_RPT_FILTER](PM_RPT_FILTER.md) | `FILTER_ID` |
