...
> Sotes organization level consent synchronization history by patient.
...
| 5 | `CONSENT_PATIENTSYNC_AUDIT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CONSENT_PATIENTSYNC_AUDIT table. |
| 6 | `CONSENT_TYPE_FLAG` | DOUBLE |  |  | Flag indicating the type of consent. |
| 7 | `ORGANIZATION_ID` | DOUBLE |  | FK→ | The identifier of the organization associated with the sync event. |
| 8 | `PERSON_ID` | DOUBLE |  | FK→ | Unique identifier of the patient to which the sync event applies. |
| 9 | `SUCCESS_IND` | DOUBLE |  |  | Indicates wherher the sync was completed successfully. |
| 10 | `SYNC_DT_TM` | DATETIME |  |  | Date and time indicating when the synchronization happened. |
