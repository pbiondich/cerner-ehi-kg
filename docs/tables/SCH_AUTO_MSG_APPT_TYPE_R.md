...
> Table used to store the relation between appointment reminders and appointment types.
...
| 2 | `APPT_TYPE_CD` | DOUBLE | NOT NULL |  | The code value of an appointment type which has a relation to an automated messaging build. |
...
| 5 | `SCH_AUTO_MSG_APPT_TYPE_R_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCH_AUTO_MSG_APPT_TYPE_R table. |
| 6 | `SCH_AUTO_MSG_ID` | DOUBLE | NOT NULL | FK→ | The id of an automated messaging build which has a relation to a resource. |
...
| 12 | `VERSION_DT_TM` | DATETIME | NOT NULL |  | The version date and time marks the date the record became historical. |
...
|--------|--------------|---------------|
| `SCH_AUTO_MSG_ID` | [SCH_AUTO_MSG](SCH_AUTO_MSG.md) | `SCH_AUTO_MSG_ID` |
