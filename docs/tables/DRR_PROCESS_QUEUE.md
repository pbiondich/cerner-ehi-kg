...
> Queue table to store selected persons to be processed by DRR. Activity table.

**Description:** DRR_PROCESS_QUEUE  
**Table type:** ACTIVITY  
**Primary key:** `DRR_PROCESS_QUEUE_ID`  
**Columns:** 16  
...
| 2 | `DRR_PROCESS_QUEUE_ID` | DOUBLE | NOT NULL | PK | primary key use sequence DRR_SEQ. |
| 3 | `DRR_SERVICE_IDENT` | DOUBLE | NOT NULL |  | used by the drr queue service to -reserve- items to work on |
| 4 | `NEXT_RETRY_TS` | DATETIME(6) |  |  | Next time retry will be attempted |
...
| From table | Column |
|------------|--------|
| [DRR_PROCESS_PLAN](DRR_PROCESS_PLAN.md) | `DRR_PROCESS_QUEUE_ID` |
