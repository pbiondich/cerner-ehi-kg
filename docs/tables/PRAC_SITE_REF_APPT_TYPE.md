...
> Defines the appointment types that the practice site uses when accepting referrals.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `APPT_SYNONYM_CD` | DOUBLE | NOT NULL |  | Code value specifies whether the PRACTICE_SITE has referral appointment type Surgery, Cardiology etc. |
| 2 | `PRACTICE_SITE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related practice site. |
| 3 | `PRAC_SITE_REF_APPT_TYPE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PRAC_SITE_REF_APPT_TYPE table. |
