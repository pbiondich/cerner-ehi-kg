...
> This table stores the mapping of standard ICD10 code patterns to the AOAV_DIAGNOSIS_GROUP
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTIVE_IND` | DOUBLE | NOT NULL |  | The table row is active or inactive. |
| 2 | `AOAV_DIAGNOSIS_GROUP_CD` | DOUBLE | NOT NULL |  | Unique generated number that identifies the code value of the AOAV_DIAGNOSIS_GROUP code set |
| 3 | `AOAV_DIAG_GROUP_MAP_REF_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 4 | `REGEX_NBR` | DOUBLE |  |  | Number that orders and distinguishes src_regex patterns |
| 5 | `SRC_REGEX_TXT` | VARCHAR(255) |  |  | The regular expression pattern for a broad category or specific code patter of ICD10 codes |
