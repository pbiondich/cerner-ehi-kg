...
> Holds all of the transaction data whether it gets posted or not.
...
| 5 | `ACTIVITY_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to activity log- only populated if posted |
| 6 | `BATCH_TRANS_EXT_ID` | DOUBLE | NOT NULL |  | Primary key |
| 7 | `BATCH_TRANS_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the batch_trans table |
...
| 9 | `EDI_ADJ_AMOUNT` | DOUBLE | NOT NULL |  | From CAS loop |
| 10 | `EDI_ADJ_DR_CR_FLAG` | DOUBLE | NOT NULL |  | Debit/Credit flag for the EDI adj amount |
| 11 | `EDI_ADJ_QUANTITY` | DOUBLE | NOT NULL |  | From CAS loop |
| 12 | `EDI_ADJ_REASON_CD` | DOUBLE | NOT NULL |  | Adjustment reason |
...
| 14 | `ESRD_PAYMENT_AMOUNT` | DOUBLE | NOT NULL |  | End State Renal Disease (ESRD) payment amount |
| 15 | `INPAT_PROF_COMP_AMT` | DOUBLE | NOT NULL |  | Non-payable professional component amount related to inpatient adjudication |
| 16 | `NON_INPAT_PROF_COMP_AMT` | DOUBLE | NOT NULL |  | Non-payable professional component amount related to Medicare adjudication not related to inpatient |
| 17 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Foreign key to the table for which this entry points (Bill, Charge, Transaction) |
| 18 | `PARENT_ENTITY_NAME` | VARCHAR(20) |  |  | CHARGE |
| 19 | `POSTED_IND` | DOUBLE | NOT NULL |  | Indicates whether the transaction was posted or not |
| 20 | `SEQUENCE_NBR` | DOUBLE | NOT NULL |  | Relates to the sequence number on the Batch_Trans_File table through the Batch_Trans_id. |
