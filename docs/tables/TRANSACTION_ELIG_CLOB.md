...
> Table stores the eligibility response for an eligibility transaction
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `TRANSACTION_ELIGIBILITY_ID` | DOUBLE | NOT NULL | FK→ | The identifier of the transaction_eligibility table to which this eligibility message is related. |
| 2 | `TRANSACTION_ELIG_CLOB_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the TRANSACTION_ELIG_CLOB table. |
| 3 | `TRANS_MESSAGE_CLOB` | LONGTEXT |  |  | Contains the eligibility transaction response message. |
