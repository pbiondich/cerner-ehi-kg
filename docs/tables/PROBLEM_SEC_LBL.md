...
> Storage for security attributes associated to millennium data entities.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION_PRSNL_ID` | DOUBLE |  |  | The ID of the personnel who has taken an action (modified, deleted, etc) this label. |
...
| 6 | `PROBLEM_ID` | DOUBLE |  | FK→ | The ID of the data entity that this security attribute is associated with. |
| 7 | `PROBLEM_SEC_LBL_ID` | DOUBLE | NOT NULL |  | A system generated number to uniquely identify a row on the PROBLEM_SEC_LBL table. |
| 8 | `SENSITIVITY_REASON_CD` | DOUBLE |  |  | The code value indicating why the tagged data is considered sensitive. |
