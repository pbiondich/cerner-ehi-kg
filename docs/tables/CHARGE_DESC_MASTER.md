...
> Each row in the charge_desc_master table represents an unique service that can be charged.
...
| 5 | `CDM_CODE_TXT` | VARCHAR(254) |  |  | Client defined code used to represent a service or charge |
| 6 | `CHARGE_DESC_MASTER_ID` | DOUBLE | NOT NULL |  | A system generated number used to uniquely identify a row on the CHARGE_DESC_MASTER table. |
| 7 | `DESCRIPTION` | VARCHAR(254) |  |  | Client defined description used to represent a service or charge. |
...
| 9 | `SERVICE_TYPE_FLAG` | DOUBLE |  |  | Indicates the type of service to be billed. |
