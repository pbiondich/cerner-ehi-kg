...
> Table used to store the relation between appointment reminders and resources.
...
| 4 | `RESOURCE_CD` | DOUBLE | NOT NULL |  | The code value of a resource which has a relation to an automated messaging build. |
| 5 | `SCH_AUTO_MSG_ID` | DOUBLE | NOT NULL | FK→ | The id of an automated messaging build which has a relation to a resource. |
| 6 | `SCH_AUTO_MSG_RESOURCE_R_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCH_AUTO_MSG_RESOURCE_R table. |
