...
| 3 | `CODED_OBSERVATION_DT_TM` | DATETIME |  |  | The date and time that the observable entity was recorded by a care professional. |
| 4 | `CODED_OBSERVATION_SCT` | VARCHAR(100) |  |  | The SNOMED CT concept ID which is used to identify an observable entity. |
| 5 | `CODED_OBSERVATION_SEQ` | DOUBLE |  |  | Sequence number of coded clinical observation. |
...
| 10 | `LOC_CODED_OBSERVATION_DT_TM` | DATETIME |  |  | The local date and time that the observable entity was recorded by a care professional. |
| 11 | `OBSERVATION_VALUE` | VARCHAR(10) |  |  | The value of a clinical investigation result item. |
| 12 | `TOTAL_UPDATES` | DOUBLE | NOT NULL |  | The numbers of updates that have occurred on this table. |
| 13 | `UCUM_UNIT_OF_MEASUREMENT` | VARCHAR(10) |  |  | The unit of measurement using the Unified Code for Units of Measure code system. |
...
| 15 | `UKRWH_CDS_CODED_OBSRV_KEY` | DOUBLE | NOT NULL |  | This is the value of the unique primary identifier of the UKRWH_CDS_CODED_OBSERVATION table. It is an internal system assigned number. |
