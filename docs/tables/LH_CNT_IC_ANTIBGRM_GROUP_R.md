...
> This is a relation table which will hold the data for any Groupings. (ex. Organism Groups, Specimen Groups, etc.)
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `LH_CNT_IC_ANTIBGRM_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a single row on the LH_CNT_IC_ANTIBGRM_GROUP table. |
| 2 | `LH_CNT_IC_ANTIBGRM_GROUP_R_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_CNT_IC_ANTIBGRM_GROUP_R table. |
| 3 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Parent_Entity_ID will be the code_value for the group type. (ex. This field will contain ORAGNISM_CD for a Organism Grouping) |
| 4 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | Parent Rows Table Name |
