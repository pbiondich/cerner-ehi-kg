...
> Relates Millennium concept Diagnosis to a Diagnosis
...
| 3 | `CHILD_DIAGNOSIS_ID` | DOUBLE | NOT NULL | FK→ | Forgein key, Unique generated number that identifies a single row on the DIAGNOSIS table [Primary key id from the DIAGNOSIS table] |
| 4 | `DIAGNOSIS_DIAGNOSIS_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the DIAGNOSIS_DIAGNOSIS_RELTN table. |
...
| 6 | `PARENT_DIAGNOSIS_ID` | DOUBLE | NOT NULL | FK→ | Forgein key, Unique generated number that identifies a single row on the DIAGNOSIS table |
