...
> Doucments the optional comments for patient interest change workflow in Clinical Trials
...
| 2 | `COMMENT_ADDED_DT_TM` | DATETIME | NOT NULL |  | The date and time when comment was added |
| 3 | `COMMENT_ADDED_PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The person who added the comment |
| 4 | `COMMENT_TXT` | VARCHAR(2000) |  |  | comment added during the interest change workflow. Free Form text. |
| 5 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The patient for whom the interest change comment is being added |
| 6 | `PT_INTEREST_COMMENT_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
