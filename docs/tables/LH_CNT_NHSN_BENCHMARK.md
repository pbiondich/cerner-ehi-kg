...
> This table will store benchmark and target mapping for each facility and nurse unit, mapped through NHSN Location mapping utility
...
| 4 | `LH_CNT_NHSN_BENCHMARK_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_CNT_NHSN_BENCHMARK table. |
| 5 | `LH_CNT_NHSN_EVENT_METRIC_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to LH_CNT_NHSN_EVENT_METRIC table. |
| 6 | `NHSN_TARGET_TYPE_CD` | DOUBLE | NOT NULL |  | This is the NHSN Target Type. |
| 7 | `REPORTING_QUARTER_FLAG` | DOUBLE | NOT NULL |  | This is the quarter the benchmark is reported on. |
| 8 | `REPORTING_YEAR` | DOUBLE | NOT NULL |  | This is the year the benchmark is reported on. |
| 9 | `TARGET_PERCENT_VALUE` | DOUBLE | NOT NULL |  | This is the target percentage value. |
...
|--------|--------------|---------------|
| `LH_CNT_NHSN_EVENT_METRIC_ID` | [LH_CNT_NHSN_EVENT_METRIC](LH_CNT_NHSN_EVENT_METRIC.md) | `LH_CNT_NHSN_EVENT_METRIC_ID` |
