...
> A label used to group note phrases.
...
| 5 | `LABEL_CD` | DOUBLE | NOT NULL |  | The label to apply. |
| 6 | `NOTE_PHRASE_ID` | DOUBLE | NOT NULL | FK→ | The note phrase the label applies to. |
| 7 | `NOTE_PHRASE_LABEL_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the NOTE_PHRASE_LABEL table. |
...
| 13 | `USER_ID` | DOUBLE | NOT NULL | FK→ | The user the label applies to. |
