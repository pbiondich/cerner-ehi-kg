...
> This table stores tracking information for calls made during order processing to external services.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION_SEQUENCE` | DOUBLE | NOT NULL | FK→ | The sequence of the ORDER_ACTION that executed the external service call being tracked. |
| 2 | `ORDER_EXT_CALL_TRACKING_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the order_ext_call_tracking table. |
| 3 | `ORDER_ID` | DOUBLE | NOT NULL | FK→ | Identifies the order associated to the external service call being tracked. |
| 4 | `SERVICE_NAME_TXT` | VARCHAR(255) | NOT NULL |  | Human-readable name of the external service being tracked. |
| 5 | `TRANSACTION_CALL_DT_TM` | DATETIME |  |  | The date/time at which the external service was called. |
