...
> This table is used to solve the many to many relationship when it comes to the access rights that abilling entity may have to templates from another billing entity.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACCESS_CD` | DOUBLE | NOT NULL |  | Determines what rights the billing entity has in relationship to the account template. (read only, update, no access) - No longer used |
| 2 | `ACCT_TEMPL_ID` | DOUBLE | NOT NULL | FK→ | Identifies the account template in the relationship. |
...
| 8 | `BE_AT_RELTN_ID` | DOUBLE | NOT NULL |  | Billing Entity Account Template Relation |
| 9 | `BILLING_ENTITY_ID` | DOUBLE | NOT NULL | FK→ | Relates the billing entity to this relationship. |
