...
> Defined the location associated with a scheduled appointment
...
| 13 | `SCHEDULE_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for the event schedule. The schedules are used to track rescheduling of an event. |
| 14 | `SCH_CLINIC_ID` | DOUBLE | NOT NULL | FK→ | Clinic Id assoiciated to the location and schedule. |
