...
> This table assigns a drug to one or more therapeutic/chemical categories.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `DRUG_IDENTIFIER` | VARCHAR(6) | NOT NULL | FK→ | This field contains Multum's identification codes for generic drugs. These are extremely important codes for developers who are using Multum's database products because they serve as the primary link to all of Multum's advanced clinical services. Multum's drug_id is always of the form "d#####". |
...
| 4 | `MULTUM_CATEGORY_ID` | DOUBLE | NOT NULL | FK→ | This field is used to assign a therapeutic/chemical category to a specific drug. |
