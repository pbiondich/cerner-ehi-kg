...
| 6 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | The value of the primary identifier of the table to which the row is related (i.e., episode_id, sch_event_id, encounter, order etc.) |
| 7 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | The upper case name of the table to which this row is related (i.e., EPISODE, SCH_EVENT, ENCOUNTER, ORDER, etc.) |
| 8 | `REFERRAL_ENTITY_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the REFERRAL_ENTITY_RELTN table. |
| 9 | `REFERRAL_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related referral record. |
