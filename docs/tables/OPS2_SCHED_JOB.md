...
> The operations schedule job table stores the information about a job that has been initialized for a particular operations date.

**Description:** Operations Scheduled Job  
**Table type:** ACTIVITY  
**Primary key:** `OPS2_SCHED_JOB_ID`  
**Columns:** 19  
...
| 2 | `ACTUAL_END_DT_TM` | DATETIME |  |  | The date and time for when the job or job within a job group finished executing. |
| 3 | `ACTUAL_START_DT_TM` | DATETIME |  |  | The date and time for when the job or job within a job group started executing. |
...
| 7 | `INITIALIZE_DT_TM` | DATETIME |  |  | The date and time this scheduled job was initialized. |
| 8 | `OPS2_JOB_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to OPS2_JOB. Indicates to which job or job within a job group this scheduled job represents. |
| 9 | `OPS2_SCHED_CTRL_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to OPS2_SCHED_CTRL_GROUP. Indicates to which scheduled control group this scheduled job belongs. |
| 10 | `OPS2_SCHED_JOB_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to OPS2_SCHED_JOB_GROUP. Indicates to which scheduled job group this scheduled job belongs. |
| 11 | `OPS2_SCHED_JOB_ID` | DOUBLE | NOT NULL | PK | Unique, generated number that identifies a single row on the OPS2_SCHED_JOB table. |
| 12 | `ORIG_OPS2_SCHED_JOB_ID` | DOUBLE | NOT NULL | FK→ | This field is used to track versions of the scheduled jobs. This field will always point back to the initial value created. This may be used to find the correct version when combined with begin and end effective dates and times. |
| 13 | `SCHEDULE_DATE_DT_TM` | DATETIME | NOT NULL |  | The date and time this scheduled job is scheduled to execute. |
| 14 | `STATUS_CD` | DOUBLE | NOT NULL |  | State of the scheduled job. |
...
| `OPS2_SCHED_JOB_GROUP_ID` | [OPS2_SCHED_JOB_GROUP](OPS2_SCHED_JOB_GROUP.md) | `OPS2_SCHED_JOB_GROUP_ID` |
| `ORIG_OPS2_SCHED_JOB_ID` | [OPS2_SCHED_JOB](OPS2_SCHED_JOB.md) | `OPS2_SCHED_JOB_ID` |

## Referenced by (2)

| From table | Column |
|------------|--------|
| [OPS2_SCHED_JOB](OPS2_SCHED_JOB.md) | `ORIG_OPS2_SCHED_JOB_ID` |
| [OPS2_SCHED_STEP](OPS2_SCHED_STEP.md) | `OPS2_SCHED_JOB_ID` |
