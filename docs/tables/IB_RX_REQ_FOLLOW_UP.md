...
> Tracks follow-up requests from the pharmacy for inbound eRx workflows.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `FOLLOW_UP_CNT` | DOUBLE | NOT NULL |  | Count that indicates the number of follow-up attempts for this Rx request. |
| 2 | `IB_RX_REQ_FOLLOW_UP_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the IB_RX_REQ_FOLLOW_UP table. |
| 3 | `IB_RX_REQ_ID` | DOUBLE | NOT NULL | FK→ | References the associated Rx for the follow-up request. |
| 4 | `MESSAGE_IDENT` | VARCHAR(255) | NOT NULL |  | References the associated Rx for the follow-up request. |
| 5 | `RECEIVED_DT_TM` | DATETIME |  |  | Date/Time the follow-up request was received. |
| 6 | `RELATES_TO_MESSAGE_IDENT` | VARCHAR(255) | NOT NULL |  | MESSAGE_IDENTIFIER of the linked Rx follow-up request used in the conversation. |
