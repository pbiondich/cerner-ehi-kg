...
> Stores favorite items for users of AP Applications.
...
| 5 | `AP_FAVORITE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the AP_FAVORITE table. |
| 6 | `GROUP_CD` | DOUBLE |  |  | The processing group that this personnel has marked as a favorite. |
| 7 | `PRSNL_ID` | DOUBLE |  | FK→ | The personnel that has chosen favorite items. |
| 8 | `TASK_ASSAY_CD` | DOUBLE |  | FK→ | Task Assay Code that has been chosen as a favorite. |
