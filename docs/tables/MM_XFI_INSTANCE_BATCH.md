...
> Used to group a set of MM_XFI_INSTANCE rows for batch processing.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BATCH_GROUP_ID` | DOUBLE |  | FK→ | Stores the batch_id for a set of transactions |
| 2 | `BATCH_PROCESS_FLAG` | DOUBLE | NOT NULL |  | Stores the status of the batch, if the batch is processed the indicator will be set to 1 else it will be 0. |
| 3 | `MM_XFI_INSTANCE_BATCH_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the MM_XFI_INSTANCE_BATCH table. |
