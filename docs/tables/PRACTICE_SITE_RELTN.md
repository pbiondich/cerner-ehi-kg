...
> Associates a location or an organization to a practice site.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Parent Entity Name , which we can join the Organization/Location table to PRACTICE_SITE, could be organization/location |
| 2 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | Parent Entity Name , which we can join the Organization/Location table to PRACTICE_SITE, could be organization/location |
| 3 | `PRACTICE_SITE_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key relationship to the Practice_Site table. |
| 4 | `PRACTICE_SITE_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PRACTICE_SITE_RELTN table. |
