...
> Stores various Surescript messages for New Prescriptions (NewRx), Refill Requests (RefReq), Refill Responses (RefRes), Verify, Status, and Error messages.

**Description:** Pharmacy Ambulatory Prescription Message  
**Table type:** ACTIVITY  
**Primary key:** `RXA_PRESCRIPTION_MSG_ID`  
**Columns:** 25  
...
| 17 | `REF_RXA_PRESCRIPTION_MSG_ID` | DOUBLE | NOT NULL | FK→ | Identifies the reference prescription message identifier that this row relates to |
| 18 | `RXA_PRESCRIPTION_MSG_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the RXA_PRESCRIPTION_MSG table. |
| 19 | `RX_MSG_REQUEST_TYPE_CD` | DOUBLE | NOT NULL |  | Identifies a specific prescription message request type (ie. Generic Substitution, Prior Authorization, Therapeutic Interchange, Drug Usage Evaluation, Script Clarification, Out of Stock, Prescriber Authorization, etc) |
...
|--------|--------------|---------------|
| `REF_RXA_PRESCRIPTION_MSG_ID` | [RXA_PRESCRIPTION_MSG](RXA_PRESCRIPTION_MSG.md) | `RXA_PRESCRIPTION_MSG_ID` |

## Referenced by (6)

| From table | Column |
|------------|--------|
| [ORDER_DISPENSE](ORDER_DISPENSE.md) | `RXA_PRESCRIPTION_MSG_ID` |
| [RXA_PRESCRIPTION_MSG](RXA_PRESCRIPTION_MSG.md) | `REF_RXA_PRESCRIPTION_MSG_ID` |
| [RX_PENDING_REFILL](RX_PENDING_REFILL.md) | `RXA_PRESCRIPTION_MSG_ID` |
| [RX_REFILL_HX](RX_REFILL_HX.md) | `RXA_PRESCRIPTION_MSG_ID` |
| [RX_REFILL_TRACK_HX](RX_REFILL_TRACK_HX.md) | `REQUEST_MSG_ID` |
| [RX_REFILL_TRACK_HX](RX_REFILL_TRACK_HX.md) | `RXA_PRESCRIPTION_MSG_ID` |
