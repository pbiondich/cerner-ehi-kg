...
> Table containing event information for meaningful use NQF stage 2 2020
...
| 3 | `BR_DATAM_VSET_ITEM_ID` | DOUBLE | NOT NULL |  | References the id in BR_DATAM_VAL_SET_ITEM. |
| 4 | `BR_DATAM_VSET_ITEM_NEG_ID` | DOUBLE | NOT NULL |  | References the negation in BR_DATAM_VAL_SET_ITEM. |
...
| 20 | `LAST_UPDATED_PROVIDER_ID` | DOUBLE | NOT NULL |  | This gives the ID of the provider whoever has updated the row last |
| 21 | `LH_AMB_EVENT_DATA_2020_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_AMB_EVENT_DATA_2020 table. |
