...
> Contains the security assignments for Discern Analytics 2.0 users.
...
| 6 | `DA_USER_SECURITY_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the DA_USER_SECURITY table. |
...
| 9 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Unique identifier for the Discern Analytics item that security is being set for. |
| 10 | `PARENT_ENTITY_NAME` | VARCHAR(30) |  |  | Table name corresponding to the Discern Analytics item security is being set for. |
| 11 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | Identifier for the person that data security is being set for. |
| 12 | `SECURITY_ASSIGNMENT_CD` | DOUBLE | NOT NULL |  | Code value for the security assignment that is to be associated with an entity. |
