...
> Stores all physicians associated with a charge.
...
| 2 | `CHARGE_EVENT_ACT_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the charge_event_act table. |
| 3 | `PRSNL_ID` | DOUBLE | NOT NULL |  | Foreign key to the prsnl table. |
| 4 | `PRSNL_TYPE_CD` | DOUBLE | NOT NULL |  | Value from code_set 13029. This denotes the type of physician. |
