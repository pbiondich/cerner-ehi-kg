...
> This table defines the columns that a personnel has chosen to appear on all their worklists.
...
| 5 | `ATTRIBUTE_CD` | DOUBLE | NOT NULL |  | The column that a person has chosen to be on their worklist view. |
| 6 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The personnel who is defining the column as a choice. |
| 7 | `RAD_WRKLST_COL_CHOICE_ID` | DOUBLE | NOT NULL |  | Unique, generated number that identifies a single row on the RAD_WRKLIST_COL_CHOICE table. |
| 8 | `SEQ_NBR` | DOUBLE | NOT NULL |  | Defines the order in which the columns are displayed to the user. |
...
| 14 | `WRKLST_TYPE_CD` | DOUBLE | NOT NULL |  | The type of worklist the column will be a choice for. |
