...
> This table contains all the default roles that are defined.
...
| 2 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | References the Organization table for Organization. The unique identifier of the organization (from the organization table) to which the member belongs. |
...
| 4 | `POSITION_CD` | DOUBLE | NOT NULL |  | The position is used to determine the applications and tasks the personnel is authorized to use |
| 5 | `PROT_DEFAULT_ROLE_ID` | DOUBLE | NOT NULL |  | Primary Key. Generated from sequence protocol_def_seq. |
| 6 | `PROT_ROLE_CD` | DOUBLE | NOT NULL |  | The role which is played by the person/entity on the amendment. |
| 7 | `ROLE_TYPE_CD` | DOUBLE | NOT NULL |  | The Type of role played. eg., personal/Institutional |
