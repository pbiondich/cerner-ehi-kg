...
> Stores history information for a relationship between an authorization and a referral.
...
| 5 | `AUTHORIZATION_ID` | DOUBLE |  | FK→ | Uniquely identifies the related row on the authorization table. |
| 6 | `AUTH_REFERRAL_RELTN_HIST_ID` | DOUBLE | NOT NULL |  | A system generated number used to uniquely identify a row on the AUTH_REFERRAL_RELTN_HIST table. |
| 7 | `AUTH_REFERRAL_RELTN_ID` | DOUBLE |  | FK→ | A system generated number to uniquely identify a row on the AUTH_REFERRAL_RELTN table. |
| 8 | `CHANGE_BIT` | DOUBLE |  |  | Identifies which columns have had a change. |
...
| 10 | `REFERRAL_ID` | DOUBLE |  | FK→ | Uniquely identifies the related referral |
| 11 | `TRACKING_BIT` | DOUBLE |  |  | Identifies which columns are being tracked for history. |
...
| `AUTH_REFERRAL_RELTN_ID` | [AUTH_REFERRAL_RELTN](AUTH_REFERRAL_RELTN.md) | `AUTH_REFERRAL_RELTN_ID` |
