...
> Stores information about a relationship between an authorization item and an order.

**Description:** Authorization Item Order Relationship  
**Table type:** ACTIVITY  
**Primary key:** `AUTH_ITEM_ORDER_RELTN_ID`  
**Columns:** 15  
...
| 5 | `AUTHORIZATION_ITEM_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related authorization item. |
| 6 | `AUTH_ITEM_ORDER_RELTN_ID` | DOUBLE | NOT NULL | PK | Uniquely identifies information about a relationship between an authorization item and an order |
| 7 | `AUTH_USED_CNT` | DOUBLE | NOT NULL |  | The number of the item's authorized units that were used for the order. |
...
| 10 | `ORDER_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related row on the orders table. |
...
| From table | Column |
|------------|--------|
| [AUTH_ITEM_ORDER_R_HIST](AUTH_ITEM_ORDER_R_HIST.md) | `AUTH_ITEM_ORDER_RELTN_ID` |
