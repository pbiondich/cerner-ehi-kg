...
> This table contains word equivalents to be used when searching nomenclature terms.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CMT_WORDEQIV_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
...
| 7 | `WORDEQIV_NBR` | DOUBLE | NOT NULL |  | The words, phrases and abbreviations that share a common Wordequiv_nbr value are interchangeable for the purposes of searches. Example: An equivalent block could contain the following: "TB", "tuberculosis", "tuberculous" Note: Wordequiv_nbr is not maintained as a unique identifier across releases. It should only be regarded as an index to link equivalents in the context of a particular release. |
| 8 | `WORDROLE_MEAN` | VARCHAR(12) | NOT NULL |  | MEANING |
| 9 | `WORDTEXT_NAME` | VARCHAR(100) | NOT NULL |  | A word, phrase or abbreviation that is equivalent to the WordText of other rows that share the same Wordequiv_nbr. Note: If a word or phrase has two or more possible meanings it may be represented by more than one row in this table. Each row containing the same WordText must be associated with a different Wordequiv_nbr value. |
| 10 | `WORDTYPE_MEAN` | VARCHAR(12) | NOT NULL |  | MEANING |
