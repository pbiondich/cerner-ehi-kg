...
> Table used to store the multiple pharmacy review required indicator of Order Catalog.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CATALOG_CD` | DOUBLE | NOT NULL | FK→ | Identifies the orderable item. Foreign Key from ORDER_CATALOG table. |
| 2 | `MULTIPLE_PHARMACY_REVIEW_IND` | DOUBLE | NOT NULL |  | Stores the multiple pharmacy review indicator. 0 indicates that catalog is not eligible for Multiple pharmacy review, 1 indicates that catalog is eligible for Multiple pharmacy review. |
| 3 | `OC_MULTI_PHARMACY_REVIEW_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the OC_MULTI_PHARMACY_REVIEW table. |
