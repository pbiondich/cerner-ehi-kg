...
> This table contains information about the digital slides sent by a foreign imaging system. This includes parameters required to connect to a foreign imaging system to view the digital slides.
...
| 4 | `AP_DIGITAL_SLIDE_ID` | DOUBLE |  | FK→ | This field uniquely identifies a row (a slide) included in the AP_DIGITAL_SLIDE table. |
| 5 | `AP_DIGITAL_SLIDE_INFO_ID` | DOUBLE | NOT NULL |  | A system generated number that uniquely identifies a single row on the AP_DIGITAL_SLIDE_INFO table. |
...
| 13 | `URL_ELEM_STRING_TXT` | VARCHAR(200) |  |  | Specifies the type of url element stored in url_elemnt_string_txt. |
| 14 | `URL_ELEM_TYPE_TFLG` | VARCHAR(30) |  |  | Contains the URL element string which identifies the identifier to launch the URL |
...
|--------|--------------|---------------|
| `AP_DIGITAL_SLIDE_ID` | [AP_DIGITAL_SLIDE](AP_DIGITAL_SLIDE.md) | `AP_DIGITAL_SLIDE_ID` |
