...
> Location for MPages configuration information.
...
| 2 | `BEGIN_EFFECTIVE_DT_TM` | DATETIME |  |  | Date and time the record started to be effective. |
| 3 | `CONFIG_KEY` | VARCHAR(255) | NOT NULL |  | MPages configuration within a given type. |
| 4 | `CONFIG_TXT` | VARCHAR(4000) | NOT NULL |  | MPages configuration value for a given key. |
...
| 6 | `MP_CONTENT_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the MP_CONTENT_CONFIG table. |
| 7 | `MP_CONTENT_CONFIG_TYPE_CD` | DOUBLE | NOT NULL |  | The type of content being configured. |
