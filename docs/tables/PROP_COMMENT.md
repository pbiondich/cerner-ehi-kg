...
|--:|--------|------|:-----:|-------|------------|
| 1 | `COMMENT_TYPE_CD` | DOUBLE | NOT NULL |  | Comment type code for the order. |
| 2 | `LONG_TEXT_ID` | DOUBLE | NOT NULL |  | Long Text Identifier |
| 3 | `ORDER_ID` | DOUBLE | NOT NULL |  | Unique number that ties the order laboratory row to the orders tables. |
| 4 | `PROP_ID` | DOUBLE | NOT NULL |  | The identifier of the row in the table. |
