...
> Scheduling Refernence Comments
...
| 10 | `PARENT_TABLE` | VARCHAR(32) | NOT NULL |  | The parent table corresponding to the PARENT_ID |
| 11 | `SUB_TEXT_CD` | DOUBLE | NOT NULL | FK→ | The coded identifier for the scheduling sub text type. The sub text types allow for multiple set of text within a text type (prep, post, guidelines, etc.) |
| 12 | `SUB_TEXT_MEANING` | VARCHAR(12) |  |  | The 12-character string corresponding to the scheduling text sub-type. |
| 13 | `TEXT_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier used to store the actual test on the LONG_TEXT table. |
| 14 | `TEXT_TYPE_CD` | DOUBLE | NOT NULL | FK→ | The coded identifier for the scheduling text type. |
| 15 | `TEXT_TYPE_MEANING` | VARCHAR(12) |  |  | The 12-character string corresponding to the scheduling text type. |
