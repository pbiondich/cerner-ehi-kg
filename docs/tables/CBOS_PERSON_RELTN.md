...
> Relates a Consolidated Benefit Order Schedule record to one or more Person.
...
| 6 | `CBOS_PERSON_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CBOS_PERSON_RELTN table. |
| 7 | `CONS_BO_SCHED_ID` | DOUBLE | NOT NULL | FK→ | This is the consolidated benefit order schedule for this person relation. |
...
| 11 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the orgaization table. It is an internal system assigned number. |
| 12 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related person. |
