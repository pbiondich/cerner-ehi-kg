...
> The operations step table stores information about a step and its default parameter values. A step relates one to one with a request defined in Application Registration.

**Description:** Operations Step  
**Table type:** REFERENCE  
**Primary key:** `OPS2_STEP_ID`  
**Columns:** 23  
...
| 6 | `OPERATOR_MSG_TXT` | VARCHAR(100) |  |  | A message to be communicated to the 'operator'. |
| 7 | `OPS2_JOB_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to OPS2_JOB. Indicates to which job or job within a job group this step belongs. |
| 8 | `OPS2_STEP_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the OPS2_STEP table. |
| 9 | `OPS2_STEP_TEMPLATE_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to OPS2_STEP_TEMPLATE. Indicates to which step template this step was originally created from. |
| 10 | `ORIG_OPS2_STEP_ID` | DOUBLE | NOT NULL | FK→ | This field is used to track versions of the steps. This field will always point back to the initial value created. This may be used to find the correct version when combined with begin and end effective dates and times. |
| 11 | `OUTPUT_DISTRIBUTION_TXT` | VARCHAR(100) |  |  | User input parameters for the request 'output distribution' field. |
| 12 | `REQUEST_NAME` | VARCHAR(100) | NOT NULL |  | The request name that this step executes. |
| 13 | `REQUEST_NBR` | DOUBLE | NOT NULL |  | The request number that this step executes. |
| 14 | `STEP_SEQ` | DOUBLE | NOT NULL |  | Number to order the sequence of steps within a job. |
| 15 | `STOP_JOB_ON_ERROR_IND` | DOUBLE | NOT NULL |  | Indicates if the entire job should stop further executing if this step fails (STOP_JOB_ON_ERROR_IND = 1) or continue executing (STOP_JOB_ON_ERROR_IND = 0). |
| 16 | `THRESHOLD_IND` | DOUBLE | NOT NULL |  | Indicates if the elapsed time for this step should be monitored. |
| 17 | `THRESHOLD_PCT` | DOUBLE | NOT NULL |  | The percentage over the step's average elapsed time at which a threshold alert should be reported. |
| 18 | `TIME_ZONE_TXT` | VARCHAR(100) |  |  | The time zone context to be provided when executing the request of the step. This is often used by reporting solutions to provide the context in which date and time values should be formated. |
...
| `OPS2_STEP_TEMPLATE_ID` | [OPS2_STEP_TEMPLATE](OPS2_STEP_TEMPLATE.md) | `OPS2_STEP_TEMPLATE_ID` |
| `ORIG_OPS2_STEP_ID` | [OPS2_STEP](OPS2_STEP.md) | `OPS2_STEP_ID` |

## Referenced by (2)

| From table | Column |
|------------|--------|
| [OPS2_SCHED_STEP](OPS2_SCHED_STEP.md) | `OPS2_STEP_ID` |
| [OPS2_STEP](OPS2_STEP.md) | `ORIG_OPS2_STEP_ID` |
