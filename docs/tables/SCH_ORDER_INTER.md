...
| 7 | `CATALOG_CD` | DOUBLE | NOT NULL | FK→ | The unique identifier for the order catalog entry. |
...
| 9 | `INTER_TYPE_CD` | DOUBLE | NOT NULL | FK→ | A coded identifier corresponding to the type of interaction. |
| 10 | `INTER_TYPE_MEANING` | VARCHAR(12) |  |  | A 12-character corresponding to the interaction type code. |
| 11 | `LOCATION_CD` | DOUBLE | NOT NULL | FK→ | location code |
...
| 13 | `SEQ_GROUP_ID` | DOUBLE | NOT NULL | FK→ | A unique identifier for a scheduling sequence group. |
...
| `INTER_TYPE_CD` | [CODE_VALUE](CODE_VALUE.md) | `CODE_VALUE` |
