...
> Stores all custom fields attributed to specific referrals

**Description:** REFERRAL_CUSTOM  
**Table type:** ACTIVITY  
**Primary key:** `REFERRAL_CUSTOM_ID`  
**Columns:** 14  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CREATE_DT_TM` | DATETIME | NOT NULL |  | Dzte and time the referral was created |
| 2 | `REFERRAL_CUSTOM_FIELD_KEY` | VARCHAR(255) | NOT NULL |  | Identifies a unique custom field name for the specific referral. |
| 3 | `REFERRAL_CUSTOM_ID` | DOUBLE | NOT NULL | PK | PRIMARY KEY |
| 4 | `REFERRAL_CUSTOM_VALUE_BOOLEAN` | DOUBLE | NOT NULL |  | Contains the boolean value associated to the custom field defined in referral_custom_field_key. |
| 5 | `REFERRAL_CUSTOM_VALUE_DOUBLE` | DOUBLE | NOT NULL |  | Contains the double value associated to the custom field defined in referral_custom_field_key. |
| 6 | `REFERRAL_CUSTOM_VALUE_DT_TM` | DATETIME |  |  | Contains the date/time value associated to the custom field defined in referral_custom_field_key. |
| 7 | `REFERRAL_CUSTOM_VALUE_STRING` | VARCHAR(255) |  |  | Contains the string value associated to the custom field defined in referral_custom_field_key. |
| 8 | `REFERRAL_CUSTOM_VALUE_TYPE` | VARCHAR(40) | NOT NULL |  | Identifies the data type of the value corresponding to the custom key defined in referral_custom_field_key. This will indicate the referral_custom_value_* column that is populated. |
| 9 | `REFERRAL_ID` | DOUBLE | NOT NULL | FK→ | A foreign key value from the REFERRAL table which Uniquely identifies the related referral. |
...
| From table | Column |
|------------|--------|
| [REFERRAL_CUSTOM_HIST](REFERRAL_CUSTOM_HIST.md) | `REFERRAL_CUSTOM_ID` |
