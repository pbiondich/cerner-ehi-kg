...
> Contains a relationship between personnel and service sets.
...
| 5 | `FACILITY_ORG_ID` | DOUBLE | NOT NULL | FK→ | The organization that is being associated to the service set. |
| 6 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Related Entity ID (from prsnl or prsnl_group table) |
| 7 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | The name of the table that the Parent_Entity_ID is from. (prsnl or prsnl_group) |
| 8 | `PPA_BEG_EFFECTIVE_DT_TM` | DATETIME | NOT NULL |  | The date and time that the row becomes effective. |
| 9 | `PPA_END_EFFECTIVE_DT_TM` | DATETIME | NOT NULL |  | The date and time of when this row is no longer effective. |
| 10 | `RC_PRSNL_SERVICE_SET_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RC_PRSNL_SERVICE_SET_RELTN table. |
| 11 | `RC_SERVICE_SET_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the service set associated with this relationship. |
