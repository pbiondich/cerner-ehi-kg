# SN_TRACK_EVENT_RELTN

> Stote Form codes associated with track events. ( ASC Set Perioperative Tracking Events based on Powerform Events)

**Description:** Surginet Track Event Relation Details  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `EVENT_CD` | DOUBLE |  | FK→ | Clinical event code of the power form associated with a track event.; |
| 2 | `SN_TRACK_EVENT_RELTN_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 3 | `TRACK_EVENT_ID` | DOUBLE |  | FK→ | The track event the form is associated to (from the TRACK_EVENT table).; |
