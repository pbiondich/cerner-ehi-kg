...
> Used for tracking history of columns on the ENCNTR_DATA_NOT_COLL table.
...
| 6 | `ENCNTR_DATA_NOT_COLL_HIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ENCNTR_DATA_NOT_COLL_HIST table. |
| 7 | `ENCNTR_DATA_NOT_COLL_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the encounter not collected table. |
| 8 | `EST_FINANCIAL_RESP_AMT_CD` | DOUBLE | NOT NULL |  | The information describing if the estimated financial responsibility amount is not collected or why it is not collected for the encounter (CS 4660008). |
| 9 | `PM_HIST_TRACKING_ID` | DOUBLE | NOT NULL |  | This is the value of the unique primary identifier of the pm_hist_tracking table. It is an internal system assigned number. |
| 10 | `REFERRING_PHYSICIAN_CD` | DOUBLE | NOT NULL |  | The information describing if the referring provider is not collected or why it is not collected for the encounter. |
| 11 | `TRACKING_BIT` | DOUBLE | NOT NULL |  | Identifies which columns are being tracked for history. |
| 12 | `TRANSACTION_DT_TM` | DATETIME | NOT NULL |  | ** OBSOLETE **. Use column updt_dt_tm for any filtering/ordering query. If transaction date time is needed, it should be retrieved from pm_hist_tracking table. Note that its date may be in the past, as in before the update date time. |
...
|--------|--------------|---------------|
| `ENCNTR_DATA_NOT_COLL_ID` | [ENCNTR_DATA_NOT_COLL](ENCNTR_DATA_NOT_COLL.md) | `ENCNTR_DATA_NOT_COLL_ID` |
