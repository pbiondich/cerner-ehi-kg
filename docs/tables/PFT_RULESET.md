...
> Relates Rulesets to the long_blob_reference table.

**Description:** ProFit Ruleset  
**Table type:** REFERENCE  
**Primary key:** `PFT_RULESET_ID`  
**Columns:** 13  
...
| 5 | `LONG_BLOB_ID` | DOUBLE | NOT NULL | FK→ | Identifier for the physical rule set in the LONG_BLOB_REFERENCE table. |
| 6 | `PFT_RULESET_ID` | DOUBLE | NOT NULL | PK | Id which uniquely identifies a Rule set. |
| 7 | `RULESET_KEY` | VARCHAR(50) | NOT NULL |  | An internal key to identify Rule sets. |
| 8 | `SEQ_NBR` | DOUBLE | NOT NULL |  | Identifies, if necessary, the sequence within a set of multiple Rule sets. |
...
| From table | Column |
|------------|--------|
| [PFT_RULESET_FLEX_RELTN](PFT_RULESET_FLEX_RELTN.md) | `PFT_RULESET_ID` |
