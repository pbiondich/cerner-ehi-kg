...
> This table stores the Flex functions and actions that will be used by the engine.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION` | DOUBLE |  |  | This is an action associated with this row in the function table. |
...
| 7 | `DESCRIPTION` | VARCHAR(255) |  |  | A description of the function that is both for our purposes and to give the client an easy way of picking a function. |
...
| 9 | `FLEX_FUNCTION_ID` | DOUBLE | NOT NULL |  | This is an sequence to the function that is in this table. |
| 10 | `FUNCTION` | DOUBLE |  |  | This is an integer value used by the image to help determine the conversation type. |
| 11 | `PARENT_ENTITY_ID` | DOUBLE |  |  | This tells us which data source we are associated with. |
| 12 | `PARENT_ENTITY_NAME` | VARCHAR(32) |  |  | This tells us which data source we are associated with. |
