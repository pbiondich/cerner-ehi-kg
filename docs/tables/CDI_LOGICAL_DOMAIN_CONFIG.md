...
> Document Imaging logical domain-specific configuration.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ALIAS_CONTRIB_SRC_CD` | DOUBLE | NOT NULL |  | Specifies the contributor source to use when resolving document type aliases for this domain. |
| 2 | `CDI_LOGICAL_DOMAIN_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CDI_LOGICAL_DOMAIN_CONFIG table. |
