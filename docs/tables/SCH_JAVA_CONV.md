...
> Table used to store information regarding the Java conversations configured in SchTools.
...
| 5 | `SCH_CONV_ACTION_TYPE_CD` | DOUBLE | NOT NULL |  | Unique number that identifies the Java conversation action type. |
| 6 | `SCH_JAVA_CONV_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the SCH_JAVA_CONV table. |
| 7 | `SCH_JAVA_CONV_IDENT` | VARCHAR(255) | NOT NULL |  | Unique identifier that is mapped to a Java conversation. It is created using the "Conversations & Rules" segment in Revenue Cycle Maintanence tool. |
| 8 | `SCH_JAVA_CONV_NAME` | VARCHAR(500) | NOT NULL |  | Name of the Java conversation mapped to an identifier. It is created using the "Conversations & Rules" segment in Revenue Cycle Maintanence tool. |
...
| 14 | `VERSION_DT_TM` | DATETIME | NOT NULL |  | The version date and time marks the date the record became historical.. Version Date time defaulted to 12/31/2100. |
