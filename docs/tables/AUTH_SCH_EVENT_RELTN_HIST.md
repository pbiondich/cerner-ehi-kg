...
> Stores information about the history of an authorization/schedule event relationship.
...
| 5 | `AUTHORIZATION_ID` | DOUBLE |  | FK→ | Unique system generated number that identifies a single row on the AUTHORIZATION table to which the schedule event is related. |
| 6 | `AUTH_SCH_EVENT_RELTN_HIST_ID` | DOUBLE | NOT NULL |  | Unique system generated number that identifies a single row on the AUTH_SCH_EVENT_RELTN_HIST table.. |
| 7 | `AUTH_SCH_EVENT_RELTN_ID` | DOUBLE |  | FK→ | Unique system generated number that identifies a single row on the AUTH_SCH_EVENT_RELTN table. |
...
| 10 | `SCH_EVENT_ID` | DOUBLE |  | FK→ | Unique system generated number that identifies a single row on the SCH_EVENT table. |
...
| `AUTH_SCH_EVENT_RELTN_ID` | [AUTH_SCH_EVENT_RELTN](AUTH_SCH_EVENT_RELTN.md) | `AUTH_SCH_EVENT_RELTN_ID` |
