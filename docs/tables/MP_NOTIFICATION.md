...
> Track notification actions needed or taken within MPages.

**Description:** MPages Notification  
**Table type:** ACTIVITY  
**Primary key:** `MP_NOTIFICATION_ID`  
**Columns:** 15  
...
| 5 | `ENCNTR_ID` | DOUBLE | NOT NULL | FK→ | The encounter on which the communication has been posted. |
| 6 | `MP_NOTIFICATION_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the MP_NOTIFICATION table. |
| 7 | `NOTIFICATION_SEQ` | DOUBLE | NOT NULL |  | Sequences the notification type flag values with an encounter and personnel group. |
| 8 | `NOTIFICATION_TYPE_FLAG` | DOUBLE | NOT NULL |  | Indicates what type of notification has been created. |
| 9 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The person for which the communication has been posted. |
| 10 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The personnel who has posted the communication. |
...
| From table | Column |
|------------|--------|
| [MP_NOTIFICATION_DETAIL](MP_NOTIFICATION_DETAIL.md) | `MP_NOTIFICATION_ID` |
