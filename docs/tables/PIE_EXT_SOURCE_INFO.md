...
> Stores information about external sources that are configured through the trust model
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `PIE_EXT_SOURCE_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key value from the primay key of a row in the External Source table |
| 2 | `PIE_EXT_SOURCE_INFO_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 3 | `SOURCE_DESC` | VARCHAR(255) |  |  | Description of the external source that is beiing configured |
| 4 | `SOURCE_NAME` | VARCHAR(255) |  |  | Name of the source configured |
...
|--------|--------------|---------------|
| `PIE_EXT_SOURCE_ID` | [PIE_EXT_SOURCE](PIE_EXT_SOURCE.md) | `PIE_EXT_SOURCE_ID` |
