...
> This table is used to store Blood Stream Infection PC metrics from the Lighthouse Abstractor (eQuality Check). This table is at the encounter grain.
...
| 4 | `BIRTH_WEIGHT` | VARCHAR(10) |  |  | Weight of the birth |
| 5 | `BLOODSTREAM_INFECTION_FLAG` | DOUBLE |  |  | Identifies if the patient had a bloodstream infection present on admission.1 - Yes, there was a bloodstream infection present on admission0 - No, there was not a bloodstream infection present on admission999 - Question was not answered |
| 6 | `BLOOD_INF_CONFIRMED_FLAG` | DOUBLE |  |  | A flag indicating if a bloodstream Infection is confirmed. 0 - No, 1 - Yes, 999 - Missing. |
| 7 | `CLINICAL_TRIAL_FLAG` | DOUBLE |  |  | Identifies if the patient is excluded from the measure due to participation in a clinical trial. |
...
| 33 | `FIRST_PROCESS_DT_TM` | DATETIME |  |  | The date/time the record was first loaded into the table. |
...
| 36 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time the record was last loaded into the table. |
| 37 | `LH_ABS_PC_BSI_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_ABS_PC_BSI_METRICS table. |
...
| 39 | `NUMERATOR_1_IND` | DOUBLE |  |  | Represents if the person is in the numerator for PC1 |
| 40 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Identifies the medical record number of the patient. |
| 41 | `OTH_DIAGNOSIS_LIST` | VARCHAR(400) |  |  | A comma separated list of other diagnoses associated with the encounter. |
...
| 54 | `REJECT_1_IND` | DOUBLE |  |  | Represents if the person is rejected for PC1 |
| 55 | `SAMPLE_IND` | DOUBLE |  |  | This case represent part of a sample |
| 56 | `UPDT_CNT` | DOUBLE |  |  | Set to 0 on insert. Incremented by 1 on update. Used to recognize update conflict where data in a row updated by one application is at risk of being lost by a second application attempting to update the row. |
| 57 | `UPDT_DT_TM` | DATETIME |  |  | The date and time the row was last inserted or updated. |
...
| 59 | `UPDT_SOURCE` | VARCHAR(50) |  |  | The script name responsible for updating the record. |
| 60 | `UPDT_TASK` | VARCHAR(50) |  |  | The registered (assigned) task number for the process that inserted or updated the row. |
