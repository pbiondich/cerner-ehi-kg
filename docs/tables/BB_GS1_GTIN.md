...
> Maintains a relationship of manufacturers, product types and the Global Trade Item Numbers that represent them.
...
| 2 | `BB_GS1_GTIN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the BB_GS1_GTIN table. |
| 3 | `GTIN_TXT` | VARCHAR(22) | NOT NULL |  | Contains the global trade identification number in this relationship. |
| 4 | `MANUFACTURER_ID` | DOUBLE | NOT NULL | FK→ | The organization Id for the blood bank manufacturer. |
| 5 | `PRODUCT_CD` | DOUBLE | NOT NULL |  | Contains the related product catalog code. |
