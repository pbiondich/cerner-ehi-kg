...
> Stores information to help with managing the Medicare health plan.

**Description:** Encounter Meidicare Management  
**Table type:** ACTIVITY  
**Primary key:** `ENCNTR_MEDICARE_MGMT_ID`  
**Columns:** 15  
...
| 8 | `ENCNTR_MEDICARE_MGMT_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the encntr_medicare_mgmt table. |
| 9 | `IN_RESIDENCE_DAYS` | DOUBLE |  |  | The total number of individual days that qualified as being in residence, and does not include future or planned days. |
| 10 | `REMAINING_COVERED_DAYS` | DOUBLE |  |  | The number of covered days remaining for Medicare in the current benefit period as of the current date. |
...
| From table | Column |
|------------|--------|
| [ENCNTR_MEDICARE_MGMT_HIST](ENCNTR_MEDICARE_MGMT_HIST.md) | `ENCNTR_MEDICARE_MGMT_ID` |
