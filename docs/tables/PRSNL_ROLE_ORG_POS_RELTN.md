...
> This table is used to store the Job role code and position relationship for an organization.
...
| 5 | `AREA_OF_WORK_CD` | DOUBLE | NOT NULL |  | The area where a job role is performed. It further defines the job role for a personnel. |
...
| 8 | `JOB_ROLE_CD` | DOUBLE | NOT NULL |  | Descibes the personnel's job as related to this organization; e.g., Cardiac Surgeon or Physical Therapist. |
| 9 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | The organization that has a relationship with the Job Role. |
| 10 | `POSITION_CD` | DOUBLE | NOT NULL |  | Position code value |
| 11 | `ROLE_ORG_POS_RELTN_ID` | DOUBLE | NOT NULL |  | The unique ID of the Personnel job rol position |
