...
> Stores information relating to a person's communication preferences
...
| 5 | `ADDR_TYPE_CD` | DOUBLE |  |  | identifies the type of address for postal deliveries. |
| 6 | `CHANGE_BIT` | DOUBLE | NOT NULL |  | Identifies which columns have had a change. |
| 7 | `COMMUNICATION_TYPE_CD` | DOUBLE | NOT NULL |  | identifies the type of communication (i.e., appointment reminders, clinical communication, statements, etc). |
| 8 | `CONTACT_METHOD_CD` | DOUBLE | NOT NULL |  | denotes the person's preferred method of communication should he or she need to be contacted for the associated communication type. |
| 9 | `PERSON_PREF_COMM_HIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a row on the PERSON_PREF_COMM_HIST table. |
| 10 | `PERSON_PREF_COMM_ID` | DOUBLE | NOT NULL | FK→ | Unique generated number that identifies a row on the PERSON_PREF_COMM table. |
| 11 | `PHONE_TYPE_CD` | DOUBLE |  |  | identifies the type of email, phone, or fax (i.e., home, business, etc). |
...
| 19 | `VERIFY_DT_TM` | DATETIME |  |  | the date and time the communication type was last verified. |
...
|--------|--------------|---------------|
| `PERSON_PREF_COMM_ID` | [PERSON_PREF_COMM](PERSON_PREF_COMM.md) | `PERSON_PREF_COMM_ID` |
