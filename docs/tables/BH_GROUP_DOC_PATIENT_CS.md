...
> The charges (bill items) for each patient and their encounter for the therapy session.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BH_GROUP_DOC_PATIENT_CS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the BH_GROUP_DOC_PATIENT_CS table. |
| 2 | `BH_GROUP_DOC_PATIENT_ID` | DOUBLE | NOT NULL | FK→ | The patient that the information on this row pertains to. |
| 3 | `BILL_ITEM_ID` | DOUBLE | NOT NULL | FK→ | Selection of a charge(bill_item) to drop charges; |
| 4 | `BILL_ITEM_STATUS_CD` | DOUBLE | NOT NULL |  | Status of the bill item (in-progress, dropped, etc.) |
