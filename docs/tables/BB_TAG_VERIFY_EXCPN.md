...
> Contains a row for each product with failed tag verification that is dispensed to patient(by overriding the exception).
...
| 2 | `BB_TAG_VERIFY_EXCPN_ID` | DOUBLE | NOT NULL |  | A system generated number to uniquely identify a row on the BB_TAG_VERIFY_EXCPN table. |
| 3 | `EXCEPTION_ID` | DOUBLE |  | FK→ | Uniquely identifies the related row on the BB_EXCEPTION table. |
| 4 | `TAG_PRODUCT_NBR_TXT` | VARCHAR(20) |  |  | Product number scanned from tag. |
| 5 | `TAG_PRODUCT_SUB_NBR_TXT` | VARCHAR(2) |  |  | Stores the sub number of product scanned from tag |
| 6 | `TAG_PRODUCT_TYPE_CD` | DOUBLE |  |  | Product type scanned from tag |
