...
> Contains details of the SNOMED CT coded clinical findings.
...
| 3 | `CODED_FINDING_DT_TM` | DATETIME |  |  | The date and time that the clinical finding was recorded by a care professional. |
| 4 | `CODED_FINDING_SCT` | VARCHAR(100) |  |  | The SNOMED CT concept ID which is used to identify a finding. |
| 5 | `CODED_FINDING_SEQ` | DOUBLE |  |  | Sequence number of coded clinical finding. |
...
| 10 | `LOC_CODED_FINDING_DT_TM` | DATETIME |  |  | The local date and time that the clinical finding was recorded by a care professional. |
| 11 | `TOTAL_UPDATES` | DOUBLE | NOT NULL |  | The numbers of updates that have occurred on this table. |
| 12 | `UKRWH_CDS_AE_ATTENDANCE_KEY` | DOUBLE | NOT NULL |  | A number allocated by an Accident and Emergency Department to provide a unique identifier for each Accident and Emergency Attendance. |
| 13 | `UKRWH_CDS_CODED_FINDING_KEY` | DOUBLE | NOT NULL |  | This is the value of the unique primary identifier of the UKRWH_CDS_CODED_FINDING table. It is an internal system assigned number. |
...
| 18 | `UPDT_TASK` | VARCHAR(40) | NOT NULL |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
