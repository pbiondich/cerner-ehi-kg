...
|--:|--------|------|:-----:|-------|------------|
| 1 | `RAD_EXPOSURE_ID` | DOUBLE | NOT NULL | PK | This is a unique meaningless number used only for identification of a row on this table. |
| 2 | `SEQUENCE` | DOUBLE |  |  | The sequence field indicates the order of the series within seq_exam_id. |
| 3 | `SEQ_EXAM_ID` | DOUBLE | NOT NULL | FK→ | The sequential_exam_id field is a foreign key to the exam_data table. It uniquely identifies the exam. |
...
| From table | Column |
|------------|--------|
| [RAD_EXPOSURE_DATA](RAD_EXPOSURE_DATA.md) | `RAD_EXPOSURE_ID` |
