...
> Contains information on claim responses from the insurance payer.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `REJECT_CD` | DOUBLE | NOT NULL |  | The NCPDP reject code supplied by the payer when they rejected the claim |
| 2 | `RX_CLAIM_ID` | DOUBLE | NOT NULL | FK→ | The claim that this response pertains to. |
| 3 | `RX_CLAIM_RESPONSE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RX_CLAIM_RESPONSE table. |
...
|--------|--------------|---------------|
| `RX_CLAIM_ID` | [RX_CLAIM](RX_CLAIM.md) | `RX_CLAIM_ID` |
