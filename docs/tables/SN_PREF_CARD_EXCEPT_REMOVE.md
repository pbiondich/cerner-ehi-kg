...
> Defines what items are removed as part of an exception
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTIVE_DT_TM` | DATETIME | NOT NULL |  | Date and Time the item removal exception was made active or inactive |
...
| 4 | `CREATE_DT_TM` | DATETIME | NOT NULL |  | Date and Time the item removal exception was created; |
| 5 | `CREATE_PRSNL_ID` | DOUBLE | NOT NULL | FK→ | User who added the removal of the item for the exception |
| 6 | `ITEM_ID` | DOUBLE | NOT NULL | FK→ | The item being removed for the exception. Key value from the ITEM_DEFINITION table |
| 7 | `SN_PREF_CARD_EXCEPTION_ID` | DOUBLE |  | FK→ | Preference Card Exception the item is for; |
| 8 | `SN_PREF_CARD_EXCEPT_REMOVE_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
