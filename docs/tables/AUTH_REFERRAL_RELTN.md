...
> Stores information about a relationship between an authorization and a referral.

**Description:** Authorization Referral Relationship  
**Table type:** ACTIVITY  
**Primary key:** `AUTH_REFERRAL_RELTN_ID`  
**Columns:** 12  
...
| 6 | `AUTH_REFERRAL_RELTN_ID` | DOUBLE | NOT NULL | PK | A system generated number to uniquely identify a row on the AUTH_REFERRAL_RELTN table. |
| 7 | `REFERRAL_ID` | DOUBLE |  | FK→ | Unique generated number that identifies a single row on the Referral table. |
...
| From table | Column |
|------------|--------|
| [AUTH_REFERRAL_RELTN_HIST](AUTH_REFERRAL_RELTN_HIST.md) | `AUTH_REFERRAL_RELTN_ID` |
