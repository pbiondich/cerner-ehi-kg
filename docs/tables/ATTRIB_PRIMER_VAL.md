...
> Contains Attribute Primer Value Information.
...
| 5 | `ATTRIB_CATALOG_ID` | DOUBLE | NOT NULL | FK→ | Foreign key to the attrib_catalog table. |
| 6 | `ATTRIB_PRIMER_VAL_ID` | DOUBLE | NOT NULL |  | Primary Key |
...
| 8 | `CODE_SET` | DOUBLE |  |  | Code set to which an attribute related to. |
| 9 | `CODE_VALUE` | DOUBLE |  |  | Code value associated to the attribute. |
...
| 11 | `PRIORITY_SEQ` | DOUBLE |  |  | Sequence of related rows |
