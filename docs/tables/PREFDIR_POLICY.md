...
> A grammatical expression constructed using tokens from the context, that describes the process in which preferences should be coalesced or collated. Policies are created within the model and the preference service looks up the policy to determine the coalescing sequence.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ENTRY_ID` | DOUBLE | NOT NULL |  | Entry id that we are setting a policy for. |
