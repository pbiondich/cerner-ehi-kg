...
> Configuration of the surcharge percentages based on the payer's participation status.
...
| 3 | `CREATE_PRSNL_ID` | DOUBLE | NOT NULL | FK→ | The person_id of the person from the personnel table (prsnl) that caused the original insert of the row in the table. |
...
| 6 | `PARTICIPATION_STATUS_CD` | DOUBLE | NOT NULL |  | Codeset which contains participation types to which we are associating surcharge percentages. |
| 7 | `PFT_SURCHARGE_RATE_CONFIG_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the PFT_SURCHARGE_RATE_CONFIG table. |
| 8 | `RATE_PCT` | DOUBLE | NOT NULL |  | Surcharge percentage rates for Payers. |
