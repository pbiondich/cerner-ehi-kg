...
> Determines the order of the display per Person Management Report
...
| 9 | `DESCENDING_IND` | DOUBLE |  |  | Determines whether this order by will be descending |
...
| 11 | `FIELD_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key tying the order by to a specific field on the pm_rpt_field table |
| 12 | `ORDER_ID` | DOUBLE | NOT NULL |  | Primary Key attribute |
| 13 | `ORDER_SEQUENCE` | DOUBLE | NOT NULL |  | Determines the sequence of the fields in the order by for a particular report |
| 14 | `REPORT_ID` | DOUBLE | NOT NULL | FK→ | Foreign Key tying the order by to a particular report on the pm_rpt_report table |
