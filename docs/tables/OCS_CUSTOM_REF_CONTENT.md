...
> Stores the custom reference content for synonyms.
...
| 3 | `DISPLAY_NAME` | VARCHAR(255) |  |  | Name used to display in the applications for the reference content. |
...
| 5 | `OCS_CUSTOM_REF_CONTENT_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the OCS_CUSTOM_REF_CONTENT table. |
| 6 | `REFERENCE_LOCATION` | VARCHAR(2000) |  |  | The url/path of the reference content. |
| 7 | `REFERENCE_TYPE_TFLG` | VARCHAR(16) |  |  | Indicates the type of reference content being stored. EPED - Experience and Evidence based database for children's medication. |
| 8 | `SYNONYM_ID` | DOUBLE |  | FK→ | Identifies the orderable item. Foreign Key from ORDER_CATALOG_SYNONYM table. |
