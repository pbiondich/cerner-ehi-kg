...
> The purpose of the pft_queue_definition table is to store reference data defining the workflow queues.
...
| 8 | `PFT_ENTITY_STATUS_CD` | DOUBLE | NOT NULL |  | The status of the information contained in a workflow queue. |
| 9 | `PFT_ENTITY_TYPE_CD` | DOUBLE | NOT NULL |  | Type of information contained in a workflow queue. |
| 10 | `PFT_QUEUE_DEFINITION_ID` | DOUBLE | NOT NULL |  | Unique identifier for the pft_queue_definition table. |
| 11 | `PFT_QUEUE_EVENT_CD` | DOUBLE | NOT NULL |  | Specifies what the event associated to the queue. |
| 12 | `PFT_QUEUE_EVENT_TYPE_FLAG` | DOUBLE | NOT NULL |  | Flag to store the type of queue event. ENQUEUE = 1, DEQUEUE = 2, QUEUE UPDATE WITH DEQUEUE = 3, QUEUE UPDATE WITH HISTORY ONLY = 4 |
| 13 | `PRIORITY_SEQ` | DOUBLE |  |  | Priority sequence stores the order in which the queue is processed. |
| 14 | `RELEASE_IND` | DOUBLE |  |  | Specifies if releasing an item from the queue is permitted. |
