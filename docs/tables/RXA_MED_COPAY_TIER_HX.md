...
> To add and track copay tier value(s) for a product
...
| 3 | `COPAY_TIER_CD` | DOUBLE | NOT NULL |  | The copay tier that is applicable for the item_id. |
| 4 | `COPAY_TIER_TZ` | DOUBLE | NOT NULL |  | Stores the time zone of copay tier information |
...
| 6 | `ITEM_ID` | DOUBLE | NOT NULL | FK→ | The item that participates in this copay tier. |
| 7 | `RXA_MED_COPAY_TIER_HX_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RXA_MED_COPAY_TIER_HX table. |
