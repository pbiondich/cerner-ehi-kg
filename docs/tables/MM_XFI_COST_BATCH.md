...
> Used to group a set of MM_XFI_COST rows for batch processing.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `BATCH_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Stores the batch_id for a set of transactions |
...
| 3 | `MM_XFI_COST_BATCH_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the MM_XFI_COST_BATCH table. |
