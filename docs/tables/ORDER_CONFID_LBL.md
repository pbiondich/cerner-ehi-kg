...
> Stores confidentiality information for each order.
...
| 7 | `ORDER_CONFID_LBL_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ORDER_CONFID_LBL table. |
| 8 | `ORDER_ID` | DOUBLE |  | FK→ | The ID of the order the confidentiality labels are applied to. |
