...
> Stores equivalent orderables for a case step.
...
| 4 | `CATALOG_CD` | DOUBLE | NOT NULL |  | Stores the orderable procedure that represents the equivalent. |
...
| 6 | `UCMR_CASE_STEP_ID` | DOUBLE | NOT NULL | FK→ | Stores the case step identifier to which this equivalent applies. |
| 7 | `UCMR_EQUIVALENT_STEP_ID` | DOUBLE | NOT NULL |  | Uniquely identifies an equivalent orderable for a case step. |
