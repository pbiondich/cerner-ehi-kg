...
> Stores the relationship between Registration Actions and Conversations.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `RCA_CONVERSATION_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier that identifies a single row on the RCA_CONVERSATION table across domains. |
| 2 | `RCA_REG_ACTION_CONV_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RCA_REG_ACTION_CONV_RELTN. |
| 3 | `RCA_REG_ACTION_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier that identifies a single row on the RCA_REG_ACTION table across domains. |
