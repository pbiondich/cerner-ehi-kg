...
> This table will maintain the dosing weights per person per documentation time that are used during IV drip titration calculations.
...
| 2 | `DOSING_WEIGHT_ID` | DOUBLE | NOT NULL |  | Primary Key from Sequence CARENET_SEQ |
...
| 4 | `EVENT_ID` | DOUBLE | NOT NULL |  | Event_id of the weight used for the person_id at the documentation time (begin_effective_dt_tm) |
| 5 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | The person that the weight (event_id) was used for |
