...
> Order Supply location table stores the location information entered during the pharmacy discharge process.
...
| 2 | `ORDER_SUPPLY_LOCATION_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ORDER_SUPPLY_LOCATION table. |
| 3 | `ORDER_SUPPLY_REVIEW_ID` | DOUBLE | NOT NULL | FK→ | Id that identifies a row in ORDER_SUPPLY_REVIEW table |
| 4 | `PHARMACY_SUPPLY_LOCATION_CD` | DOUBLE | NOT NULL |  | The location from where order will be supplied. |
...
|--------|--------------|---------------|
| `ORDER_SUPPLY_REVIEW_ID` | [ORDER_SUPPLY_REVIEW](ORDER_SUPPLY_REVIEW.md) | `ORDER_SUPPLY_REVIEW_ID` |
