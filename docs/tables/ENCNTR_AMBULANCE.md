...
> Stores information about the ambulance associated to the encounter.

**Description:** Encounter Ambulance  
**Table type:** ACTIVITY  
**Primary key:** `ENCNTR_AMBULANCE_ID`  
**Columns:** 14  
...
| 5 | `COMPANY_NAME` | VARCHAR(100) |  |  | The name of the ambulance company. |
| 6 | `ENCNTR_AMBULANCE_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the ENCNTR_AMBULANCE table. |
| 7 | `ENCNTR_ID` | DOUBLE | NOT NULL | FK→ | The value of the primary identifier of the parent table to which the encounter's ambulance row is related. |
| 8 | `HOME_REGION_CD` | DOUBLE | NOT NULL |  | The information describing the geographical area typically served by the ambulance. |
| 9 | `SERVICE_NUMBER_IDENT` | VARCHAR(100) |  |  | The service number related to the ambulance visit. |
...
| From table | Column |
|------------|--------|
| [ENCNTR_AMBULANCE_HIST](ENCNTR_AMBULANCE_HIST.md) | `ENCNTR_AMBULANCE_ID` |
