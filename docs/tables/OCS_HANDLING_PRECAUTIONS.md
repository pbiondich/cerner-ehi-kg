...
> Table used to store the hazardous information about a medication.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `HANDLING_PRECAUTIONS_LONG_BLOB` | LONGBLOB |  |  | Stores large binary data containing warnings/instructions for nurses regarding the medication they are preparing for the patient. |
| 2 | `OCS_HANDLING_PRECAUTIONS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the OCS_HAZARDOUS_INFO table. |
| 3 | `SYNONYM_ID` | DOUBLE | NOT NULL | FK→ | Identifies the synonym. Foreign Key from ORDER_CATALOG_SYNONYM table. |
