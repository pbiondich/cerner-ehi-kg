...
> This table is used to group a generic set of items.
...
| 3 | `BR_GPRO_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the br_gpro table. |
| 4 | `BR_GPRO_NAME` | VARCHAR(255) |  |  | Name string that identifies the GPRO. |
...
| 12 | `ORIG_BR_GPRO_ID` | DOUBLE | NOT NULL |  | Used for versioning. Contains the original PK from BR_GPRO |
| 13 | `TAX_ID_NBR_TXT` | VARCHAR(50) |  |  | Stores the eligible providers's tax id number. |
...
| 19 | `UPDT_USER` | VARCHAR(40) |  |  | The user that performs the insert or update to the record |
