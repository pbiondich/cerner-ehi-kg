...
> This table contains aggregated claim data by the given dimensions for a given activity date.
...
| 5 | `MONTHLY_CLAIM_EVENT_SMRY_ID` | DOUBLE | NOT NULL |  | Uniquely identifies aggregated claim data by the given dimensions for a given activity date. |
| 6 | `PRIORITY_SEQUENCE` | DOUBLE | NOT NULL |  | Identifies who is responsible for the claim. (i.e. 1 = Primary, 2 = Secondary, 3 = Tertiary) |
| 7 | `RC_D_BILLING_ENTITY_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the billing entity associated to this fact row. |
| 8 | `RC_D_BILL_TYPE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the bill type related to this fact row. |
| 9 | `RC_D_CLAIM_EVENT_TYPE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies a claim event type dimension row related to this fact row. |
| 10 | `RC_D_FINANCIAL_CLASS_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the financial class related to this fact row. |
| 11 | `RC_D_HEALTH_PLAN_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the health plan for which the claim was generated. |
...
| 14 | `TOTAL_BILLED_AMT` | DOUBLE | NOT NULL |  | Represents the total amount billed of all the claims aggregated into a given row for a given month. |
| 15 | `TOTAL_DAYS_SINCE_DISCHARGE` | DOUBLE | NOT NULL |  | Represents the total number of days between discharge and the claim event (submission for example) for all of the claims summarized for a given month. |
| 16 | `TOTAL_DAYS_SINCE_GENERATION` | DOUBLE | NOT NULL |  | Represents the total number of days between generation and the claim event (submission for example) for all of the claims summarized for a given month. |
| 17 | `TOTAL_DAYS_SINCE_SUBMISSION` | DOUBLE | NOT NULL |  | Represents the total number of days between submission and the claim event (transmission for example) for all of the claims summarized for a given month. |
| 18 | `TOTAL_NUMBER_OF_CLAIMS` | DOUBLE | NOT NULL |  | Represents the number of claims that have been aggregated into this row for a given month. |
