...
> Information for electronically prescribed orders.
...
| 2 | `ORDER_ERX_INFO_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ORDER_ERX_INFO table. |
| 3 | `ORDER_ID` | DOUBLE |  | FK→ | ORDER_ID from orders table associated to this prescription. |
