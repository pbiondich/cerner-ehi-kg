...
> Stores data gathered by the LH_E_STEMI_2025 script.
...
| 45 | `EXCEP_IND` | DOUBLE |  |  | Identifies if the encounter is in the Denominator Exceptions for STEMI. |
| 46 | `EXCLUDE_IND` | DOUBLE |  |  | Identifies if the encounter is Excluded for STEMI |
...
| 50 | `FT_ADMIN_DT_TM` | DATETIME |  |  | Identifies the date/time Fibrinolytic therapy medication was administered |
| 51 | `FT_ADMIN_ORD_DETAIL` | VARCHAR(100) |  |  | Order details for qualifying medication order for value set 'FIBRINOLYTIC_THERAPY' |
...
| 54 | `HIC_MBI_TXT` | VARCHAR(50) |  |  | HIC or MBI number of the patient. |
| 55 | `INTRAC_INTRAS_SURG_PROC_DT_TM` | DATETIME |  |  | Identifies the documentation time on procedure for INTRACRANIAL_OR_INTRASPINAL_SURGERY |
| 56 | `INTRAC_INTRAS_SURG_PROC_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code procedure for INTRACRANIAL_OR_INTRASPINAL_SURGERY |
| 57 | `IPP_IND` | DOUBLE |  |  | Indicates the encounter within the initial patient population |
| 58 | `ISCHEMIC_DX_DT_TM` | DATETIME |  |  | Identifies the documentation time on diagnosis for ISCHEMIC_STROKE |
| 59 | `ISCHEMIC_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for ISCHEMIC_STROKE |
| 60 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the last ETL process started that updated this record. |
| 61 | `LH_E_STEMI_2025_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_E_STEMI_2025_METRICS table. |
| 62 | `LOGICAL_DOMAIN_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, If you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
| 63 | `MAJOR_SURG_PROC_DT_TM` | DATETIME |  |  | Identifies the documentation time on procedure for MAJOR_SURGICAL_PROCEDURE |
| 64 | `MAJOR_SURG_PROC_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code procedure for MAJOR_SURGICAL_PROCEDURE |
| 65 | `MALIGNANT_INTRA_DX_DT_TM` | DATETIME |  |  | Identifies the documentation time on diagnosis for MALIGNANT_INTRACRANIAL_NEOPLASM_GROUP |
| 66 | `MALIGNANT_INTRA_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for MALIGNANT_INTRACRANIAL_NEOPLASM_GROUP |
| 67 | `MECH_CIRC_DEVICE_PROC_DT_TM` | DATETIME |  |  | Identifies the documentation time on procedure for MECHANICAL_CIRCULATORY_ASSIST_DEVICE |
| 68 | `MECH_CIRC_DEVICE_PROC_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code procedure for MECHANICAL_CIRCULATORY_ASSIST_DEVICE |
| 69 | `NEURO_IMP_DX_DT_TM` | DATETIME |  |  | Identifies the documentation time on diagnosis for NEUROLOGIC_IMPAIRMENT |
| 70 | `NEURO_IMP_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for NEUROLOGIC_IMPAIRMENT |
| 71 | `NUM_IND` | DOUBLE |  |  | Identifies if the encounter is in the Numerator for STEMI |
| 72 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Medical Record Number of the record. |
| 73 | `PARTITION_DT_TM` | DATETIME |  |  | The date/time the encounter was discharged from the facility. |
| 74 | `PATIENT_IDENT` | VARCHAR(50) |  |  | Identifies the earliest documented medical record number of the patient. |
| 75 | `PAT_EXPIRED_IND` | DOUBLE |  |  | Indicates if patient exipred |
| 76 | `PERC_CORO_PROC_DT_TM` | DATETIME |  |  | Identifies the documentation time on procedure for PERCUTANEOUS_CORONARY_INTERVENTION |
| 77 | `PERC_CORO_PROC_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code procedure for PERCUTANEOUS_CORONARY_INTERVENTION |
...
| 83 | `PREGNANT_STATE_DX_DT_TM` | DATETIME |  |  | Identifies the documentation time on diagnosis for PREGNANT_STATE. |
| 84 | `PREGNANT_STATE_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for PREGNANT_STATE |
| 85 | `PRIOR_TO_ADMIT_TPA_DX_DT_TM` | DATETIME |  |  | Identifies the documentation date time diagnosis for PRIOR_TO_ADMISSION_TPA. |
| 86 | `PRIOR_TO_ADMIT_TPA_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for PRIOR_TO_ADMISSION_TPA. |
...
| 90 | `STEMI_DX_DT_TM` | DATETIME |  |  | Identifies the documentation time on diagnosis for STEMI |
| 91 | `STEMI_DX_NOMEN` | VARCHAR(50) |  |  | Identifies the documentation code diagnosis for STEMI |
| 92 | `THROM_MED_ADV_EVENT_IND` | DOUBLE |  |  | Identifies if there was any adverse reaction to Allergy for not giving Thrombolytic meds |
| 93 | `THROM_MED_ALLERGY_IND` | DOUBLE |  |  | Identifies if there was a Allergy Reason for not giving Thrombolytic meds |
