...
> Stores reference data defining which Document Template to use based on various criteria.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `EXT_DOC_TEMPLATE_CONFIG_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
...
| 3 | `PERSONA_CD` | DOUBLE | NOT NULL |  | The type of person viewing the document (e.g. PATIENT, PROVIDER). |
| 4 | `TEMPLATE_ID` | DOUBLE | NOT NULL | FK→ | The ID of the template to be used for this configuration. Foreign key from CR_REPORT_TEMPLATE. |
| 5 | `TYPE_CD` | DOUBLE | NOT NULL |  | The type of document being viewed (e.g. PATHOLOGY, RADIOLOGY). |
