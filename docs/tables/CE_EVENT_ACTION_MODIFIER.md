...
> Modifier table to the ce_event_prsnl table. This would contain extensions of action_type_cd from ce_event_prsnl table
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTION_TYPE_MODIFIER_CD` | DOUBLE | NOT NULL |  | CODE SET: 254550 Extension or modification of type of action for an event prsnl. For example, an action_type of Convey can have an action_modifier_type of email, phone mail etc. |
| 2 | `CE_EVENT_ACTION_MODIFIER_ID` | DOUBLE | NOT NULL |  | SEQUENCE NAME:OCF_SEQ Primary Key for the table. |
| 3 | `EVENT_ACTION_MODIFIER_ID` | DOUBLE | NOT NULL |  | SEQUENCE NAME:OCF_SEQ Uniquely identifies a logical event action modifier row. There may be more than one row with the same event_action_modifier_id, but only one of those rows will be current as indicated by the valid_until_dt_tm field. |
| 4 | `EVENT_ID` | DOUBLE | NOT NULL |  | Identifier to a logical event row in clinical_event table. |
| 5 | `EVENT_PRSNL_ID` | DOUBLE | NOT NULL |  | Identifier to a logical event prsnl row in ce_event_prsnl table. |
...
| 11 | `VALID_FROM_DT_TM` | DATETIME | NOT NULL |  | Creation date for row. Contains the "Beginning Point" of when a row in the table is valid. |
| 12 | `VALID_UNTIL_DT_TM` | DATETIME | NOT NULL |  | Contains the "End Point" of when a row in the table is valid.This indicates when |
