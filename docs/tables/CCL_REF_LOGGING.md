...
> used to log refs to key system tables
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `REF_COMMAND` | VARCHAR(2000) |  |  | command used in the reference to the table being tracked. |
| 2 | `REF_ID` | DOUBLE | NOT NULL |  | sequence of the audit record |
| 3 | `REF_PRCNAME` | VARCHAR(30) |  |  | procedure that caused the audit record |
| 4 | `REF_RDBMSNAME` | VARCHAR(30) |  |  | database name |
| 5 | `REF_TABLE` | VARCHAR(31) |  |  | affected table |
| 6 | `REF_TYPE` | CHAR(1) |  |  | type of access to affected table ( read, write, update, delete, insert ) |
| 7 | `REF_USERNAME` | VARCHAR(30) |  |  | user that caused audit record |
