...
> Table stores relationship between the source and the concept
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CONCEPT_TXT` | VARCHAR(255) | NOT NULL |  | concept text which represents ALLERGY, PROBLEMS etc., |
| 2 | `PIE_EXT_SOURCE_CONCEPT_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 3 | `PIE_EXT_SOURCE_ID` | DOUBLE | NOT NULL | FK→ | External Source Identifier |
| 4 | `TRUST_TXT` | VARCHAR(255) | NOT NULL |  | trust text which represents one of the value DIRECT_WRITE, STAGE or IGNORE |
