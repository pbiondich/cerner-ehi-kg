...
> This table stores the facility code relation between Clinical_Validation and Code_Value.
...
| 3 | `CLINICAL_VALIDATION_FAC_R_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the Clinical_Validation_Fac_R table. |
| 4 | `CLINICAL_VALIDATION_ID` | DOUBLE | NOT NULL | FK→ | Foreign key from the Clinical_vallidation table. Relates a facility code to a clinical validation. |
...
| 6 | `FACILITY_CD` | DOUBLE | NOT NULL |  | The Facility associated with this row. A zero value indicates all facilities. |
