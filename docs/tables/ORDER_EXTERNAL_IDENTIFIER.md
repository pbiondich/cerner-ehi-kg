...
> External identifiers of orders.
...
| 4 | `ORDER_EXTERNAL_IDENTIFIER_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the ORDER_EXTERNAL_IDENTIFIER table. |
| 5 | `ORDER_ID` | DOUBLE |  | FK→ | The order_id from Orders table. |
