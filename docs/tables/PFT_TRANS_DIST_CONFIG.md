...
> Stores distribution methods related to billing entity and account type (Client/Selfpay)
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACCOUNT_TYPE_FLAG` | DOUBLE | NOT NULL |  | Account type to which this transaction distribution applies. (Selfpay/Client) |
...
| 7 | `BILLING_ENTITY_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the related row on the BILLING_ENTITY table. |
| 8 | `CREATE_DT_TM` | DATETIME |  |  | Date and Time the row was created. |
| 9 | `CREATE_PRSNL_ID` | DOUBLE |  | FK→ | the person_id of the person from the personnel table (prsnl) that created the row in the table. |
| 10 | `DISTRIBUTION_METHOD_CD` | DOUBLE | NOT NULL |  | Transaction distirbution method from codeset 4064000. |
...
| 13 | `PFT_TRANS_DIST_CONFIG_ID` | DOUBLE | NOT NULL |  | System generated number to uniquely identify a row on the PFT_TRANS_DIST_CONFIG table. |
