...
> Store list of person_id for each EHI request
...
| 5 | `EHI_QUEUE_ID` | DOUBLE | NOT NULL | FK→ | Unique identifier for an EHI Queue |
| 6 | `EHI_QUEUE_PERSON_RELTN_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY - Unique generated number that identifies a single row |
| 7 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | Unique identifier for a person |
