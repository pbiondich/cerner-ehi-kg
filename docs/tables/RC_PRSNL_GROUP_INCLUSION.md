...
> Personnel members of a personnel group that are included from personnel preferential agreements.
...
| 2 | `PRSNL_GROUP_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the personnel group from which the personnel is being included. |
| 3 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies the personnel that is being included from the personnel group. |
| 4 | `RC_PRSNL_GROUP_INCLUSION_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RC_PRSNL_GROUP_INCLUSION table. |
