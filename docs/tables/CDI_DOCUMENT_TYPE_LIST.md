...
> Stores document type selection applied to a position or user.
...
| 5 | `CDI_DOCUMENT_TYPE_LIST_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the CDI_DOCUMENT_TYPE_LIST table. |
| 6 | `CODE_SET` | DOUBLE | NOT NULL |  | Specifies which code set the EVENT_CD field is from. Can be from code set 72 (clinical document types) or 26820 (non-clinical document types). |
| 7 | `EVENT_CD` | DOUBLE | NOT NULL |  | Code value of the document type for this row. Can be from code set 72 (clinical document types) or 26820 (non-clinical document types). associated code set is stored in column code_set. |
| 8 | `POSITION_CD` | DOUBLE | NOT NULL |  | Position code to which this list belongs. Examples are DBA, ADMIN, PATIENT. 0 value indicates list belongs to prsnl identified by PRSNL_ID. |
| 9 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | Provider to which this list belongs. 0 value indicates list belongs to the position identified by POSITION_CD. |
