...
> Identifies a Relationship between an organization and a service set.
...
| 2 | `FACILITY_ORG_ID` | DOUBLE | NOT NULL | FK→ | The organization that is being associated to the service set. |
| 3 | `RC_ORG_SERVICE_SET_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RC_ORG_SERVICE_SET_RELTN table. |
| 4 | `RC_SERVICE_SET_ID` | DOUBLE | NOT NULL | FK→ | The service set that is associated to this relationship. |
