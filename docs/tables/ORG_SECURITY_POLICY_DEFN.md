...
> Stores the policy definition defining organization access control for different concepts.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CONCEPT_IDENT` | VARCHAR(100) | NOT NULL |  | Indentifier used to uniquely identify the concept. |
| 2 | `HONOR_SECURITY_IND` | DOUBLE | NOT NULL |  | Determines whether the policy definition should be honored or not. If it is set to 1, the policies within the definition will be honored. If it is set to 0, the policies within the definition will not be honored. |
| 3 | `ORG_SECURITY_POLICY_DEFN_ID` | DOUBLE | NOT NULL |  | A system generated number used to uniquely identify a row on the ORG_SECURITY_POLICY_DEFN table. |
| 4 | `POLICY_DEFINITION_TXT` | VARCHAR(4000) | NOT NULL |  | The policy definition for the concept defining organization access control. |
