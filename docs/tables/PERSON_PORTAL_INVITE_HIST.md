...
> Used for tracking history changes to PERSON_PORTAL_INVITE rows.
...
| 8 | `ERROR_REASON_CD` | DOUBLE | NOT NULL |  | Information describing the reason that the attempt to send an invitation to the person was not successful. |
| 9 | `INVITE_STATUS_CD` | DOUBLE | NOT NULL |  | The status of the person's portal invitation (i.e., sent, sending, error, self-enrolled) |
| 10 | `PATIENT_PERSON_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the person table. It is an internal system assigned number. |
| 11 | `PERSON_PORTAL_INVITE_HIST_ID` | DOUBLE | NOT NULL |  | Uniquely Identifies a row on the PERSON_PORTAL_INVITE_HIST table. |
| 12 | `PERSON_PORTAL_INVITE_ID` | DOUBLE | NOT NULL | FK→ | Uniquely identifies a related row on the PERSON_PORTAL_INVITE table. |
...
| `PERSON_PORTAL_INVITE_ID` | [PERSON_PORTAL_INVITE](PERSON_PORTAL_INVITE.md) | `PERSON_PORTAL_INVITE_ID` |
