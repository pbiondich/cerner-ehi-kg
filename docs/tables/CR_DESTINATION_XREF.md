...
> This table lists cross reference between an Entity PRSNL, ORGANIZATION, SERVICE_RESOUCE and LOCATION) and Devices specific to Clinical Reporting workflows
...
| 5 | `CR_DESTINATION_XREF_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 6 | `DESTINATION_TYPE_CD` | DOUBLE | NOT NULL |  | Destination type specific to device and dms_service_identifier FROM CODE SET: 4636013 |
| 7 | `DEVICE_CD` | DOUBLE | NOT NULL | FK→ | Foreign Key value from DEVICE table |
| 8 | `DMS_SERVICE_IDENTIFIER` | VARCHAR(110) |  |  | Indicates ( identifies ) the DMS service |
| 9 | `PARENT_ENTITY_ID` | DOUBLE | NOT NULL |  | Parent entity id for destination type. A PK value from table identified in PARENT_ENTITY_NAME |
| 10 | `PARENT_ENTITY_NAME` | VARCHAR(30) | NOT NULL |  | name of the destination type parent table entity |
