...
> Table containing the qualifying encounters for Meaningful Use NQF Stage 2 2022.

**Description:** LH_AMB_QUAL_ENCNTR_2022  
**Table type:** ACTIVITY  
**Primary key:** `HEALTH_SYSTEM_SOURCE_ID`, `LH_AMB_QUAL_ENCNTR_2022_ID`  
**Columns:** 62  
...
| 2 | `BR_DATAM_VSET_ITEM_ID` | DOUBLE |  |  | References the id in BR_DATAM_VSET_ITEM. |
| 3 | `BR_DATAM_VSET_ITEM_NEG_ID` | DOUBLE |  |  | References the negation in BR_DATAM_VSET_ITEM. |
...
| 7 | `D_PERSON_ID` | DOUBLE |  | FK→ | Identifies the person associated with the quality measure. Foreign key to the LH_D_PERSON table. |
| 8 | `D_QUERY_ID` | DOUBLE |  | FK→ | Identifies the query associated with the quality measure. Foreign key to the LH_D_QUERY table. |
| 9 | `ENCNTR_ID` | DOUBLE |  |  | Identifies the encounter against which the quality measure is associated. |
...
| 15 | `HEALTH_PLAN_ID` | DOUBLE |  |  | The identifier for the patient¿s health plan |
| 16 | `HEALTH_SYSTEM_ID` | DOUBLE |  |  | Identifies the delivery network responsible for supplying the data. |
| 17 | `HEALTH_SYSTEM_SOURCE_ID` | DOUBLE | NOT NULL | PK FK→ | Identifies the unique source within the delivery network responsible for supplying the data. |
...
| 20 | `LH_AMB_QUAL_ENCNTR_2022_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the LH_AMB_QUAL_ENCNTR_2022 table. |
| 21 | `LOC_NURSE_UNIT_CD` | DOUBLE |  |  | The code value corresponding to the nursing unit of the encounter. |
| 22 | `LOGICAL_DOMAIN_ID` | DOUBLE |  |  | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, If you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
| 23 | `NEGATION_IND` | DOUBLE |  |  | Indicates whether the given result code is a negation or not (QRDA) |
| 24 | `ORGANIZATION_ID` | DOUBLE |  |  | References the id in ORGANIZATION. |
...
| 35 | `PERSON_ID` | DOUBLE |  |  | Identifies the person against which the qualify measure is associated. |
...
| 39 | `PERSON_RACE_CODE_DISPLAY` | VARCHAR(500) |  |  | The textual representation of the patient's race |
...
| From table | Column |
|------------|--------|
| [LH_AMB_EP_RELTN_2022](LH_AMB_EP_RELTN_2022.md) | `HEALTH_SYSTEM_SOURCE_ID` |
| [LH_AMB_EP_RELTN_2022](LH_AMB_EP_RELTN_2022.md) | `LH_AMB_QUAL_ENCNTR_2022_ID` |
