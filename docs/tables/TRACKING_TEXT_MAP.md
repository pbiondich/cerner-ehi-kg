...
> Provides temporary mapping of unique string/date combination to long_text or long_blob row.

**Description:** Tracking Text Map  
**Table type:** ACTIVITY  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `LONG_TEXT_ID` | DOUBLE | NOT NULL | FK→ | ID of the row in LONG_TEXT table. |
| 2 | `TEXT_DT_TM` | DATETIME |  |  | used to map a date to a long_text or long_blob row |
| 3 | `TEXT_KEY` | VARCHAR(100) |  |  | used to map a string to a long_text or long_blob row |
| 4 | `TRACKING_TEXT_MAP_ID` | DOUBLE | NOT NULL |  | Primary Key |
