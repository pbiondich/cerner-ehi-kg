...
> This table relates a document to its history (as known by Millennium) of document identifiers.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `DOCUMENT_IDENT` | VARCHAR(100) | NOT NULL |  | The identifier of the document which is unique within the context of the system. |
| 2 | `SI_DOCUMENT_IDENT_RELTN_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 3 | `SI_DOCUMENT_METADATA_ID` | DOUBLE | NOT NULL | FK→ | The document metadata on the SI_DOCUMENT_METADATA table. |
| 4 | `SYSTEM_TXT` | VARCHAR(100) |  |  | The URI namespace for the identifier value. |
...
|--------|--------------|---------------|
| `SI_DOCUMENT_METADATA_ID` | [SI_DOCUMENT_METADATA](SI_DOCUMENT_METADATA.md) | `SI_DOCUMENT_METADATA_ID` |
