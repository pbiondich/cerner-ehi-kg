...
> This table keeps track of CPC's and PCF's who have a need to map to a location already associated to another CPC or PCF.
...
| 3 | `BR_CPC_ID` | DOUBLE |  | FK→ | The CPC group that is associated to this location. |
| 4 | `BR_CPC_LOC_RELTN_ID` | DOUBLE |  |  | Unique, generated number that identifies a single row on the BR_CPC_LOC_RELTN table. |
...
| 6 | `LH_BR_CPC_LOC_RELTN_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_BR_CPC_LOC_RELTN table. |
| 7 | `LOCATION_CD` | DOUBLE |  |  | The code for the location that needs to be associated to the CPC or PCF listed in BR_CPC_ID. |
| 8 | `ORIG_BR_CPC_LOC_RELTN_ID` | DOUBLE |  |  | The original id for the row. |
...
| 12 | `UPDT_SOURCE` | VARCHAR(50) |  |  | The script name responsible for updating the record. |
