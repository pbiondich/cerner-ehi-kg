...
> Holds information about Validate study job parameters
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `CPS_JP_CAMM_VALIDATESTUDY_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 2 | `CPS_JQ_CAMM_VALIDATESTUDY_ID` | DOUBLE | NOT NULL |  | Unique number that identifies a single row on the CPS_JQ_VALIDATESTUDY table. |
| 3 | `PARAM_NAME` | VARCHAR(200) | NOT NULL |  | parameter name for the CPS_JQ_VALIDATESTUDY job. |
| 4 | `PARAM_VALUE` | VARCHAR(4000) | NOT NULL |  | value of the corresponding CPS_JQ_VALIDATESTUDY parameter. |
| 5 | `SEQUENCE_NBR` | DOUBLE | NOT NULL |  | Number that specifies the order of the parameter for the corresponding CPS_JQ_VALIDATESTUDY job. |
