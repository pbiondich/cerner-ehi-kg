...
| 5 | `BATCH_IND` | DOUBLE |  |  | Flag to determine whether this destination will print to the PM batch document que (PM_DOC_BATCH_PRINT table) |
...
| 7 | `COPIES` | DOUBLE |  |  | Number of copies of the document to print to this destination |
| 8 | `DESTINATION_ID` | DOUBLE | NOT NULL |  | Primary key (sequence generated). |
| 9 | `DISTRIBUTION_ID` | DOUBLE | NOT NULL | FK→ | Foreign key attribute associating the destination with a specific distribution on the pm_doc_distribution table |
| 10 | `DOCUMENT_ID` | DOUBLE | NOT NULL | FK→ | Foreign key attribute associating the destination to a specific document on the pm_doc_document table. |
...
| 12 | `OUTPUT_DEST_CD` | DOUBLE |  |  | The output destination (printer) for this destination. Can be multiple Code Sets. |
...
| `DOCUMENT_ID` | [PM_DOC_DOCUMENT](PM_DOC_DOCUMENT.md) | `DOCUMENT_ID` |
