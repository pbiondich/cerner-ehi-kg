...
> This table is used to store IMM metrics from the Lighthouse Abstractor (eQuality Check). This table is at the encounter grain.

**Description:** LH_ABS_IMM_METRICS  
**Table type:** ACTIVITY  
...
| 4 | `ADM_AGE` | DOUBLE |  |  | The patient's age on admission. |
...
| 9 | `DISC_DISP_FLAG` | DOUBLE |  |  | The patient's discharge disposition on the day of discharge. |
...
| 16 | `D_BR_CCN_ID` | DOUBLE | NOT NULL | FK→ | CMS Certification Number. |
| 17 | `D_BR_HCO_ID` | DOUBLE | NOT NULL | FK→ | Healthcare organization Number. |
...
| 21 | `D_DISCH_DISP_ID` | DOUBLE | NOT NULL | FK→ | The conditions under which the patient left the facility at the time of discharge. |
...
| 25 | `D_METRIC_2_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric |
| 26 | `D_METRIC_3_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric |
| 27 | `D_METRIC_4_ID` | DOUBLE | NOT NULL | FK→ | Identifies the metric identifier for the Lighthouse metric |
...
| 30 | `EXCLUDE_1_IND` | DOUBLE |  |  | Identifies patients excluded from IMM-1 |
| 31 | `EXCLUDE_2_IND` | DOUBLE |  |  | Identifies patients excluded from IMM-2 |
| 32 | `EXTRACT_DT_TM` | DATETIME |  |  | The date/time that the record was extracted from the source system. |
| 33 | `FINANCIAL_NBR_TXT` | VARCHAR(50) |  |  | The financial number alias associated to the encounter. |
| 34 | `FIRST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the first ETL process started that created this record. |
| 35 | `FLUVACSTATUS_FLAG` | DOUBLE |  |  | Identifies the patient's influenza vaccination status |
...
| 38 | `IMM1A_IND` | DOUBLE |  |  | Identifies if the patient qualifies for IMM-1 Overall Measure (age >= 6 years ) |
| 39 | `IMM1B_IND` | DOUBLE |  |  | Identifies if the patient qualifies for IMM-1 Age 65 and older |
| 40 | `IMM1C_IND` | DOUBLE |  |  | Identifies if the patient qualifies for IMM-1 Age 6 through 64 years |
| 41 | `IMM_2_REASON_TXT` | VARCHAR(250) |  |  | Stores the category reason for IMM-2 obtained during the completion of abstraction. |
| 42 | `LAST_PROCESS_DT_TM` | DATETIME |  |  | The date/time that the last ETL process started that updated this record. |
| 43 | `LH_ABS_IMM_METRICS_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_ABS_IMM_METRICS table. |
...
| 45 | `NUMERATOR_2_IND` | DOUBLE |  |  | Identifies patients that qualify for IMM-2 |
| 46 | `ORG_MRN_TXT` | VARCHAR(50) |  |  | Identifies the medical record number of the patient. |
| 47 | `OTH_DIAGNOSIS_LIST` | VARCHAR(500) |  |  | A comma separated list of other diagnoses associated with the encounter. |
| 48 | `OTH_PROCEDURE_DATES_LIST` | VARCHAR(500) |  |  | A comma separated list of dates the other procedures associated with the encounter were performed. |
| 49 | `OTH_PROCEDURE_LIST` | VARCHAR(500) |  |  | A comma separated list of other procedures associated with the encounter. |
...
| 54 | `PNEVACSTATUS_FLAG` | DOUBLE |  |  | Identifies the patient's pneumococcal vaccine status |
...
| 61 | `PSYCHSETNG_FLAG` | DOUBLE |  |  | identifies if there is a response for Psychiatric Care Setting. 0 = NO 1 = YES 999 = MISSING |
| 62 | `REJECT_1_IND` | DOUBLE |  |  | Identifies patients reject for IMM-1 |
| 63 | `REJECT_2_IND` | DOUBLE |  |  | Identifies patients reject for IMM-2 |
...
| 68 | `UPDT_SOURCE` | VARCHAR(50) | NOT NULL |  | The script name responsible for updating the record. |
| 69 | `UPDT_TASK` | VARCHAR(50) | NOT NULL |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
