...
> This table is used for activity monitoring of HLA application.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `HLA_ORDER_LOCK_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the HLA_ORDER_LOCK table. |
| 2 | `ILOCK_IND` | DOUBLE | NOT NULL |  | Indicates if the patient order is locked or not. 1 - locked, 2 - not looked. |
| 3 | `PERSON_ID` | DOUBLE | NOT NULL | FK→ | Person whose order is locked. This infromation will be obtained from the script. |
| 4 | `PERSON_LOCK_DT_TM` | DATETIME |  |  | The date and time the technician locked the person order. |
| 5 | `TECH_LOCK_ID` | DOUBLE | NOT NULL | FK→ | The technician who has locked the person order. |
...
| `TECH_LOCK_ID` | [PRSNL](PRSNL.md) | `PERSON_ID` |
