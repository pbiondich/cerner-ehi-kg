...
> Table containing event information for meaningful use NQF stage 2 2016
...
| 7 | `EXTRACT_DT_TM` | DATETIME |  |  | The time in which the nqf load script that updated this record was run |
...
| 12 | `LH_AMB_EVENT_DATA_2016_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the LH_AMB_EVENT_DATA_2016 table. |
...
| 17 | `POP_REG_DT_TM` | DATETIME |  |  | The date of the qualifying encounter that corresponds to this record. |
...
| 23 | `QUAL_REG_DT_TM` | DATETIME |  |  | Indicates the date of a candidate for deletion. |
