...
> Used for tracking history of columns on the ENCNTR_SPAN_CODE table
...
| 7 | `DATA_STATUS_CD` | DOUBLE |  |  | Data status indicates a level of authenticity of the row data. Typically this will either be AUTHENTICATED or UNAUTHENTICATED. |
| 8 | `DATA_STATUS_DT_TM` | DATETIME |  |  | The date and time that the data_status_cd was set. |
| 9 | `DATA_STATUS_PRSNL_ID` | DOUBLE |  |  | The person who caused the data_status_cd to be set or change. |
| 10 | `ENCNTR_SPAN_CODE_HIST_ID` | DOUBLE | NOT NULL |  | A system generated number used to uniquely identify a row on the ENCNTR_SPAN_CODE_HIST table. |
...
| 12 | `PM_HIST_TRACKING_ID` | DOUBLE |  | FK→ | This is the value of the unique primary identifier of the pm_hist_tracking table. It is an internal system assigned number. |
| 13 | `SPAN_CD` | DOUBLE |  |  | a ub92 span code (universal billing) |
| 14 | `SPAN_CODE_ID` | DOUBLE |  | FK→ | this is the value of the unique primary identifier of the encounter span code table. it is an internal system assigned number. |
| 15 | `SPAN_FROM_DT_TM` | DATETIME |  |  | Date and time the UB92 code span begins. |
| 16 | `SPAN_TO_DT_TM` | DATETIME |  |  | Date and time to which the UB92 code spans. |
| 17 | `UPDT_APPLCTX` | DOUBLE |  |  | The application context number from the record info block. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
...
| 20 | `UPDT_ID` | DOUBLE |  |  | The person_id of the person from the personnel table (prsnl) that caused the last insert or update of the row in the table. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
...
| `SPAN_CODE_ID` | [ENCNTR_SPAN_CODE](ENCNTR_SPAN_CODE.md) | `SPAN_CODE_ID` |
