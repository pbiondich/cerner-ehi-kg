# PPR_CONSENT_POLICY

> This table contains the consent policy which could be tied to an organization to the system.

**Description:** PPR Consent Policy  
**Table type:** REFERENCE  
**Primary key:** `CONSENT_POLICY_ID`  
**Columns:** 23  
**Referenced by:** 2 columns

[← index](../index.md)

## Columns

| # | Column | Type | Null? | Flags | Definition |
|--:|--------|------|:-----:|-------|------------|
| 1 | `ACTIVE_IND` | DOUBLE | NOT NULL |  | The table row is active or inactive. A row is generally active unless it is in an inactive state such as logically deleted, combined away, pending purge, etc. |
| 2 | `ACTIVE_STATUS_CD` | DOUBLE | NOT NULL |  | Indicates the status of the row itself (not the data in the row) such as active, inactive, combined away, pending purge, etc. |
| 3 | `ACTIVE_STATUS_DT_TM` | DATETIME | NOT NULL |  | The date and time that the active_status_cd was set. |
| 4 | `ACTIVE_STATUS_PRSNL_ID` | DOUBLE | NOT NULL |  | The person who caused the active_status_cd to be set or change. |
| 5 | `BEG_EFFECTIVE_DT_TM` | DATETIME |  |  | The date and time for which this table row becomes effective. Normally, this will be the date and time the row is added, but could be a past or future date and time. |
| 6 | `BLOB_REF_SCAN_ID` | DOUBLE | NOT NULL | FK→ | Blob reference id for a scanned in documents. |
| 7 | `COMMENT_TXT` | VARCHAR(250) |  |  | Contains any comments about the policy. |
| 8 | `CONSENT_POLICY_ID` | DOUBLE | NOT NULL | PK | The Consent Policy Id that is maintained for all the minor versions. The unique primary key of this table. |
| 9 | `CONSENT_TYPE_CD` | DOUBLE | NOT NULL |  | Contains the code for the type of consent. An example would be privacy, record access, consent to treat, etc. |
| 10 | `END_EFFECTIVE_DT_TM` | DATETIME |  |  | The date/time after which the row is no longer valid as active current data. This may be valued with the date that the row became inactive. |
| 11 | `EXTERNAL_ORG_CD` | DOUBLE | NOT NULL |  | This code identifies the external organization related to this policy. |
| 12 | `LOGICAL_DOMAIN_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for a logical domain. This identifier allows the data to be grouped by logical domain. For example, If you assign clients a logical_domain_id this would allow you to store data for multiple clients on this table. |
| 13 | `NAME_CD` | DOUBLE | NOT NULL |  | The name of the policy. |
| 14 | `ORGANIZATION_ID` | DOUBLE | NOT NULL | FK→ | This is the value of the unique primary identifier of the organization table. It is an internal system assigned number. |
| 15 | `POLICY_ID` | DOUBLE | NOT NULL |  | The Primary Key from the sequence Patient_privacy_seq |
| 16 | `POLICY_MODEL_FLAG` | DOUBLE | NOT NULL |  | Identifies the policy model; 0 - None; 1 - Pass-Through; 2 - Opt-In; 3 - Opt-Out |
| 17 | `POLICY_TEXT_ID` | DOUBLE | NOT NULL | FK→ | The Long Blob ID where the Policy Text is stored. |
| 18 | `UPDT_APPLCTX` | DOUBLE | NOT NULL |  | The application context number from the record info block. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 19 | `UPDT_CNT` | DOUBLE | NOT NULL |  | Set to 0 on insert. Incremented by 1 on update. Used to recognize update conflict where data in a row updated by one application is at risk of being lost by a second application attempting to update the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 20 | `UPDT_DT_TM` | DATETIME | NOT NULL |  | The date and time the row was last inserted or updated. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 21 | `UPDT_ID` | DOUBLE | NOT NULL |  | The person_id of the person from the personnel table (prsnl) that caused the last insert or update of the row in the table. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 22 | `UPDT_TASK` | DOUBLE | NOT NULL |  | The registered (assigned) task number for the process that inserted or updated the row. The UPDT family of columns are typically used for housekeeping and external system process and should never be depended on for solution specific logic. |
| 23 | `VERSION_NBR` | DOUBLE | NOT NULL |  | Version Number for the policy updated automatically by the system, Read only by the application. |

_Flags: PK = primary key · FK→ = published foreign key (see Joins out)._

## Joins out — this table references (published FKs)

| Column | → References | Parent column |
|--------|--------------|---------------|
| `BLOB_REF_SCAN_ID` | [BLOB_REFERENCE](BLOB_REFERENCE.md) | `BLOB_REF_ID` |
| `LOGICAL_DOMAIN_ID` | [LOGICAL_DOMAIN](LOGICAL_DOMAIN.md) | `LOGICAL_DOMAIN_ID` |
| `ORGANIZATION_ID` | [ORGANIZATION](ORGANIZATION.md) | `ORGANIZATION_ID` |
| `POLICY_TEXT_ID` | [LONG_TEXT_REFERENCE](LONG_TEXT_REFERENCE.md) | `LONG_TEXT_ID` |

## Referenced by (2)

| From table | Column |
|------------|--------|
| [ORG_CONTRIBUTOR_SYS_RELTN](ORG_CONTRIBUTOR_SYS_RELTN.md) | `CONSENT_POLICY_ID` |
| [PPR_CONSENT_STATUS](PPR_CONSENT_STATUS.md) | `CONSENT_POLICY_ID` |

