...
> Gather data for Bedrock reports
...
| 2 | `BR_REPORT_HISTORY_ID` | DOUBLE | NOT NULL |  | Unique id from br_report_history_table |
| 3 | `BR_REPORT_STATISTICS_ID` | DOUBLE | NOT NULL |  | Unique is for the table |
| 4 | `QUALIFYING_ITEMS` | DOUBLE |  |  | Number to indicate how much of data is good |
| 5 | `STATISTIC_MEANING` | VARCHAR(25) |  |  | Unique meaning for the type of statistic |
| 6 | `STATUS_FLAG` | DOUBLE | NOT NULL |  | Number to indicate if data is good, incomplete,etc.. |
| 7 | `TOTAL_ITEMS` | DOUBLE |  |  | Total number of data items being reviewed |
