...
> This table is used to store the history of the LH_CNT_WL_FACTOR_STATUS table
...
| 3 | `FACTOR_STATUS_CD` | DOUBLE | NOT NULL |  | CODE SET: 4003478. The factor status value |
| 4 | `FACTOR_STATUS_END_DT_TM` | DATETIME |  |  | The date time at which this status will no longer be active |
| 5 | `LH_CNT_WL_FACTOR_ID` | DOUBLE | NOT NULL |  | Foreign key to LH_CNT_WL_FACTOR |
| 6 | `LH_CNT_WL_FACTOR_STATUS_H_ID` | DOUBLE | NOT NULL |  | PRIMARY KEY |
| 7 | `LH_CNT_WL_FACTOR_STATUS_ID` | DOUBLE | NOT NULL |  | FK Value from the LH_CNT_WL_FACTOR_STATUS table |
