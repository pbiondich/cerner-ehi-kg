...
> This table will store the external operations performed on the Financial Accounts model

**Description:** Account Management Financial Accounts Operation Log  
**Table type:** ACTIVITY  
**Primary key:** `AM_FA_OPERATION_LOG_ID`  
**Columns:** 11  
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `AM_FA_OPERATION_LOG_ID` | DOUBLE | NOT NULL | PK | Unique generated number that identifies a single row on the AM_FA_OPERATION_LOG. |
...
| 3 | `OPERATION_DT_TM` | DATETIME |  |  | This represents when the operation occurred. |
| 4 | `OPERATION_IDENT` | VARCHAR(30) | NOT NULL |  | The external identifier for operation being applied to Financial Accounts model. |
| 5 | `OPERATION_TYPE_ENUM` | DOUBLE | NOT NULL |  | Operation type of the current operation. |
| 6 | `PARENT_AM_FA_OPERATION_LOG_ID` | DOUBLE |  | FK→ | Identifies the parent AM_FA_OPERATION_LOG_ID to which the operation is associated. |
...
| `PARENT_AM_FA_OPERATION_LOG_ID` | [AM_FA_OPERATION_LOG](AM_FA_OPERATION_LOG.md) | `AM_FA_OPERATION_LOG_ID` |

## Referenced by (1)

| From table | Column |
|------------|--------|
| [AM_FA_OPERATION_LOG](AM_FA_OPERATION_LOG.md) | `PARENT_AM_FA_OPERATION_LOG_ID` |
