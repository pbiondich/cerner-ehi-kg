...
> Table stores service types as submitted on the eligibility inquiry.
...
|--:|--------|------|:-----:|-------|------------|
| 1 | `SERVICE_TYPE_CD` | DOUBLE | NOT NULL |  | The type of service for which eligibility was requested for a patient. |
| 2 | `TRANSACTION_ELIGIBILITY_ID` | DOUBLE | NOT NULL | FK→ | Identifier of the transaction_eligibility table to which this service type is related. |
| 3 | `TRANSACTION_ELIG_SERVICE_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the TRANSACTION_ELIG_SERVICE table. |
