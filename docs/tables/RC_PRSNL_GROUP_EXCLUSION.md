...
> Personnel members of a personnel group that are excluded from personnel preferential agreements.
...
| 2 | `PRSNL_GROUP_ID` | DOUBLE | NOT NULL | FK→ | ID of the personnel group from which the personnel is being exluded. |
| 3 | `PRSNL_ID` | DOUBLE | NOT NULL | FK→ | ID of personnel being excluded from the personnel group. |
| 4 | `RC_PRSNL_GROUP_EXCLUSION_ID` | DOUBLE | NOT NULL |  | Unique generated number that identifies a single row on the RC_PRSNL_GROUP_EXCLUSION table. |
