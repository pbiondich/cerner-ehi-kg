...
| 5 | `BEG_DT_TM` | VARCHAR(20) |  |  | Begin Date and Time value |
...
| 7 | `BOOK_INSTR_ID` | DOUBLE | NOT NULL | FK→ | The unique identifier for the scheduling instruction. |
...
| 10 | `FUTURE_IND` | DOUBLE |  |  | Denotes if the order's future indicator should be set to yes or no. |
...
| 12 | `PRIORITY_CD` | DOUBLE | NOT NULL | FK→ | The coded identifier for the order priority code. |
| 13 | `PRIORITY_MEANING` | VARCHAR(12) |  |  | A 12-character description corresponding to the order priority code. |
...
|--------|--------------|---------------|
| `BOOK_INSTR_ID` | [SCH_BOOK_INSTR](SCH_BOOK_INSTR.md) | `BOOK_INSTR_ID` |
| `PRIORITY_CD` | [CODE_VALUE](CODE_VALUE.md) | `CODE_VALUE` |
